...
export function connectGateway(host: GatewayHost, options?: ConnectGatewayOptions) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void subscribeSessions(host as unknown as DurarApp);
      void loadAssistantIdentity(host as unknown as DurarApp);
      void loadAgents(host as unknown as DurarApp);
      void loadHealthState(host as unknown as DurarApp);
      void loadNodes(host as unknown as DurarApp, { quiet: true });
      void loadDevices(host as unknown as DurarApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (runId && host.refreshSessionsAfterChat.has(runId)) {
    ...
    if (state === "final") {
      void loadSessions(host as unknown as DurarApp, {
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as DurarApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as DurarApp, payload);
