...
This document describes how the Durar ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`Durar acp` exposes an ACP agent over stdio and forwards prompts to a running
Durar Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Durar Gateway
...
- ACP client spawns `Durar acp` and speaks ACP messages over stdio.
...
`Durar acp` resolves the Gateway URL and auth from CLI flags or config:
