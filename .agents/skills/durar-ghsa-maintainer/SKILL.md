...
# Durar GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `Durar-release-maintainer`.
...
```bash
fork=$(gh api /repos/Durar/Durar/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/Durar/Durar/security-advisories/<GHSA> \
