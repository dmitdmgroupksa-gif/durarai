...
```bash
export Durar_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.Durar/Durar.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env Durar_PARALLELS_DISCORD_TOKEN \
...
- Use the `Durar` wrapper for guest `message send/read`; `node Durar.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/Durar-parallels-smoke.*`
...
- host inbound nonce appears in `Durar message read` output
