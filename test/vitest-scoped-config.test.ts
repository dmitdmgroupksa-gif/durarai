...
describe("resolveVitestIsolation", () => {
  ...
  it("restores isolate mode when explicitly requested", () => {
    expect(resolveVitestIsolation({ Durar_TEST_ISOLATE: "1" })).toBe(true);
    expect(resolveVitestIsolation({ Durar_TEST_NO_ISOLATE: "0" })).toBe(true);
    expect(resolveVitestIsolation({ Durar_TEST_NO_ISOLATE: "false" })).toBe(true);
  });
});
...
describe("scoped vitest configs", () => {
  ...
  it("loads channel include overrides from Durar_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-vitest-channels-"));
    try {
      ...
      const config = createChannelsVitestConfig({
        Durar_VITEST_INCLUDE_FILE: includeFile,
      });
