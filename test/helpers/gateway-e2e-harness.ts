...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `Durar-e2e-${name}-`));
  const configDir = path.join(homeDir, ".Durar");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          Durar_SKIP_PROVIDERS: "1",
          Durar_SKIP_GMAIL_WATCHER: "1",
          Durar_SKIP_CRON: "1",
          Durar_SKIP_BROWSER_CONTROL_SERVER: "1",
          Durar_SKIP_CANVAS_HOST: "1",
          Durar_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
