...
function createTempHome(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-test-env-real-home-"));
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-Durar.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.Durar/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.Durar/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".Durar", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".Durar", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.Durar_LIVE_TEST_QUIET = "1";
    process.env.Durar_CONFIG_PATH = "~/custom-Durar.json5";
    process.env.Durar_TEST_HOME = priorIsolatedHome;
    process.env.Durar_STATE_DIR = path.join(priorIsolatedHome, ".Durar");
    ...
    expect(process.env.Durar_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".Durar", "Durar.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".Durar", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".Durar", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.Durar_LIVE_TEST = "1";
    process.env.Durar_LIVE_USE_REAL_HOME = "1";
    process.env.Durar_LIVE_TEST_QUIET = "1";
