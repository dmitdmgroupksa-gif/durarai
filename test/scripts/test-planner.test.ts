...
describe("test planner", () => {
  ...
  it("uses smaller shared extension batches on constrained local hosts", () => {
    const env = {
      RUNNER_OS: "macOS",
      Durar_TEST_HOST_CPU_COUNT: "8",
      Durar_TEST_HOST_MEMORY_GIB: "16",
      Durar_TEST_LOAD_AWARE: "0",
    };
  ...
  it("honors the requested Windows platform when building the cache path", () => {
    expect(
      resolveVitestFsModuleCachePath({
        cwd: "/repo",
        env: {
          Durar_VITEST_FS_MODULE_CACHE: "1",
        },
        platform: "win32",
        unitId: "unit-fast-1",
      }),
  ...
  it("respects an explicit cache path override", () => {
    expect(
      resolveVitestFsModuleCachePath({
        cwd: "/repo",
        env: {
          Durar_VITEST_FS_MODULE_CACHE_PATH: "/tmp/custom-vitest-cache",
        },
        platform: "linux",
        unitId: "unit-fast-1",
      }),
  ...
  it("does not force a cache path when the cache is disabled", () => {
    expect(
      resolveVitestFsModuleCachePath({
        cwd: "/repo",
        env: {
          Durar_VITEST_FS_MODULE_CACHE: "0",
        },
        platform: "linux",
        unitId: "unit-fast-1",
      }),
