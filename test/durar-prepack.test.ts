...
import { collectPreparedPrepackErrors, shouldSkipPrepack } from "../scripts/Durar-prepack.ts";

describe("shouldSkipPrepack", () => {
  it("treats unset and explicit false values as disabled", () => {
    expect(shouldSkipPrepack({})).toBe(false);
    expect(shouldSkipPrepack({ Durar_PREPACK_PREPARED: "0" })).toBe(false);
    expect(shouldSkipPrepack({ Durar_PREPACK_PREPARED: "false" })).toBe(false);
  });

  it("treats non-false values as enabled", () => {
    expect(shouldSkipPrepack({ Durar_PREPACK_PREPARED: "1" })).toBe(true);
    expect(shouldSkipPrepack({ Durar_PREPACK_PREPARED: "true" })).toBe(true);
  });
});
