...
function rewriteFacadeTypeMapSpecifier(specifier: string): string {
  if (!specifier.startsWith("@Durar/")) {
    return specifier;
  }
  return `${GENERATED_FACADE_TYPE_MAP_DIST_PREFIX}${specifier.slice("@Durar/".length)}`;
}
...
function rewriteGeneratedFacadeTypeMapDts(): void {
  ...
  const rewritten = source.replace(/@Durar\/([a-z0-9-]+\/[^")\s]+)/g, (_match, suffix: string) =>
    rewriteFacadeTypeMapSpecifier(`@Durar/${suffix}`),
  );
  if (rewritten !== source) {
