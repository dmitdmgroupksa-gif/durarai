...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "Durar.plugin.json");
    ...
    if (packageJson.Durar && "extensions" in packageJson.Durar) {
      packageJson.Durar = {
        ...packageJson.Durar,
        extensions: rewritePackageExtensions(packageJson.Durar.extensions),
        ...(typeof packageJson.Durar.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.Durar.setupEntry) }
          : {}),
      };
    }
