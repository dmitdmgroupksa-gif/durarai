...
const EXPECTED_REPOSITORY_URL = "https://github.com/dmitdmgroupksa-gif/durarai";
const MAX_CALVER_DISTANCE_DAYS = 2;
const REQUIRED_PACKED_PATHS = ["dist/control-ui/index.html"];
const CONTROL_UI_ASSET_PREFIX = "dist/control-ui/assets/";
const NPM_PACK_MAX_BUFFER_BYTES = 64 * 1024 * 1024;
const skipPackValidationEnv = "Durar_NPM_RELEASE_SKIP_PACK_CHECK";
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`Durar-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `Durar-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
