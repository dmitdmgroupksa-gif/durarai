...
const Durar_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncDurarDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.Durar;
  if (!current || current === "workspace:*" || !Durar_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncMinHostVersion(pkg: PackageJson, targetVersion: string): boolean {
  const installConfig = pkg.Durar?.install;
  const current = installConfig?.minHostVersion;
  if (!current || !Durar_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Durar release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncDurarDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncDurarDependencyRange(pkg.peerDependencies, targetVersion);
