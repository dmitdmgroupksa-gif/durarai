...
# Durar Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://Durar.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Durar."
...
GUM_VERSION="${Durar_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Durar Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://Durar.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$Durar_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.Durar.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_Durar_paths() {
    ...
    rm -rf "$npm_root"/.Durar-* "$npm_root"/Durar 2>/dev/null || true
}

extract_Durar_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_Durar_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/Durar" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/Durar"|"/usr/local/bin/Durar")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/Durar/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale Durar symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing Durar binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Durar package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_Durar_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Durar npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*Durar" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_Durar_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_Durar_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Durar npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an Durar binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${Durar_TAGLINE_INDEX:-}" ]]; then
        if [[ "${Durar_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((Durar_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${Durar_NO_ONBOARD:-0}
NO_PROMPT=${Durar_NO_PROMPT:-0}
DRY_RUN=${Durar_DRY_RUN:-0}
INSTALL_METHOD=${Durar_INSTALL_METHOD:-}
Durar_VERSION=${Durar_VERSION:-latest}
USE_BETA=${Durar_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/Durar"
GIT_DIR=${Durar_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${Durar_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${Durar_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${Durar_VERBOSE:-0}"
VERIFY_INSTALL="${Durar_VERIFY_INSTALL:-0}"
Durar_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Durar installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://Durar.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/Durar)
...
Environment variables:
  Durar_INSTALL_METHOD=git|npm
  Durar_VERSION=latest|next|main|<semver>|<spec>
  Durar_BETA=0|1
  Durar_GIT_DIR=...
  Durar_GIT_UPDATE=0|1
  Durar_NO_PROMPT=1
  Durar_VERIFY_INSTALL=1
  Durar_DRY_RUN=1
  Durar_NO_ONBOARD=1
  Durar_VERBOSE=1
  Durar_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://Durar.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://Durar.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://Durar.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Durar checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Durar source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_Durar_checkout() {
    ...
    if ! grep -Eq '"name"[[:space:]]*:[[:space:]]*"(Durar|durar-cli)"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://Durar.ai/install.sh | bash"
}
...
ensure_Durar_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/Durar" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/Durar" ]]; then
        ln -sf "$npm_root/Durar/dist/entry.js" "${npm_bin}/Durar"
        ui_info "Created Durar bin link at ${npm_bin}/Durar"
    fi
    return 0
}

# Check for existing Durar installation
check_existing_Durar() {
    if [[ -n "$(type -P Durar 2>/dev/null || true)" ]]; then
        ui_info "Existing Durar installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make Durar show as \"command not found\" in new terminals."
...
warn_Durar_not_found() {
    ui_warn "Installed, but Durar is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t Durar 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named Durar; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_Durar_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P Durar 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/Durar" ]]; then
        echo "${npm_bin}/Durar"
        return 0
    fi
...
install_Durar_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/dmitdmgroupksa-gif/durarai.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Durar from git checkout: ${repo_dir}"
    else
        ui_info "Installing Durar from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Durar" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Durar" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/Durar" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/Durar"
    ui_success "Durar wrapper installed to \$HOME/.local/bin/Durar"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view Durar dist-tags.beta 2>/dev/null || true)"
...
install_Durar() {
    ...
    if [[ -z "${Durar_VERSION}" ]]; then
        Durar_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${Durar_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${Durar_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Durar v${resolved_version}"
    else
        ui_info "Installing Durar (${Durar_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${Durar_VERSION}")"

    if ! install_Durar_npm "${install_spec}"; then
    ...
    ensure_Durar_bin_link || true

    ui_success "Durar installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${Durar_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_Durar_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (Durar not on PATH yet)"
        warn_Durar_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${Durar_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.Durar/workspace-${profile}"
    else
        echo "${HOME}/.Durar/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${Durar_CONFIG_PATH:-$HOME/.Durar/Durar.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run Durar onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but Durar not on PATH; skipping onboarding"
        warn_Durar_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run Durar onboard to retry"
        return
    }
}
...
if ! declare -F extract_Durar_semver >/dev/null 2>&1; then
...
resolve_Durar_version() {
    ...
    local claw="${Durar_BIN:-}"
    if [[ -z "$claw" ]] && command -v Durar &> /dev/null; then
        claw="$(command -v Durar)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_Durar_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/Durar/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/Durar/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: Durar not on PATH yet"
        warn_Durar_not_found
        return 1
    fi

    run_quiet_step "Checking Durar version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: Durar gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_Durar_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Durar checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set Durar_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_Durar_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/Durar" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/Durar"
    ...
    Durar_BIN="$(resolve_Durar_bin || true)"
    ...
    installed_version=$(resolve_Durar_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Durar installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Durar installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/Durar"
        ui_kv "Update command" "Durar update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://Durar.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running Durar doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                Durar_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                Durar_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                Durar_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run Durar doctor and Durar plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run Durar onboard later"
        else
            local config_path="${Durar_CONFIG_PATH:-$HOME/.Durar/Durar.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (Durar not on PATH yet)"
                    warn_Durar_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run Durar onboard to finish setup"
            return 0
        fi
    fi

    if command -v Durar &> /dev/null; then
        local claw="${Durar_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_Durar_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (Durar daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if Durar_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: Durar daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${Durar_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
