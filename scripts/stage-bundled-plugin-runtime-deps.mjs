...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.Durar?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.Durar) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.Durar;
  ...
  if (packageJson.peerDependenciesMeta?.Durar) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.Durar;
  ...
  if (packageJson.devDependencies?.Durar) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.Durar;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".Durar-runtime-deps-stamp.json");
}
...
function stageInstalledRootRuntimeDeps(params) {
  ...
  const stagedNodeModulesDir = path.join(
    makeTempDir(
      os.tmpdir(),
      `Durar-runtime-deps-${sanitizeTempPrefixSegment(path.basename(pluginDir))}-`,
    ),
    "node_modules",
  );
...
function installPluginRuntimeDeps(params) {
  ...
  const tempInstallDir = makeTempDir(
    os.tmpdir(),
    `Durar-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}-`,
  );
