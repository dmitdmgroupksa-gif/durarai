...
EXTRA_MOUNTS="${Durar_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${Durar_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${Durar_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${Durar_DOCKER_SOCKET:-}"
TIMEZONE="${Durar_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$Durar_CONFIG_DIR/Durar.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == Durar_GATEWAY_TOKEN=* ]]; then
      token="${line#Durar_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
ensure_control_ui_allowed_origins() {
  if [[ "${Durar_GATEWAY_BIND}" == "loopback" ]]; then
    return 0
  fi

  local allowed_origin_json
  local current_allowed_origins
  allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$Durar_GATEWAY_PORT" "$Durar_GATEWAY_PORT")"
...
sync_gateway_mode_and_bind() {
  ...
  run_prestart_cli config set gateway.bind "$Durar_GATEWAY_BIND" >/dev/null
  echo "Pinned gateway.mode=local and gateway.bind=$Durar_GATEWAY_BIND for Docker setup."
}
...
run_prestart_cli() {
  # During setup, avoid the shared-network Durar-cli service because it
  ...
  run_prestart_gateway --entrypoint node Durar-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" Durar-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "Durar_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "Durar_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
Durar_CONFIG_DIR="${Durar_CONFIG_DIR:-$HOME/.Durar}"
Durar_WORKSPACE_DIR="${Durar_WORKSPACE_DIR:-$HOME/.Durar/workspace}"

validate_mount_path_value "Durar_CONFIG_DIR" "$Durar_CONFIG_DIR"
validate_mount_path_value "Durar_WORKSPACE_DIR" "$Durar_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "Durar_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "Durar_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "Durar_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "Durar_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "Durar_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "Durar_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$Durar_CONFIG_DIR"
mkdir -p "$Durar_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$Durar_CONFIG_DIR/identity"
mkdir -p "$Durar_CONFIG_DIR/agents/main/agent"
mkdir -p "$Durar_CONFIG_DIR/agents/main/sessions"

export Durar_CONFIG_DIR
export Durar_WORKSPACE_DIR
export Durar_GATEWAY_PORT="${Durar_GATEWAY_PORT:-18789}"
export Durar_BRIDGE_PORT="${Durar_BRIDGE_PORT:-18790}"
export Durar_GATEWAY_BIND="${Durar_GATEWAY_BIND:-lan}"
export Durar_IMAGE="$IMAGE_NAME"
export Durar_DOCKER_APT_PACKAGES="${Durar_DOCKER_APT_PACKAGES:-}"
export Durar_EXTENSIONS="${Durar_EXTENSIONS:-}"
export Durar_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export Durar_HOME_VOLUME="$HOME_VOLUME_NAME"
export Durar_ALLOW_INSECURE_PRIVATE_WS="${Durar_ALLOW_INSECURE_PRIVATE_WS:-}"
export Durar_SANDBOX="$SANDBOX_ENABLED"
export Durar_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export Durar_TZ="$TIMEZONE"
...
if [[ -z "${Durar_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    Durar_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $Durar_CONFIG_DIR/Durar.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      Durar_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      Durar_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      Durar_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${Durar_CONFIG_DIR}:/home/node/.Durar"
    gateway_workspace_mount="${Durar_WORKSPACE_DIR}:/home/node/.Durar/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${Durar_INSTALL_DOCKER_CLI:-}" ]]; then
    export Durar_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  Durar_EXTRA_MOUNTS \
  Durar_HOME_VOLUME \
  Durar_DOCKER_APT_PACKAGES \
  Durar_EXTENSIONS \
  Durar_SANDBOX \
  Durar_DOCKER_SOCKET \
  DOCKER_GID \
  Durar_INSTALL_DOCKER_CLI \
  Durar_ALLOW_INSECURE_PRIVATE_WS \
  Durar_TZ

if [[ "$IMAGE_NAME" == "Durar:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "Durar_DOCKER_APT_PACKAGES=${Durar_DOCKER_APT_PACKAGES}" \
    --build-arg "Durar_EXTENSIONS=${Durar_EXTENSIONS}" \
    --build-arg "Durar_INSTALL_DOCKER_CLI=${Durar_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Durar metadata subdirectory
# (.Durar/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh Durar-gateway -c \
  'find /home/node/.Durar -xdev -exec chown node:node {} +; \
   [ -d /home/node/.Durar/workspace/.Durar ] && chown -R node:node /home/node/.Durar/workspace/.Durar || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from Durar_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $Durar_GATEWAY_BIND"
echo "Gateway token: $Durar_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm Durar-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm Durar-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm Durar-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.Durar.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d Durar-gateway

# --- Sandbox setup (opt-in via Durar_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: Durar-sandbox:bookworm-slim"
    run_docker_build \
      -t "Durar-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker Durar-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg Durar_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (Durar_IMAGE=Durar:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: Durar_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Durar config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.Durar.ai/gateway/sandboxing"
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate Durar-gateway
  fi
else
...
echo "Config: $Durar_CONFIG_DIR"
echo "Workspace: $Durar_WORKSPACE_DIR"
echo "Token: $Durar_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f Durar-gateway"
echo "  ${COMPOSE_HINT} exec Durar-gateway node dist/index.js health --token \"$Durar_GATEWAY_TOKEN\""
