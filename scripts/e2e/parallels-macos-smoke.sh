...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://Durar.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_Durar_BIN="/opt/homebrew/bin/Durar"
GUEST_Durar_ENTRY="/opt/homebrew/lib/node_modules/Durar/Durar.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/Durar-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/Durar-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://Durar.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: Durar@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__Durar_READY__\\n'\r"
expect "__Durar_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/Durar-prl.sh <<'__Durar_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__Durar_SCRIPT__\r"
send -- "/bin/bash /tmp/Durar-prl.sh; rc=\$?; rm -f /tmp/Durar-prl.sh; printf '__Durar_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1
...
resolve_latest_version() {
  ...
  npm view Durar version --userconfig "$(mktemp)"
}
...
export Durar_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/Durar-install.sh
bash /tmp/Durar-install.sh${version_arg_q}
$GUEST_Durar_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_Durar_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/Durar-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/Durar-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/Durar"; check_path "\$root/Durar/extensions"; if [ -d "\$root/Durar/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/Durar/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_Durar_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_Durar_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_Durar_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_Durar_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli "$GUEST_Durar_BIN" models set "$MODEL_ID"
  guest_current_user_cli \
    /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" \
    "$GUEST_Durar_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_Durar_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from Durar dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/Durar-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Durar Control</title>' /tmp/Durar-dashboard-smoke.html >/dev/null; then
      if grep -F '<Durar-app></Durar-app>' /tmp/Durar-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>Durar Control</title>' /tmp/Durar-dashboard-smoke.html >/dev/null
grep -F '<Durar-app></Durar-app>' /tmp/Durar-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/Durar-discord-token <<'__Durar_TOKEN__'
$DISCORD_TOKEN_VALUE
__Durar_TOKEN__
cat >/tmp/Durar-discord-guilds.json <<'__Durar_GUILDS__'
$guilds_json
__Durar_GUILDS__
token="\$(tr -d '\n' </tmp/Durar-discord-token)"
guilds_json="\$(cat /tmp/Durar-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_Durar_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_Durar_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_Durar_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_Durar_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_Durar_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_Durar_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_Durar_ENTRY channels status --probe --json
rm -f /tmp/Durar-discord-token /tmp/Durar-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "Durar-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "Durar-main-upgrade.tgz"
