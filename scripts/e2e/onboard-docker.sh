...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/Durar-bin:$PATH"
  mkdir -p /tmp/Durar-bin
  cat > /tmp/Durar-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$Durar_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_Durar_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/Durar-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/Durar-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$Durar_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/Durar-e2e-$1.XXXXXX"
  }

  set_isolated_Durar_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export Durar_HOME="$home_dir"
    export Durar_STATE_DIR="$home_dir/.Durar"
    export Durar_CONFIG_PATH="$Durar_STATE_DIR/Durar.json"
    mkdir -p "$Durar_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$Durar_STATE_DIR/workspace"
    config_path="$Durar_CONFIG_PATH"
    sessions_dir="$Durar_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_Durar_env "$home_dir"
	    ...
	    node "$Durar_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$Durar_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$Durar_ENTRY\" configure --section skills" send_skills_flow
