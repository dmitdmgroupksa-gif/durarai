...
IMAGE_NAME="${Durar_IMAGE:-Durar-mcp-channels-e2e}"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="Durar-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t Durar-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "Durar_STATE_DIR=/tmp/Durar-state" \
  -e "Durar_CONFIG_PATH=/tmp/Durar-state/Durar.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "Durar_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
