...
const extensionRestartMetadataFiles = new Set(["Durar.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.Durar_FORCE_BUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by Durar_FORCE_BUILD",
...
const logRunner = (message, deps) => {
  if (deps.env.Durar_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[Durar] ${message}\n`);
};

const runDurar = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["Durar.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
