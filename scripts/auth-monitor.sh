...
#
# Suggested cron: */30 * * * * /home/admin/Durar/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Durar notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., Durar-alerts)
...
STATE_FILE="$HOME/.Durar/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Durar if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use Durar
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Durar to $NOTIFY_PHONE..."
            Durar send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Durar is down. Run: ssh l36 '~/Durar/scripts/mobile-reauth.sh'" "urgent"
    exit 1
