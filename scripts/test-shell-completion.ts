 ...
 * `Durar update`. Use it to verify the completion installation flow
...
\`Durar update\`, \`Durar doctor\`, and \`Durar onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Durar Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`Durar completion --install\` later to enable.`));
    return;
  }
