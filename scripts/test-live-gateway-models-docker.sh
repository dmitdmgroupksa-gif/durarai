...
elif [[ -n "${Durar_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(Durar_live_collect_auth_dirs_from_csv "${Durar_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(Durar_live_collect_auth_files_from_csv "${Durar_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e Durar_LIVE_TEST=1 \
  -e Durar_LIVE_GATEWAY_MODELS="${Durar_LIVE_GATEWAY_MODELS:-modern}" \
  -e Durar_LIVE_GATEWAY_PROVIDERS="${Durar_LIVE_GATEWAY_PROVIDERS:-}" \
  -e Durar_LIVE_GATEWAY_MAX_MODELS="${Durar_LIVE_GATEWAY_MAX_MODELS:-24}" \
  -e Durar_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${Durar_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}" \
  -v "$ROOT_DIR":/src:ro \
