...
# Build and bundle Durar into a minimal .app we can open.
# Outputs to dist/Durar.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Durar.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Durar"
BUNDLE_ID="${BUNDLE_ID:-ai.Durar.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/Durar/Durar/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Durar/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :DurarBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :DurarGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Durar"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Durar"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Durar"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Durar" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Durar/Resources/Durar.icns" "$APP_ROOT/Contents/Resources/Durar.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Durar/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying DurarKit resources"
DurarKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/DurarKit_DurarKit.bundle"
if [ -d "$DurarKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/DurarKit_DurarKit.bundle"
  cp -R "$DurarKIT_BUNDLE" "$APP_ROOT/Contents/Resources/DurarKit_DurarKit.bundle"
else
  echo "WARN: DurarKit resource bundle not found at $DurarKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Durar"
killall -q Durar 2>/dev/null || true
