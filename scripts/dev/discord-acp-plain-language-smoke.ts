...
type DriverMode = "token" | "webhook" | "Durar";
...
function resolveStateDir(): string {
  const override = process.env.Durar_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.Durar_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".Durar");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver Durar] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Durar spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|Durar> Driver transport mode (default: token)\n" +
    ...
    "  --Durar-bin <path>        Durar CLI binary for driver=Durar (default: Durar)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  Durar_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  Durar_DISCORD_SMOKE_DRIVER\n" +
    "  Durar_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  Durar_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  Durar_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  Durar_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  Durar_DISCORD_SMOKE_AGENT\n" +
    "  Durar_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  Durar_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  Durar_DISCORD_SMOKE_POLL_MS\n" +
    "  Durar_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  Durar_DISCORD_SMOKE_Durar_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.Durar_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.Durar_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "Durar"
        ? "Durar"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.Durar_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.Durar_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.Durar_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.Durar_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.Durar_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.Durar_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.Durar_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.Durar_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.Durar_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.Durar_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const DurarBin =
    resolveArg("--Durar-bin") || process.env.Durar_DISCORD_SMOKE_Durar_BIN || "Durar";
...
async function DurarCliJson<T>(params: { DurarBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.DurarBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`Durar ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithDurar(params: {
  DurarBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await DurarCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    DurarBin: params.DurarBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "Durar") {
    return await readMessagesWithDurar({
      DurarBin: params.args.DurarBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `Durar-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("Durar message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "Durar"
            ? await readMessagesWithDurar({
                DurarBin: args.DurarBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Durar.`,
        diagnostics: {
          bindingCandidates: [
            {
