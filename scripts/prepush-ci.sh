...
run_protocol_ci_mirror() {
  local targets=(
    ...
    "apps/macos/Sources/DurarProtocol/GatewayModels.swift"
    "apps/shared/DurarKit/Sources/DurarProtocol/GatewayModels.swift"
  )
  local before after
...
has_native_swift_changes() {
  if git rev-parse --verify --quiet origin/main >/dev/null; then
    if git diff --name-only --relative origin/main...HEAD -- apps/macos apps/ios apps/shared/DurarKit | rg -q .; then
      return 0
    fi
  fi

  if git rev-parse --verify --quiet HEAD^ >/dev/null; then
    git diff --name-only --relative HEAD^..HEAD -- apps/macos apps/ios apps/shared/DurarKit | rg -q .
    return $?
  fi

  git show --name-only --relative --pretty='' HEAD -- apps/macos apps/ios apps/shared/DurarKit | rg -q .
}
...
run_linux_ci_mirror() {
  ...
  log_step "Durar_TEST_WORKERS=${Durar_TEST_WORKERS:-1} Durar_TEST_MAX_OLD_SPACE_SIZE_MB=${Durar_TEST_MAX_OLD_SPACE_SIZE_MB:-6144} pnpm test"
  Durar_TEST_WORKERS="${Durar_TEST_WORKERS:-1}" \
  Durar_TEST_MAX_OLD_SPACE_SIZE_MB="${Durar_TEST_MAX_OLD_SPACE_SIZE_MB:-6144}" \
    pnpm test
}

run_macos_ci_mirror() {
  if [[ "${Durar_PREPUSH_SKIP_MACOS:-0}" == "1" ]]; then
    log_step "Skipping macOS mirror because Durar_PREPUSH_SKIP_MACOS=1"
    return 0
  fi
