#!/usr/bin/env bash
# Rootless durar in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-durar-podman.sh launch        # Start gateway
#   ./scripts/run-durar-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   durar --container durar dashboard --no-open
#   durar --container durar channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      durar_GATEWAY_TOKEN|durar_PODMAN_CONTAINER|durar_PODMAN_IMAGE|durar_IMAGE|durar_PODMAN_PULL|durar_PODMAN_GATEWAY_HOST_PORT|durar_GATEWAY_PORT|durar_PODMAN_BRIDGE_HOST_PORT|durar_BRIDGE_PORT|durar_GATEWAY_BIND|durar_PODMAN_USERNS|durar_BIND_MOUNT_OPTIONS|durar_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-durar-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${durar_CONFIG_DIR:-$EFFECTIVE_HOME/.durar}"
ENV_FILE="${durar_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${durar_CONFIG_DIR:-$EFFECTIVE_HOME/.durar}"
ENV_FILE="${durar_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${durar_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${durar_PODMAN_CONTAINER:-durar}"
durar_IMAGE="${durar_PODMAN_IMAGE:-${durar_IMAGE:-durar:local}}"
PODMAN_PULL="${durar_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${durar_PODMAN_GATEWAY_HOST_PORT:-${durar_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${durar_PODMAN_BRIDGE_HOST_PORT:-${durar_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${durar_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$durar_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v durar >/dev/null 2>&1; then
    return 0
  fi
  durar_CONTAINER="" durar_CONFIG_DIR="$config_dir" \
    durar config get gateway.bind 2>/dev/null || true
}
...
# durar_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${durar_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v durar >/dev/null 2>&1; then
    echo "Warning: durar not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    durar_CONTAINER="" durar_CONFIG_DIR="$config_dir" \
      durar config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    durar_CONTAINER="" durar_CONFIG_DIR="$config_dir" \
      durar config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  durar_CONTAINER="" durar_CONFIG_DIR="$config_dir" \
    durar config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${durar_GATEWAY_TOKEN:-}" ]]; then
  export durar_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "durar_GATEWAY_TOKEN" "$durar_GATEWAY_TOKEN"
  echo "Generated durar_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/durar.json"
...
PODMAN_USERNS="${durar_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported durar_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (durar_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${durar_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${durar_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$durar_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.durar/.npm \
    -e durar_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.durar:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.durar/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$durar_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME durar dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME durar devices approve --latest  # if pairing required"
