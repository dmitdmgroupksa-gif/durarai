...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.Durar) ? packageJson.Durar : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.Durar?.channel?.id) || left.name;
    const rightId = trimString(right.Durar?.channel?.id) || right.name;
