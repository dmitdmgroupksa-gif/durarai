...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("Durar-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/Durar/plugins/diffs/view/",
    );
    expect((result?.details as Record<string, unknown>).viewerUrl).toEqual(
      expect.stringContaining("https://example.com/Durar/plugins/diffs/view/"),
    );
  });
...
function createApi(pluginConfig?: Record<string, unknown>): DurarPluginApi {
  return createTestPluginApi({
    ...
    pluginConfig,
    runtime: {} as DurarPluginApi["runtime"],
  }) as DurarPluginApi;
}
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: DurarPluginToolContext | undefined = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
