/**
 * Synology Chat Channel Plugin for Durar.
 *
...
} from "Durar/plugin-sdk/channel-config-helpers";
import { waitUntilAbort } from "Durar/plugin-sdk/channel-lifecycle";
import {
...
import { attachChannelToResult } from "Durar/plugin-sdk/channel-send-result";
import { createChatChannelPlugin, type ChannelPlugin } from "Durar/plugin-sdk/core";
import { createEmptyChannelDirectoryAdapter } from "Durar/plugin-sdk/directory-runtime";
import { DEFAULT_ACCOUNT_ID } from "Durar/plugin-sdk/setup";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "Durar pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: DurarConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: DurarConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to Durar",
        order: 90,
      },
      capabilities: {
    ...
    pairing: {
      text: {
        ...
        message: "Durar: your access has been approved.",
