...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): DurarConfig {
    const defaults: NonNullable<NonNullable<DurarConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: DurarConfig) {
