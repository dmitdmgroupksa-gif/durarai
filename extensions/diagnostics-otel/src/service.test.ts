...
import type { DurarPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/Durar-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): DurarPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("Durar.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("Durar.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("Durar.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("Durar.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("Durar.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("Durar.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("Durar.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("Durar.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("Durar.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("Durar.webhook.processed");
    expect(spanNames).toContain("Durar.message.processed");
    expect(spanNames).toContain("Durar.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["Durar.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("Durar.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "Durar.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["Durar.reason"]).toBe("string");
    expect(String(attrs?.["Durar.reason"])).not.toContain(
