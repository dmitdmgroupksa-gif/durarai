...
let resetInboundDedupe: typeof import("Durar/plugin-sdk/reply-runtime").resetInboundDedupe;
...
beforeEach(async () => {
  vi.resetModules();
  ({ resetInboundDedupe } = await import("Durar/plugin-sdk/reply-runtime"));
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bDurar\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("Durar: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("Durar: hello <@U2>");
  });
