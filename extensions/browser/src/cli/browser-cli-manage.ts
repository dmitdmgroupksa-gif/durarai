...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "Durar";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "Durar" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (Durar|existing-session). Default: Durar")
    .action(
      async (
        opts: {
          name: string;
