...
export function resolveDurarUserDataDir(profileName = DEFAULT_Durar_BROWSER_PROFILE_NAME) {
...
export function buildDurarChromeLaunchArgs(params: {
...
export async function launchDurarChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveDurarUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_Durar_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildDurarChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateDurarProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 Durar browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`Durar browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`Durar browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 Durar browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopDurarChrome(
