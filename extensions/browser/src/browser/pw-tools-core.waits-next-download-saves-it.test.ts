...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredDurarTmpDir: vi.fn(() => "/tmp/Durar"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-Durar-dir.js");
...
describe("pw-tools-core", () => {
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-Durar-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredDurarTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredDurarTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredDurarTmpDir.mockReturnValue("/tmp/Durar");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".Durar-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredDurarTmpDir.mockReturnValue("/tmp/Durar-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "Durar-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "Durar-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredDurarTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "Durar-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "Durar-preferred", "downloads")}${path.sep}`),
    );
  });
