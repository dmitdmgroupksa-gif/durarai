...
vi.mock("./chrome.js", () => ({
  resolveDurarUserDataDir: vi.fn(() => "/tmp/Durar-test/Durar/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "Durar-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveDurarUserDataDir).mockReturnValue(userDataDir);
