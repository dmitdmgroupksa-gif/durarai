...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["DurarPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["Durar", "pollId"]) ??
    readNestedString(value, ["Durar", "poll", "id"]) ??
    readNestedString(value, ["data", "DurarPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "Durar", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "Durar poll vote",
          displayText: "Vote recorded",
          value: { DurarPollId: pollId, pollId },
        },
      },
    ],
  };
