import type { DurarConfig, ProviderAuthResult } from "Durar/plugin-sdk/provider-auth";
import { readClaudeCliCredentialsCached } from "Durar/plugin-sdk/provider-auth";

const DEFAULT_CLAUDE_CLI_MODEL = "claude-cli/claude-sonnet-4-6";
type AgentDefaultsModel = NonNullable<NonNullable<DurarConfig["agents"]>["defaults"]>["model"];
type AgentDefaultsModels = NonNullable<NonNullable<DurarConfig["agents"]>["defaults"]>["models"];
...
export function buildAnthropicCliMigrationResult(config: DurarConfig): ProviderAuthResult {
