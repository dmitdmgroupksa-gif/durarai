...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): DurarConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as DurarConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): DurarConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as DurarConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: DurarConfig, accountId = "media") {
    await handleTelegramAction(
