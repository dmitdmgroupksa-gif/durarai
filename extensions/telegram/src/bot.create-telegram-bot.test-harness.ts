import { resolveDefaultModelForAgent } from "Durar/plugin-sdk/agent-runtime";
import type { DurarConfig } from "Durar/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "Durar/plugin-sdk/reply-runtime";
import type { MsgContext } from "Durar/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "Durar/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "Durar/plugin-sdk/reply-runtime";
import type { MockFn } from "Durar/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("Durar/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("Durar/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("Durar/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("Durar/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/Durar-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("Durar/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("Durar/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("Durar/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("Durar/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("Durar/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: DurarConfig): {
...
vi.doMock("Durar/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("Durar/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("Durar/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("Durar/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("Durar/plugin-sdk/channel-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/channel-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: DurarConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: DurarConfig) => {
