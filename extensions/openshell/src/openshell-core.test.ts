...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from Durar config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "Durar-session-1234",
        backendId: "openshell",
        runtimeLabel: "Durar-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "Durar-session-1234",
        config: expect.objectContaining({
          from: "custom-source",
        }),
      }),
      args: ["sandbox", "get", "Durar-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "Durar-session-5678",
        backendId: "openshell",
        runtimeLabel: "Durar-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "Durar",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "Durar-session-5678",
        config: expect.objectContaining({
          command: "/usr/local/bin/openshell",
          gateway: "lab",
        }),
      }),
      args: ["sandbox", "delete", "Durar-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("Durar-openshell-fs-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("Durar-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("Durar-openshell-agent-");
  ...
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("Durar-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("Durar-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("Durar-openshell-remote-agent-");
