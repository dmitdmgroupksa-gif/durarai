...
} from "Durar/plugin-sdk/allowlist-config-edit";
import { createScopedDmSecurityResolver } from "Durar/plugin-sdk/channel-config-helpers";
import { createPairingPrefixStripper } from "Durar/plugin-sdk/channel-pairing";
import { createOpenProviderConfiguredRouteWarningCollector } from "Durar/plugin-sdk/channel-policy";
import { resolveTargetsWithOptionalToken } from "Durar/plugin-sdk/channel-targets";
...
} from "Durar/plugin-sdk/outbound-runtime";
import { normalizeMessageChannel } from "Durar/plugin-sdk/routing";
import {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Durar adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
