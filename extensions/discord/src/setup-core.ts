import { DEFAULT_ACCOUNT_ID } from "Durar/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "Durar/plugin-sdk/config-runtime";
import type { DurarConfig } from "Durar/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "Durar/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "Durar/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "Durar/plugin-sdk/setup-runtime";
import { formatDocsLink } from "Durar/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: DurarConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: DurarConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: DurarConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: DurarConfig; accountId: string }) =>
    ...
    disable: (cfg: DurarConfig) => setSetupChannelEnabled(cfg, channel, false),
