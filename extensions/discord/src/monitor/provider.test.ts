...
import { AcpRuntimeError } from "Durar/plugin-sdk/acp-runtime";
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): DurarConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("Durar/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/plugin-runtime")>(
