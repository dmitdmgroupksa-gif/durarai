...
const { STORE_PATH, mockSessionStoreEntries } = vi.hoisted(() => ({
  STORE_PATH: "/tmp/Durar-exec-approvals-test.json",
...
beforeEach(() => {
  ...
  mockResolveGatewayConnectionAuth.mockReset().mockImplementation(
    async (params: {
      ...
      const envToken = params.env.Durar_GATEWAY_TOKEN;
      const envPassword = params.env.Durar_GATEWAY_PASSWORD;
...
vi.mock("../../../../src/gateway/operator-approvals-client.js", () => ({
  createOperatorApprovalsGatewayClient: async (params: {
    ...
    const envUrl = process.env.Durar_GATEWAY_URL?.trim();
...
describe("DiscordExecApprovalHandler gateway auth", () => {
  ...
  it("prefers Durar_GATEWAY_TOKEN when config token is missing", async () => {
    vi.stubEnv("Durar_GATEWAY_TOKEN", "env-gateway-token");
    const handler = new DiscordExecApprovalHandler({
      token: "discord-bot-token",
      accountId: "default",
      config: { enabled: true, approvers: ["123"] },
...
describe("DiscordExecApprovalHandler gateway auth resolution", () => {
  ...
  it("passes env URL overrides to shared gateway auth resolver", async () => {
    const previousGatewayUrl = process.env.Durar_GATEWAY_URL;
    try {
      process.env.Durar_GATEWAY_URL = "wss://gateway-from-env.example/ws";
      const handler = new DiscordExecApprovalHandler({
        token: "test-token",
        accountId: "default",
        config: { enabled: true, approvers: ["123"] },
      ...
      if (typeof previousGatewayUrl === "string") {
        process.env.Durar_GATEWAY_URL = previousGatewayUrl;
      } else {
