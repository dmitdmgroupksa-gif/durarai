...
import { resolveAgentDir } from "Durar/plugin-sdk/agent-runtime";
import { agentCommandFromIngress } from "Durar/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "Durar/plugin-sdk/agent-runtime";
import type { DurarConfig } from "Durar/plugin-sdk/config-runtime";
import type { DiscordAccountConfig, TtsConfig } from "Durar/plugin-sdk/config-runtime";
import { transcribeAudioFile } from "Durar/plugin-sdk/media-understanding-runtime";
...
import { parseTtsDirectives } from "Durar/plugin-sdk/speech";
import { textToSpeech } from "Durar/plugin-sdk/speech-runtime";
import { formatErrorMessage } from "Durar/plugin-sdk/ssrf-runtime";
import { resolvePreferredDurarTmpDir } from "Durar/plugin-sdk/temp-path";
...
function resolveVoiceTtsConfig(params: { cfg: DurarConfig; override?: TtsConfig }): {
  cfg: DurarConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredDurarTmpDir(), "discord-voice-"));
