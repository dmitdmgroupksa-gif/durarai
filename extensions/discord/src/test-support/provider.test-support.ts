...
import type { DurarConfig } from "../../../../src/plugin-sdk/discord.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: DurarConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): DurarConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("Durar/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/acp-runtime")>(
    "Durar/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("Durar/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/command-auth")>(
...
vi.mock("Durar/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/reply-runtime")>(
...
vi.mock("Durar/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/config-runtime")>(
...
vi.mock("Durar/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/runtime-env")>(
    "Durar/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("Durar/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/infra-runtime")>(
