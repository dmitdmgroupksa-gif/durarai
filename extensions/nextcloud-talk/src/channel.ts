import { describeWebhookAccountSnapshot } from "Durar/plugin-sdk/account-helpers";
...
import { createAllowlistProviderRouteAllowlistWarningCollector } from "Durar/plugin-sdk/channel-policy";
import { createChatChannelPlugin } from "Durar/plugin-sdk/core";
import { runStoppablePassiveMonitor } from "Durar/plugin-sdk/extension-shared";
import {
    ...
    base: {
      ...
      gateway: {
        ...
        logoutAccount: async ({ accountId, cfg }) => {
          const nextCfg = { ...cfg } as DurarConfig;
          ...
          if (changed) {
            if (nextSection && Object.keys(nextSection).length > 0) {
              ...
              if (Object.keys(nextChannels).length > 0) {
                nextCfg.channels = nextChannels as DurarConfig["channels"];
              } else {
