...
export function listQQBotAccountIds(cfg: DurarConfig): string[] {
...
export function resolveDefaultQQBotAccountId(cfg: DurarConfig): string {
  const qqbot = cfg.channels?.qqbot as QQBotChannelConfig | undefined;
  if (qqbot?.appId || process.env.QQBOT_APP_ID) {
...
/** Apply account config updates back into the Durar config object. */
