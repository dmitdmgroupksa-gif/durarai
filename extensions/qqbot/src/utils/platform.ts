...
/**
 * Return a path under `~/.Durar/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".Durar", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.Durar/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under Durar's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".Durar", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".Durar", "workspace", "qqbot");
