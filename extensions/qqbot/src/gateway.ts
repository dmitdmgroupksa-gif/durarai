...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  } else if (isGlobalTTSAvailable(cfg as DurarConfig)) {
    const globalProvider = (cfg as DurarConfig).messages?.tts?.provider ?? "auto";
    log?.info(
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        const hasTTS =
          ...
          isGlobalTTSAvailable(cfg as DurarConfig);
