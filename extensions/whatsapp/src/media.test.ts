...
import { optimizeImageToPng } from "Durar/plugin-sdk/media-runtime";
import { resolveStateDir } from "Durar/plugin-sdk/state-paths";
import { resolvePreferredDurarTmpDir } from "Durar/plugin-sdk/temp-path";
import { mockPinnedHostnameResolution } from "Durar/plugin-sdk/testing";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredDurarTmpDir(), "Durar-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub Durar_STATE_DIR.
    // Also keep it outside the Durar temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["Durar_STATE_DIR"]);
    process.env.Durar_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "Durar-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredDurarTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Durar state workspace and sandbox roots", async () => {
  ...
  it("rejects default Durar state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
