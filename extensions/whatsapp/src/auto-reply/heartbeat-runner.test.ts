import { redactIdentifier } from "Durar/plugin-sdk/logging-core";
import type { getReplyFromConfig } from "Durar/plugin-sdk/reply-runtime";
import { HEARTBEAT_TOKEN } from "Durar/plugin-sdk/reply-runtime";
...
vi.mock("Durar/plugin-sdk/agent-runtime", () => ({
...
vi.mock("Durar/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/routing")>();
  return {
    ...actual,
...
vi.mock("Durar/plugin-sdk/channel-runtime", async (importOriginal) => {
...
vi.mock("Durar/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/runtime-env")>();
  const logger = {
    child: () => logger,
...
vi.mock("Durar/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    redactIdentifier,
  };
...
vi.mock("Durar/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.mock("../auth-store.js", () => ({
  WA_WEB_AUTH_DIR: "/tmp/Durar-oauth/whatsapp/default",
  resolveDefaultWebAuthDir: () => "/tmp/Durar-oauth/whatsapp/default",
