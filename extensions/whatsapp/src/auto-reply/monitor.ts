import { resolveInboundDebounceMs } from "Durar/plugin-sdk/channel-inbound";
import { enqueueSystemEvent } from "Durar/plugin-sdk/channel-runtime";
import { formatCliCommand } from "Durar/plugin-sdk/cli-runtime";
import { waitForever } from "Durar/plugin-sdk/cli-runtime";
import { hasControlCommand } from "Durar/plugin-sdk/command-detection";
import { loadConfig } from "Durar/plugin-sdk/config-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "Durar/plugin-sdk/reply-history";
import { getReplyFromConfig } from "Durar/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "Durar/plugin-sdk/routing";
import { logVerbose } from "Durar/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "Durar/plugin-sdk/runtime-env";
import { getChildLogger } from "Durar/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "Durar/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("Durar channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("Durar channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
