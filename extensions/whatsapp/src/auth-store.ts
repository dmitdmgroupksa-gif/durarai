...
import { info, success } from "Durar/plugin-sdk/runtime-env";
import { getChildLogger } from "Durar/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "Durar/plugin-sdk/runtime-env";
import { resolveOAuthDir } from "Durar/plugin-sdk/state-paths";
import type { WebChannel } from "Durar/plugin-sdk/text-runtime";
import { resolveUserPath } from "Durar/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("Durar channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
