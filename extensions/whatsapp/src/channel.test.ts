...
import { createWhatsAppPollFixture, expectWhatsAppPollSent } from "Durar/plugin-sdk/testing";
...
import type { DurarConfig } from "./runtime-api.js";
...
const hoisted = vi.hoisted(() => ({
  ...
  listWhatsAppAccountIds: vi.fn((cfg: DurarConfig) => {
  ...
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/Durar-whatsapp-test",
  })),
}));
...
vi.mock("Durar/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/setup")>(
    "Durar/plugin-sdk/setup",
  );
  return {
    ...actual,
...
describe("whatsapp setup wizard", () => {
  ...
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/Durar-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("Durar channels login"),
      "WhatsApp",
    );
  });
