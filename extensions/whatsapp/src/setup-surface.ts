...
import {
  ...
  type DmPolicy,
  type DurarConfig,
} from "Durar/plugin-sdk/setup";
import type { ChannelSetupWizard } from "Durar/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "Durar/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: DurarConfig,
  patch: Partial<NonNullable<NonNullable<DurarConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: DurarConfig, dmPolicy: DmPolicy): DurarConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: DurarConfig, allowFrom?: string[]): DurarConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: DurarConfig, selfChatMode: boolean): DurarConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: DurarConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Durar can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Durar" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("Durar channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
