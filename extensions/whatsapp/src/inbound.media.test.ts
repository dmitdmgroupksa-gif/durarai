...
vi.mock("Durar/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("Durar/plugin-sdk/media-runtime")>(
    "Durar/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `Durar-inbound-media-${crypto.randomUUID()}`);
