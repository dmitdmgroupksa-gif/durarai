...
import type { DurarPluginApi, DurarPluginToolContext } from "../runtime-api.js";
import {
...
function fakeApi(overrides: Partial<DurarPluginApi> = {}): DurarPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<DurarPluginToolContext> = {}): DurarPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: DurarPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
...
describe("resolveWindowsLobsterSpawn", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-lobster-win-spawn-"));
    setProcessPlatform("win32");
  });
