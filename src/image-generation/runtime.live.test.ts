...
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.Durar_LIVE_REQUIRE_PROFILE_KEYS);
...
function withPluginsEnabled(cfg: DurarConfig): DurarConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
...
describeLive("image generation live (provider sweep)", () => {
  it("generates images for every configured image-generation variant with available auth", async () => {
    ...
    const providerFilter = parseCsvFilter(process.env.Durar_LIVE_IMAGE_GENERATION_PROVIDERS);
    const caseFilter = parseCaseFilter(process.env.Durar_LIVE_IMAGE_GENERATION_CASES);
    const envModelMap = parseProviderModelMap(process.env.Durar_LIVE_IMAGE_GENERATION_MODELS);
