...
This directory contains hooks that ship with Durar. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.Durar/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.Durar/Durar.json`:
...
2. Restart gateway: `pkill -9 -f 'Durar.*gateway' && pnpm Durar gateway`
3. Enable the hook: `Durar hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.Durar.ai/automation/hooks
