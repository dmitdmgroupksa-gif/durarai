...
homepage: https://docs.Durar.ai/automation/hooks#command-logger
metadata:
  {
    "Durar":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.Durar/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.Durar/logs/commands.log ~/.Durar/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/Durar`:
   ```
   /home/username/.Durar/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
