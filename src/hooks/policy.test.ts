...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "Durar-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "Durar-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "Durar-bundled");
      const plugin = makeHookEntry("shared", "Durar-plugin");
      const managed = makeHookEntry("shared", "Durar-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("Durar-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "Durar-managed");
      const workspace = makeHookEntry("shared", "Durar-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "Durar-workspace");
      const second = makeHookEntry("shared", "Durar-workspace");
      second.hook.handlerPath = "/tmp/Durar-workspace/shared/handler-2.js";
