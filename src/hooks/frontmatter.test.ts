...
`;
    ...
    expect(parsed.Durar.emoji).toBe("💾");
    expect(parsed.Durar.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.Durar.emoji).toBe("📝");
    expect(parsed.Durar.events).toEqual(["command"]);
    expect(parsed.Durar.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.Durar.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"Durar": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"Durar": {"events": ["test"]}}');
  });
...
describe("resolveDurarMetadata", () => {
  it("extracts Durar metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveDurarMetadata(frontmatter);
  ...
  it("returns undefined when Durar key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        Durar: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Durar" },
            { id: "npm", kind: "npm", package: "@Durar/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@Durar/hook");
  });
    ...
    "Durar":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Durar" }],
      },
  }
---
...
`;
    ...
    const Durar = resolveDurarMetadata(frontmatter);
    expect(Durar).toBeDefined();
    expect(Durar?.emoji).toBe("💾");
    expect(Durar?.events).toEqual(["command:new", "command:reset"]);
    expect(Durar?.requires?.config).toEqual(["workspace.dir"]);
    expect(Durar?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const Durar = resolveDurarMetadata(frontmatter);
    expect(Durar?.emoji).toBe("disk");
    expect(Durar?.events).toEqual(["command:new"]);
  });
});
