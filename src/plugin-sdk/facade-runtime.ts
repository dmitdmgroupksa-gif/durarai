...
import { loadConfig, type DurarConfig } from "../config/config.js";
...
const EMPTY_FACADE_BOUNDARY_CONFIG: DurarConfig = {};
const jitiLoaders = new Map<string, ReturnType<typeof createJiti>>();
const loadedFacadeModules = new Map<string, unknown>();
const loadedFacadePluginIds = new Set<string>();
let cachedBoundaryRawConfig: DurarConfig | undefined;
...
function resolveSourceFirstPublicSurfacePath(params: {
  ...
  const sourceRoot = params.bundledPluginsDir ?? path.resolve(Durar_PACKAGE_ROOT, "extensions");
...
function readFacadeBoundaryConfigSafely(): DurarConfig {
  try {
...
export function loadBundledPluginPublicSurfaceModuleSync<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === Durar_PACKAGE_ROOT
        ? "Durar package root"
