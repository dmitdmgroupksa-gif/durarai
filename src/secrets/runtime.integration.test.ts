...
function createOpenAIFileRuntimeConfig(secretFile: string): DurarConfig {
  return asConfig({
    secrets: {
      providers: {
        default: {
...
describe("secrets runtime snapshot integration", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "Durar_DISABLE_PLUGIN_DISCOVERY_CACHE",
      "Durar_VERSION",
    ]);
    delete process.env.Durar_BUNDLED_PLUGINS_DIR;
    process.env.Durar_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
    delete process.env.Durar_VERSION;
  });
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    await withEnvAsync(
      {
        Durar_BUNDLED_PLUGINS_DIR: undefined,
        Durar_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
        Durar_VERSION: undefined,
      },
      async () => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          ...
          agentDirs: ["/tmp/Durar-agent-main"],
          loadAuthStore: () =>
        ...
        expect(
          ensureAuthProfileStore("/tmp/Durar-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("Durar-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("Durar-secrets-runtime-refresh-fail-", async (home) => {
      const { secretFile, agentDir } = await createOpenAIFileRuntimeFixture(home);
  ...
  it(
    ...
    async () => {
      await withTempHome("Durar-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".Durar", "Durar.json"), "utf8"),
        ) as DurarConfig;
  ...
  it(
    ...
    async () => {
      await withTempHome("Durar-secrets-runtime-web-reload-lkg-", async (home) => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          config: asConfig({
            tools: {
              web: {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("Durar-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".Durar", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".Durar", "agents", "ops", "agent");
