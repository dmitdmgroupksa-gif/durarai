...
function buildConfigForDurarTarget(entry: SecretRegistryEntry, envId: string): DurarConfig {
  const config = {} as DurarConfig;
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every Durar.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "Durar.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `Durar_SECRET_TARGET_${index}`;
      const runtimeEnvId = resolveCoverageEnvId(entry, envId);
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForDurarTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `Durar_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
