...
function setConfiguredFetchProviderKey(configTarget: DurarConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: DurarConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: DurarConfig, provider: ProviderUnderTest): unknown {
