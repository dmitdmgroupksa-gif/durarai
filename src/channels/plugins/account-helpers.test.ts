...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): DurarConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as DurarConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: DurarConfig) => string[];
  input: DurarConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: DurarConfig, expected: string) {
