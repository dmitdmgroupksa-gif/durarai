...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.Durar_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Durar";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Durar\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.Durar_MDNS_HOSTNAME?.trim() || "Durar";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
