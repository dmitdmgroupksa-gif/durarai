...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["Durar_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.Durar_STATE_DIR = tempDir;
    ...
    vi.mocked(resolveDurarPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveDurarPackageRoot).mockResolvedValue("/opt/Durar");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/Durar/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
