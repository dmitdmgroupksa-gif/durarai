...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.Durar_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*Durar-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "Durar";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "Durar");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "Durar-gw");
  ...
  records.push(`_Durar-gw._tcp IN PTR ${instanceLabel}._Durar-gw._tcp`);
  records.push(`${instanceLabel}._Durar-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._Durar-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; Durar-content-hash: ${contentHash}\n${contentBody}`;
}
