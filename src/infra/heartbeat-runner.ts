...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: DurarConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: DurarConfig) {
...
function resolveHeartbeatAgents(cfg: DurarConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: DurarConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: DurarConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: DurarConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
