...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Durar tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideDurar = path.join(os.tmpdir(), "outside-Durar", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
