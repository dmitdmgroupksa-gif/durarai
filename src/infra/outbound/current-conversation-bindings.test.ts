...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.Durar_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-current-bindings-"));
    process.env.Durar_STATE_DIR = testStateDir;
