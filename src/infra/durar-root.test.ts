...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__Durar_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "Durar-root");
...
function setPackageRoot(root: string, name = "Durar") {
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "Durar");
        const pkgRoot = path.join(project, "node_modules", "Durar");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "Durar");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "Durar.mjs")));
    ...
    {
      name: "falls through from a non-Durar moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-Durar");
    ...
    {
      name: "returns null for non-Durar package roots",
      setup: () => {
        const pkgRoot = fx("not-Durar");
        setPackageRoot(pkgRoot, "not-Durar");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "Durar.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "Durar");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
