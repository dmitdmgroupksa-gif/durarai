...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/Durar-state", "devices")).toEqual({
      dir: path.join("/tmp/Durar-state", "devices"),
      pendingPath: path.join("/tmp/Durar-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/Durar-state", "devices", "paired.json"),
    });
  });
