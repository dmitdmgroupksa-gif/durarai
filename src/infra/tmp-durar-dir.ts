...
export const POSIX_Durar_TMP_DIR = "/tmp/Durar";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredDurarTmpDirOptions = {
...
export function resolvePreferredDurarTmpDir(
  options: ResolvePreferredDurarTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "Durar" : `Durar-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[Durar] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Durar temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Durar temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_Durar_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_Durar_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_Durar_TMP_DIR)) {
      return POSIX_Durar_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_Durar_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_Durar_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_Durar_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_Durar_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_Durar_TMP_DIR;
