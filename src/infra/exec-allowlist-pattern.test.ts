...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevDurarHome = process.env.Durar_HOME;
    const prevHome = process.env.HOME;
    process.env.Durar_HOME = "/srv/Durar-home";
    process.env.HOME = "/home/other";
    const DurarHome = path.join(path.resolve("/srv/Durar-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", DurarHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevDurarHome === undefined) {
        delete process.env.Durar_HOME;
      } else {
        process.env.Durar_HOME = prevDurarHome;
      }
