...
function candidateBinDirs(opts: EnsureDurarPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `Durar` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "Durar");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.Durar_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "Durar"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `Durar`
...
/**
 * Best-effort PATH bootstrap so skills that require the `Durar` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDurarCliOnPath(opts: EnsureDurarPathOpts = {}) {
  if (isTruthyEnvValue(process.env.Durar_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.Durar_PATH_BOOTSTRAPPED = "1";
