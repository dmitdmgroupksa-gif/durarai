...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".Durar");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.Durar_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$Durar_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via Durar_STATE_DIR.
