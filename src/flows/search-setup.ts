...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<DurarConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<DurarConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: DurarConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: DurarConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: DurarConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: DurarConfig, result: DurarConfig): DurarConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: DurarConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as DurarConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with Durar configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Durar will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.Durar.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Durar will store a reference instead of the API key.",
