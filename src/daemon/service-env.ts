...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.Durar_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    Durar_PROFILE: profile,
    Durar_GATEWAY_PORT: String(port),
    Durar_LAUNCHD_LABEL: resolvedLaunchdLabel,
    Durar_SYSTEMD_UNIT: systemdUnit,
    Durar_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    Durar_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    Durar_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.Durar_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    Durar_GATEWAY_TOKEN: gatewayToken,
    Durar_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    Durar_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    Durar_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    Durar_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    Durar_LOG_PREFIX: "node",
    Durar_SERVICE_MARKER: NODE_SERVICE_MARKER,
    Durar_SERVICE_KIND: NODE_SERVICE_KIND,
    Durar_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    Durar_STATE_DIR: sharedEnv.stateDir,
    Durar_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.Durar_STATE_DIR;
  const configPath = env.Durar_CONFIG_PATH;
