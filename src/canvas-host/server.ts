...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.DurarCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.DurarCanvasA2UIAction &&
        typeof window.DurarCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.DurarSendUserAction === "function";
  ...
  window.addEventListener("Durar:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Durar node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.DurarSendUserAction === "function"
        ? window.DurarSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.Durar_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.Durar_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Durar Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
