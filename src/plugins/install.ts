...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing Durar.extensions; update the plugin package to include Durar.extensions (for example ["./dist/index.js"]). See https://docs.Durar.ai/help/troubleshooting#plugin-install-fails-with-missing-Durar-extensions';
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_Durar_EXTENSIONS: "missing_Durar_extensions",
  EMPTY_Durar_EXTENSIONS: "empty_Durar_extensions",
...
function ensureDurarExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_Durar_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json Durar.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_Durar_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    return {
      ok: false,
      error: `Bundle "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "Durar security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureDurarExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureDurarExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from Durar.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-Durar"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json Durar.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Durar >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set Durar_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Durar >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Durar and retry.`,
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "Durar security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
export async function installPluginFromFile(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin file "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "Durar security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
