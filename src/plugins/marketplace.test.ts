...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-marketplace-test-"));
  try {
    return await fn(dir);
  } finally {
...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("Durar-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir(async (homeDir) => {
      const DurarHome = path.join(homeDir, "Durar-home");
      await fs.mkdir(path.join(homeDir, ".claude", "plugins"), { recursive: true });
      await fs.mkdir(DurarHome, { recursive: true });
      await fs.writeFile(
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, Durar_HOME: DurarHome },
