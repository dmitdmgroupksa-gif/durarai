...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-Durar",
    targetDir: params?.targetDir ?? "/tmp/opik-Durar",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "Durar-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/Durar-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@Durar/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["Durar-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/Durar-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-Durar",
        spec: "@opik/opik-Durar",
        integrity: "sha512-old",
        installPath: "/tmp/opik-Durar",
      }),
      pluginIds: ["opik-Durar"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-Durar",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "Durar-codex-app-server",
        spec: "Durar-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/Durar-codex-app-server",
      }),
      pluginIds: ["Durar-codex-app-server"],
      specOverrides: {
        "Durar-codex-app-server": "Durar-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "Durar-codex-app-server",
        targetDir: "/tmp/Durar-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "Durar-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @Durar/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@Durar/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @Durar/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "Durar-codex-app-server@beta",
        resolvedName: "Durar-codex-app-server",
        resolvedSpec: "Durar-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "Durar-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "Durar-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "Durar-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "Durar-codex-app-server",
      }),
      specOverrides: {
        "Durar-codex-app-server": "Durar-codex-app-server@beta",
      },
      expectedSpec: "Durar-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "Durar-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "Durar-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@Durar/voice-call",
      targetDir: "/tmp/Durar-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@Durar/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@Durar/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@Durar/voice-call");
    expect(result.config.plugins?.entries?.["@Durar/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@Durar/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@Durar/voice-call",
      installPath: "/tmp/Durar-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { Durar_HOME: "/srv/Durar-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/Durar-home";
    mockBundledSources(
      createBundledSource({
