...
const RUNTIME_API_EXPORT_GUARDS: Record<string, readonly string[]> = {
  ...
  [bundledPluginFile("imessage", "runtime-api.ts")]: [
    'export { DEFAULT_ACCOUNT_ID, buildChannelConfigSchema, getChatChannelMeta, type ChannelPlugin, type DurarConfig } from "Durar/plugin-sdk/core";',
    'export { PAIRING_APPROVED_MESSAGE } from "Durar/plugin-sdk/channel-status";',
    'export { buildComputedAccountStatusSnapshot, collectStatusIssuesFromLastError } from "Durar/plugin-sdk/status-helpers";',
    'export { formatTrimmedAllowFromEntries, resolveIMessageConfigAllowFrom, resolveIMessageConfigDefaultTo } from "Durar/plugin-sdk/channel-config-helpers";',
    'export { looksLikeIMessageTargetId, normalizeIMessageMessagingTarget } from "./src/normalize.js";',
    'export { resolveChannelMediaMaxBytes } from "Durar/plugin-sdk/media-runtime";',
    'export { IMessageConfigSchema } from "Durar/plugin-sdk/channel-config-schema";',
    ...
    'export type IMessageAccountConfig = Omit< NonNullable<NonNullable<RuntimeApiDurarConfig["channels"]>["imessage"]>, "accounts" | "defaultAccount" >;',
  ...
  [bundledPluginFile("googlechat", "runtime-api.ts")]: [
    'export * from "Durar/plugin-sdk/googlechat";',
  ],
  [bundledPluginFile("matrix", "runtime-api.ts")]: [
    ...
    'export { assertHttpUrlTargetsPrivateNetwork, closeDispatcher, createPinnedDispatcher, resolvePinnedHostnameWithPolicy, ssrfPolicyFromAllowPrivateNetwork, type LookupFn, type SsrFPolicy } from "Durar/plugin-sdk/ssrf-runtime";',
    ...
    'export { writeJsonFileAtomically } from "Durar/plugin-sdk/json-store";',
    'export type { ChannelDirectoryEntry, ChannelMessageActionContext, DurarConfig, PluginRuntime, RuntimeLogger, RuntimeEnv, WizardPrompter } from "Durar/plugin-sdk/matrix-runtime-shared";',
    'export { formatZonedTimestamp } from "Durar/plugin-sdk/matrix-runtime-shared";',
    'export function chunkTextForOutbound(text: string, limit: number): string[] { const chunks: string[] = []; let remaining = text; while (remaining.length > limit) { const window = remaining.slice(0, limit); const splitAt = Math.max(window.lastIndexOf("\\n"), window.lastIndexOf(" ")); const breakAt = splitAt > 0 ? splitAt : limit; chunks.push(remaining.slice(0, breakAt).trimEnd()); remaining = remaining.slice(breakAt).trimStart(); } if (remaining.length > 0 || text.length === 0) { chunks.push(remaining); } return chunks; }',
  ],
  [bundledPluginFile("nextcloud-talk", "runtime-api.ts")]: [
    'export * from "Durar/plugin-sdk/nextcloud-talk";',
  ],
  ...
  [bundledPluginFile("telegram", "runtime-api.ts")]: [
    'export type { ChannelMessageActionAdapter, ChannelPlugin, DurarConfig, DurarPluginApi, PluginRuntime, TelegramAccountConfig, TelegramActionConfig, TelegramNetworkConfig } from "Durar/plugin-sdk/telegram-core";',
    'export type { TelegramApiOverride } from "./src/send.js";',
    'export type { DurarPluginService, DurarPluginServiceContext, PluginLogger } from "Durar/plugin-sdk/core";',
    'export type { AcpRuntime, AcpRuntimeCapabilities, AcpRuntimeDoctorReport, AcpRuntimeEnsureInput, AcpRuntimeEvent, AcpRuntimeHandle, AcpRuntimeStatus, AcpRuntimeTurnInput, AcpRuntimeErrorCode, AcpSessionUpdateTag } from "Durar/plugin-sdk/acp-runtime";',
    'export { AcpRuntimeError } from "Durar/plugin-sdk/acp-runtime";',
    'export { buildTokenChannelStatusSummary, clearAccountEntryFields, DEFAULT_ACCOUNT_ID, normalizeAccountId, PAIRING_APPROVED_MESSAGE, parseTelegramTopicConversation, projectCredentialSnapshotFields, resolveConfiguredFromCredentialStatuses, resolveTelegramPollVisibility } from "Durar/plugin-sdk/telegram-core";',
    'export { buildChannelConfigSchema, getChatChannelMeta, jsonResult, readNumberParam, readReactionParams, readStringArrayParam, readStringOrNumberParam, readStringParam, resolvePollMaxSelections, TelegramConfigSchema } from "Durar/plugin-sdk/telegram-core";',
