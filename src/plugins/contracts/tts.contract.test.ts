...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): DurarConfig {
  return {
    messages: {
      tts: {
        provider: "openai",
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.Durar_TTS_PREFS;
      process.env.Durar_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.Durar_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: DurarConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
