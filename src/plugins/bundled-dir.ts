...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.Durar_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveDurarPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveDurarPackageRootSync({ argv1: process.argv[1] }),
