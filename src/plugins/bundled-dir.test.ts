...
const originalBundledDir = process.env.Durar_BUNDLED_PLUGINS_DIR;
...
function createDurarRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "Durar" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.Durar_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.Durar_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "Durar-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "Durar-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    const repoRoot = createDurarRoot(layout);
    expectResolvedBundledDirFromRoot({
      repoRoot,
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createDurarRoot({
          prefix: "Durar-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        const cwdRepoRoot = createDurarRoot({
          prefix: "Durar-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "Durar.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createDurarRoot({
          prefix: "Durar-bundled-dir-override-",
          hasDistExtensions: true,
        });
        return {
          installedRoot,
