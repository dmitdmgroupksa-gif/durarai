...
function hasTrustedDurarRootIndicator(params: {
  packageRoot: string;
  ...
  const hasDurarBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("Durar")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.Durar === "string");
  const hasDurarEntrypoint = fs.existsSync(path.join(params.packageRoot, "Durar.mjs"));
  return hasCliEntryExport || hasDurarBin || hasDurarEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedDurarRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedDurarRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveDurarPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedDurarRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveDurarPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedDurarRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
      ? resolveDurarPackageRootSync({
          cwd,
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`Durar/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias ? { "Durar/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "Durar/plugin-sdk": pluginSdkAlias } : {}),
