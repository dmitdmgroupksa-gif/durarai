...
const mocks = vi.hoisted(() => ({
  ...
  loadDurarPluginCliRegistry: vi.fn(),
  loadDurarPlugins: vi.fn(),
  applyPluginAutoEnable: vi.fn(),
}));

vi.mock("./loader.js", () => ({
  loadDurarPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadDurarPluginCliRegistry(...args),
  loadDurarPlugins: (...args: unknown[]) => mocks.loadDurarPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: DurarConfig;
  autoEnabledConfig: DurarConfig;
  ...
  expect(mocks.loadDurarPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  beforeEach(() => {
    ...
    mocks.loadDurarPluginCliRegistry.mockReset();
    mocks.loadDurarPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadDurarPlugins.mockReset();
    mocks.loadDurarPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as DurarConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as DurarConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadDurarPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadDurarPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadDurarPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to awaited CLI metadata collection when runtime loading ignored async registration", async () => {
    ...
    await registerPluginCliCommands(program, {} as DurarConfig, undefined, undefined, {
      mode: "lazy",
    });

    expect(mocks.loadDurarPluginCliRegistry).toHaveBeenCalledTimes(1);
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as DurarConfig, undefined, undefined, {
      mode: "lazy",
    });
