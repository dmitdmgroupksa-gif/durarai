 ...
 */
export type DurarPluginConfigSchema = {
...
export type DurarPluginToolContext = {
  config?: DurarConfig;
  /** Active runtime-resolved config snapshot when one is available. */
  runtimeConfig?: DurarConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
  sessionKey?: string;
...
export type DurarPluginToolFactory = (
  ctx: DurarPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type DurarPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type DurarPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<DurarConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Durar uses this to expose
 ...
 *
 * Runs after Durar resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Durar auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Durar wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Durar builds its generic
...
/**
 * Provider-owned stream wrapper hook after Durar applies its generic
 ...
 *
 * Return `true` or `false` to override Durar's built-in provider cache TTL
 ...
 *
 * Runs only after Durar exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Durar should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Durar loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Durar falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Durar calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Durar performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Durar registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Durar wrappers.
   *
   ...
   *
   * Called after Durar resolves the raw configured credential but before the
   ...
   * hint than Durar's generic auth-store guidance.
   */
   ...
   * Durar's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Durar should surface a provider-specific hint for
   ...
   * merging. Durar deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Durar uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `Durar doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `Durar doctor` should offer.
   *
   ...
   *
   * Durar calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Durar's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
   ...
   * when Durar has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: DurarConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: DurarConfig, value: unknown) => void;
  applySelectionConfig?: (config: DurarConfig) => DurarConfig;
  runSetup?: (ctx: WebSearchProviderSetupContext) => DurarConfig | Promise<DurarConfig>;
...
export type DurarPluginGatewayMethod = {
  method: string;
 ...
 */
export type DurarPluginCommandDefinition = {
...
export type DurarPluginHttpRouteAuth = "gateway" | "plugin";
export type DurarPluginHttpRouteMatch = "exact" | "prefix";

export type DurarPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type DurarPluginHttpRouteParams = {
  path: string;
  handler: DurarPluginHttpRouteHandler;
  auth: DurarPluginHttpRouteAuth;
  match?: DurarPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type DurarPluginCliContext = {
  program: Command;
  config: DurarConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type DurarPluginCliRegistrar = (ctx: DurarPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Durar to keep a plugin command lazy-loaded while still
 ...
 */
export type DurarPluginCliCommandDescriptor = {
  name: string;
  description: string;
...
export type DurarPluginServiceContext = {
  config: DurarConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type DurarPluginService = {
  id: string;
  start: (ctx: DurarPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: DurarPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Durar should inject bundle MCP config for this backend.
   *
...
export type DurarPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type DurarPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: DurarPluginApi) => void | Promise<void>;
  activate?: (api: DurarPluginApi) => void | Promise<void>;
};

export type DurarPluginModule =
  | DurarPluginDefinition
  | ((api: DurarPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

/** Main registration API injected into native plugin entry files. */
export type DurarPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | DurarPluginToolFactory,
    opts?: DurarPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: DurarPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: DurarPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: DurarPluginChannelRegistration | ChannelPlugin) => void;
  ...
  registerCli: (
    registrar: DurarPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Durar can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: DurarPluginCliCommandDescriptor[];
    },
  ) => void;
  registerService: (service: DurarPluginService) => void;
   ...
   */
  registerCommand: (command: DurarPluginCommandDefinition) => void;
...
export type PluginFormat = "Durar" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * Durar does not await this callback before allowing or denying the tool call.
     */
...
export type PluginHookBeforeInstallPlugin = {
  /** Canonical plugin id Durar will install under. */
  pluginId: string;
...
export type PluginHookBeforeInstallContext = {
  ...
  /** Normalized origin of the install target (e.g. "Durar-bundled", "plugin-package"). */
  origin?: string;
};
