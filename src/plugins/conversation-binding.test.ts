...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-plugin-binding-"));
...
vi.mock("../infra/home-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    expandHomePrefix: (value: string) => {
      if (value === "~/.Durar/plugin-binding-approvals.json") {
  ...
  it.each([
    ...
    },
    {
      ...
      existingRecord: {
        ...
        targetSessionKey: "Durar-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
        targetKind: "session" as const,
        conversation: {
          channel: "telegram",
          accountId: "default",
