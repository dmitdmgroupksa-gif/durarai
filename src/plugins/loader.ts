...
import { discoverDurarPlugins } from "./discovery.js";
...
export type PluginLoadOptions = {
  config?: DurarConfig;
  activationSourceConfig?: DurarConfig;
...
export function resolveRuntimePluginRegistry(
  ...
  return getCompatibleActivePluginRegistry(options) ?? loadDurarPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: DurarPluginDefinition;
  register?: DurarPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as DurarPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as DurarPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.Durar_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverDurarPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverDurarPlugins>["candidates"][number];
  right: ReturnType<typeof discoverDurarPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadDurarPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadDurarPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  const discovery = discoverDurarPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Durar yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: DurarPluginModule | null = null;
    try {
      ...
      mod = getJiti(safeSource)(safeSource) as DurarPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
...
export async function loadDurarPluginCliRegistry(
  options: PluginLoadOptions = {},
