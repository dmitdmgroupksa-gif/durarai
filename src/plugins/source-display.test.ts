...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "Durar", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".Durar", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".Durar", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "Durar-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".Durar", "extensions"),
      },
    });
  });
});
