...
const { createDurarCodingToolsMock } = vi.hoisted(() => ({
  createDurarCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(async () => {
    vi.restoreAllMocks();
    vi.resetModules();
    createDurarCodingToolsMock.mockClear();
    createDurarCodingToolsMock.mockReturnValue([]);
    const piTools = await import("../../agents/pi-tools.js");
    vi.spyOn(piTools, "createDurarCodingTools").mockImplementation(
      createDurarCodingToolsMock,
    );
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createDurarCodingToolsMock).toHaveBeenCalledWith(
