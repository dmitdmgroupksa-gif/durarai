...
function createPerplexityConfig(apiKey: string, enabled?: boolean): DurarConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: DurarConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): DurarConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: DurarConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: DurarConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
