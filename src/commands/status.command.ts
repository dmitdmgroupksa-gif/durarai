...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByDurar ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByDurar ? "installed · " : "";
  ...
  runtime.log(theme.heading("Durar status"));
  runtime.log("");
  ...
  if (summary.taskAudit.errors > 0) {
    runtime.log("");
    runtime.log(
      theme.muted(`Task maintenance: ${formatCliCommand("Durar tasks maintenance --apply")}`),
    );
  }
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`Durar devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("Durar devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("Durar devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("Durar security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("Durar security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.Durar.ai/faq");
  runtime.log("Troubleshooting: https://docs.Durar.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("Durar status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("Durar logs --follow")}`);
  if (nodeOnlyGateway) {
    runtime.log(`  Need node service?  ${formatCliCommand("Durar node status")}`);
  } else if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("Durar status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("Durar gateway probe")}`);
  }
}
