...
export function hasExplicitMemorySearchConfig(cfg: DurarConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: DurarConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: DurarConfig, agentId: string) => { store: { path: string } } | null;
