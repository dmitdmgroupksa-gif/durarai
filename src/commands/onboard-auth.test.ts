...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under Durar_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("Durar-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-oauth-sync-"));
    ...
    process.env.Durar_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-oauth-nosync-"));
    process.env.Durar_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside Durar_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-oauth-external-"));
    process.env.Durar_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* Durar_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to Durar_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("Durar-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
