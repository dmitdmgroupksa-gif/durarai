...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Durar", ".Durar");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/Durar-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".Durar");
  ...
  it("anchors cloud detection to OS homedir when Durar_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".Durar");
    const originalDurarHome = process.env.Durar_HOME;
    process.env.Durar_HOME = "/tmp/Durar-home-override";
    ...
    try {
      ...
      if (originalDurarHome === undefined) {
