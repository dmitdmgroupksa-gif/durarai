...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: DurarConfig;
  let lastUpdatedConfig: DurarConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/Durar/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/Durar/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/Durar/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: DurarConfig) => DurarConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/Durar/agents/main",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with Durar configure --section web (recommended mode: cached). Docs: https://docs.Durar.ai/tools/web",
    );
  });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/Durar/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `Durar plugins list --json`.',
    );
  });
