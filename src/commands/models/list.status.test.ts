...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveDurarAgentDir: vi.fn().mockReturnValue("/tmp/Durar-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/Durar-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/Durar-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/Durar-dev/Durar.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveDurarAgentDir: mocks.resolveDurarAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/Durar-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveDurarAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/Durar-dev/Durar.json");
    expect(payload.auth.storePath).toBe("/tmp/Durar-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/Durar-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/Durar-agent-custom");
