...
function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<DurarConfig>;
}): ProviderPlugin {
  return {
    ...
    auth: [
      createProviderApiKeyAuthMethod({
        ...
        ...(params.applyConfig ? { applyConfig: () => params.applyConfig as DurarConfig } : {}),
        wizard: {
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as DurarConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("Durar-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/Durar-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireDurarAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
