...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-Durar-backup";
...
describe("backupVerifyCommand", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".Durar"), { recursive: true });
    delete process.env.Durar_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("Durar-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".Durar");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "Durar.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-Durar-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.Durar",
            archivePath: `${rootName}/payload/posix/tmp/.Durar`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "Durar-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "Durar-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".Durar");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-backup-verify-out-"));
    try {
      process.env.Durar_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
        JSON.stringify({
          agents: {
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.Durar/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "Durar-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "Durar-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
