...
export function createStatusScanSharedMocks(configPathLabel: string) {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/Durar-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = DurarConfig>(
  overrides: T = {} as T,
): DurarConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): DurarConfig {
