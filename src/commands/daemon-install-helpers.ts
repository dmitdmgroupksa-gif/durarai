...
export async function buildGatewayInstallPlan(params: {
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.Durar_PROFILE)
        : undefined,
  ...
  //   1. ~/.Durar/.env file vars  (lowest — user secrets / fallback keys)
  //   2. Config env vars              (Durar.json env.vars + inline keys)
  //   3. Auth-profile env refs        (credential store → env var lookups)
  //   4. Service environment          (HOME, PATH, Durar_* — highest)
  return {
    programArguments,
    workingDirectory,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("Durar gateway install")}\` after fixing the error.`;
}
