...
const SEARCH_TOOL = "https://docs.Durar.ai/mcp.SearchDurar";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.Durar.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('Durar docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.Durar.ai/");
      runtime.log(`Search: ${formatCliCommand('Durar docs "your query"')}`);
    }
    return;
  }
