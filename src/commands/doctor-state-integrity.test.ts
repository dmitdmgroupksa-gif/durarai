...
type EnvSnapshot = {
  HOME?: string;
  Durar_HOME?: string;
  Durar_STATE_DIR?: string;
  Durar_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    Durar_HOME: process.env.Durar_HOME,
    Durar_STATE_DIR: process.env.Durar_STATE_DIR,
    Durar_OAUTH_DIR: process.env.Durar_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: DurarConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: DurarConfig) {
...
async function runStateIntegrityText(cfg: DurarConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.Durar_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when Durar_OAUTH_DIR is explicitly configured", async () => {
    process.env.Durar_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: DurarConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints Durar-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: DurarConfig = {};
    ...
    expect(text).toMatch(/Durar sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/Durar sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /Durar sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
