...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Durar and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["Durar update", "Update a source checkout (git)"],
        ["Durar update --channel beta", "Switch to beta channel (git + npm)"],
        ["Durar update --channel dev", "Switch to dev channel (git + npm)"],
        ["Durar update --tag beta", "One-off update to a dist-tag or version"],
        ["Durar update --tag main", "One-off package install from GitHub main"],
        ["Durar update --dry-run", "Preview actions without changing anything"],
        ["Durar update --no-restart", "Update without restarting the service"],
        ["Durar update --json", "Output result as JSON"],
        ["Durar update --yes", "Non-interactive (accept downgrade prompts)"],
        ["Durar update wizard", "Interactive update wizard"],
        ["Durar --update", "Shorthand for Durar update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run Durar update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.Durar.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Durar update status", "Show channel + version status."],
          ["Durar update status --json", "JSON output."],
          ["Durar update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.Durar.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
