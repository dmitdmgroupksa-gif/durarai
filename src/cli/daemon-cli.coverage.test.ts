...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      Durar_GATEWAY_PORT: String(params.port),
      ...(params.token ? { Durar_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.Durar_STATE_DIR = "/tmp/Durar-cli-state";
    process.env.Durar_CONFIG_PATH = "/tmp/Durar-cli-state/Durar.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        Durar_PROFILE: "dev",
        Durar_STATE_DIR: "/tmp/Durar-daemon-state",
        Durar_CONFIG_PATH: "/tmp/Durar-daemon-state/Durar.json",
        Durar_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.Durar.gateway.plist",
    });
