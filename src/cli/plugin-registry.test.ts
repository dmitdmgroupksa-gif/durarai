...
vi.mock("../plugins/loader.js", () => ({
  loadDurarPlugins: mocks.loadDurarPlugins,
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("reloads when escalating from configured-channels to channels", async () => {
    ...
    expect(mocks.loadDurarPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadDurarPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not treat a pre-seeded partial registry as all scope", async () => {
    ...
    expect(mocks.loadDurarPlugins).toHaveBeenCalledTimes(1);
    expect(mocks.loadDurarPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config,
        throwOnLoadError: true,
