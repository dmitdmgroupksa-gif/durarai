...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("Durar-daemon-install-int-");
    configPath = path.join(tempHome, "Durar.json");
    process.env.HOME = tempHome;
    process.env.Durar_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.Durar_GATEWAY_TOKEN).toBeUndefined();
  });
});
