...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    Durar_STATE_DIR: "/tmp/Durar-daemon",
    Durar_CONFIG_PATH: "/tmp/Durar-daemon/Durar.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.Durar_STATE_DIR ?? "/tmp/Durar-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.Durar_CONFIG_PATH ?? `${stateDir}/Durar.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/Durar-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.Durar_STATE_DIR = "/tmp/Durar-cli";
    process.env.Durar_CONFIG_PATH = "/tmp/Durar-cli/Durar.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/Durar-daemon/Durar.json",
      }),
    );
  });
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.Durar_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.Durar_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.Durar_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "Durar-gateway" }],
        hints: [],
      },
      healthy: false,
