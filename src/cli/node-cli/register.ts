    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "Durar node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["Durar node status", "Check node host service status."],
          ["Durar node install", "Install the node host service."],
          ["Durar node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.Durar.ai/cli/node")}\n`,
    );
