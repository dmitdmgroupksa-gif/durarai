...
const serveDurarChannelMcp = mocks.serveDurarChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveDurarChannelMcp: mocks.serveDurarChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("Durar-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("Durar-cli-mcp-home-", async () => {
      ...
      expect(serveDurarChannelMcp).toHaveBeenCalledWith({
