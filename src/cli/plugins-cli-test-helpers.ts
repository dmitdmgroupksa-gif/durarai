...
export const loadConfig = vi.fn<() => DurarConfig>(() => ({}) as DurarConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: DurarConfig) => Promise<void>>(
  async () => undefined,
);
export const replaceConfigFile = vi.fn(
  async (params: { nextConfig: DurarConfig }) => await writeConfigFile(params.nextConfig),
);
export const resolveStateDir = vi.fn(() => "/tmp/Durar-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: DurarConfig) => writeConfigFile(config),
  replaceConfigFile: (params: { nextConfig: DurarConfig }) => replaceConfigFile(params),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as DurarConfig);
  readConfigFileSnapshot.mockImplementation(async () => {
    const config = loadConfig();
    return {
      path: "/tmp/Durar-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  resolveStateDir.mockReturnValue("/tmp/Durar-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: DurarConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: DurarConfig) => cfg);
  ...
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: DurarConfig }) => ({
    config,
    warnings: [],
  }));
  uninstallPlugin.mockResolvedValue({
    ok: true,
    config: {} as DurarConfig,
    warnings: [],
    actions: {
      entry: false,
      install: false,
  ...
  recordHookInstall.mockImplementation((cfg: DurarConfig) => cfg);
}
