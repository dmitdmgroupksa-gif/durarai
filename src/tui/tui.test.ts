...
describe("resolveInitialTuiAgentId", () => {
  const cfg: DurarConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/Durar" },
        { id: "ops", workspace: "/tmp/Durar/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/Durar/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run Durar devices list");
    expect(state.pairingHint).toContain("Durar devices list");
  });
