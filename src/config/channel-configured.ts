...
function isWhatsAppConfigured(cfg: DurarConfig, env: NodeJS.ProcessEnv): boolean {
...
function isGenericChannelConfigured(cfg: DurarConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return hasMeaningfulChannelConfig(entry);
}
