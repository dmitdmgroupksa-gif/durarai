...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.Durar_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveDurarPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    Durar_STATE_DIR: path.join(os.tmpdir(), "Durar-config-doc-baseline-state"),
    Durar_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
