...
import { parseDurarVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-config-"));
  try {
    await run(home);
  } finally {
...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    expect(logger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining("Config was last written by a newer Durar"),
    );
...
describe("config io paths", () => {
  it("uses ~/.Durar/Durar.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".Durar", 19001);
  ...
  it("defaults to ~/.Durar/Durar.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".Durar", "Durar.json"));
    });
  });

  it("uses Durar_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { Durar_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".Durar", "Durar.json"));
    });
  });

  it("honors explicit Durar_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".Durar", 20002, "custom.json");
      const io = createIoForHome(home, { Durar_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseDurarVersion(VERSION);
    if (!parsedVersion) {
