...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/Durar/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/Durar/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/Durar/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/Durar/nickserv-password.txt");
