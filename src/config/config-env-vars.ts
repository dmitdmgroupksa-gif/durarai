...
function collectConfigEnvVarsByTarget(cfg?: DurarConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: DurarConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: DurarConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: DurarConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.Durar_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
