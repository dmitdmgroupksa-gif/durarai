...
const buildStatusMessageMock = vi.hoisted(() => vi.fn(() => "Durar\n🧠 Model: GPT-5.4"));
...
async function loadFreshDurarToolsForSessionStatusTest() {
  vi.resetModules();
...
describe("session_status tool", () => {
  beforeEach(async () => {
    buildStatusMessageMock.mockClear();
    await loadFreshDurarToolsForSessionStatusTest();
  });
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("Durar");
