...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const DurarTmpDir = path.resolve(resolvePreferredDurarTmpDir());
  if (!isPathInside(DurarTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: DurarTmpDir });
  return resolved;
}
