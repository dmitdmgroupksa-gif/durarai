...
let configOverride: DurarConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: DurarConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/dmitdmgroupksa-gif/durarai/issues/31298
    previousFastTestEnv = process.env.Durar_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.Durar_TEST_FAST;
      return;
    }
    process.env.Durar_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // Durar_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Durar runtime context (internal):");
