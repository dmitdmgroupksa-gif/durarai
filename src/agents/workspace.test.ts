...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses Durar_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      Durar_HOME: "/srv/Durar-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/Durar-home"), ".Durar", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".Durar", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("Durar-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("Durar-workspace-");
    await fs.mkdir(path.join(tempDir, ".Durar"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-workspace-hardlink-"));
    try {
