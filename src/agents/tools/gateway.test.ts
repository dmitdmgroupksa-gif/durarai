...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    Durar: process.env.Durar_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.Durar === undefined) {
      delete process.env.Durar_GATEWAY_TOKEN;
    } else {
      process.env.Durar_GATEWAY_TOKEN = envSnapshot.Durar;
    }
  });
  ...
  it("uses Durar_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.Durar_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
