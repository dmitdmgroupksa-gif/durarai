...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "Durar.sandbox=1");
  args.push("--label", `Durar.sessionKey=${params.scopeKey}`);
  args.push("--label", `Durar.createdAtMs=${createdAtMs}`);
  args.push("--label", `Durar.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `Durar.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markDurarExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "Durar.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`Durar sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`Durar sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("Durar sandbox recreate --all");
}
