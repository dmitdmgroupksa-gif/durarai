...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["Durar_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns Durar_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-bundled-override-"));
    process.env.Durar_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "Durar" }));
