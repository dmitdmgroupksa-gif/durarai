    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "Durar.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "Durar.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "Durar.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("Durar-");
  const acpxRoot = await tempDirs.make("Durar-acpx-plugin-");
  const helperRoot = await tempDirs.make("Durar-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("Durar-");
  const pluginRoot = await tempDirs.make("Durar-plugin-");
  const outsideDir = await tempDirs.make("Durar-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("Durar-");
    const pluginRoot = await tempDirs.make("Durar-claude-bundle-");
