...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("Durar-bundled-");
  const workspaceDir = await createTempDir("Durar-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: DurarConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.Durar_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
