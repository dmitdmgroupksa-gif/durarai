...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): DurarConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureDurarModelsJson(validated.config);
  ...
  it("fills anthropic-vertex apiKey with the ADC sentinel when models exist", async () => {
    await withTempHome(async () => {
      const adcDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-adc-"));
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureDurarModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
