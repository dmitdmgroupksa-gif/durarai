...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a strict empty --mcp-config overlay for bundle-MCP-enabled backends without servers", async () => {
    const workspaceDir = await tempHarness.createTempDir("Durar-cli-bundle-mcp-empty-");
  ...
  it("injects a merged --mcp-config overlay for bundle-MCP-enabled backends", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("Durar-cli-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("Durar-cli-bundle-mcp-workspace-");
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/Durar-bundle-mcp-disabled",
    });
