...
describe("sessions tools", () => {
  ...
  beforeEach(() => {
    ...
    DurarToolsTesting.setDepsForTest({
      callGateway: (opts: unknown) => callGatewayMock(opts),
      config: TEST_CONFIG,
    });
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createDurarTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createDurarTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
