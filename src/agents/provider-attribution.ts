...
const Durar_ATTRIBUTION_PRODUCT = "Durar";
const Durar_ATTRIBUTION_ORIGINATOR = "Durar";
...
function formatDurarUserAgent(version: string): string {
  return `${Durar_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: Durar_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: Durar_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatDurarUserAgent(identity.version),
    },
  };
}
