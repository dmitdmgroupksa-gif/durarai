...
const defaultTools = createDurarCodingTools({ senderIsOwner: true });
...
describe("createDurarCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const disabledOpenAiTools = createDurarCodingTools({
    ...
    const anthropicTools = createDurarCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: DurarConfig = {
      tools: {
        exec: {
    ...
    const allowed = createDurarCodingTools({
    ...
    const denied = createDurarCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createDurarCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
