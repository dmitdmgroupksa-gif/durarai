...
async function withOutsideHardlinkInDurarTmp<T>(
  params: {
    DurarTmpDir: string;
  ...
  const hardlinkPath = path.join(params.DurarTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.DurarTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.DurarTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.DurarTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const DurarTmpDir = resolvePreferredDurarTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Durar tmp root",
      media: path.join(DurarTmpDir, "image.png"),
      expected: path.join(DurarTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Durar tmp root",
      media: pathToFileURL(path.join(DurarTmpDir, "photo.png")).href,
      expected: path.join(DurarTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Durar tmp root",
      media: path.join(DurarTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(DurarTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(DurarTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside Durar tmp root",
      media: path.join(os.tmpdir(), "outside-Durar", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Durar tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(DurarTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(DurarTmpDir, { recursive: true });
      const symlinkPath = path.join(DurarTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Durar tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDurarTmp(
      {
        DurarTmpDir,
  ...
  it("rejects symlinked Durar tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDurarTmp(
