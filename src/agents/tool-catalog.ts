...
function buildCoreToolGroupMap() {
  ...
  const DurarTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInDurarGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:Durar": DurarTools,
