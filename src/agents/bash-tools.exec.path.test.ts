...
describe("exec PATH login shell merge", () => {
  ...
  it("sets Durar_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-Durar-shell", {
      command: 'printf "%s" "${Durar_SHELL:-}"',
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/Durar-ssl-keys.log";
    try {
      ...
      expect(output).not.toContain("/tmp/Durar-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a Durar /approve abc123 deny",
