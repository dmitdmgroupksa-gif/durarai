...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["Durar_CONTAINER_HINT", "Durar_PROFILE"]);
    delete process.env.Durar_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:Durar|Durar) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
