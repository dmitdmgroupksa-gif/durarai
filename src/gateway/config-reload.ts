...
export function resolveGatewayReloadSettings(cfg: DurarConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: DurarConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: DurarConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: DurarConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: DurarConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: DurarConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: DurarConfig) => {
