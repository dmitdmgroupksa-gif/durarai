...
import type { loadDurarPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureDurarModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadDurarPlugins>;
  ...
  // Tests can opt out via Durar_SKIP_CHANNELS (or legacy Durar_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.Durar_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.Durar_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (Durar_SKIP_CHANNELS=1 or Durar_SKIP_PROVIDERS=1)",
    );
  }
