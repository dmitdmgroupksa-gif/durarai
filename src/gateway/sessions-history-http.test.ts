...
const READ_SCOPE_HEADER = { "x-Durar-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-session-history-"));
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __Durar?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __Durar?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__Durar?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __Durar?: { seq?: number } }>;
        messages?: Array<{ __Durar?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__Durar?.seq)).toEqual([1]);
  ...
  test("streams session history updates over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(
        (
          messageEvent.data as {
            message?: { __Durar?: { id?: string; seq?: number } };
          }
        ).message?.__Durar,
      ).toMatchObject({
