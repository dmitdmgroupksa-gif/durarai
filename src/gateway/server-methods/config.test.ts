...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/Durar.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/Durar.json"],
    });
  });
  ...
  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/Durar.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/Durar.json"],
    });
  });
...
describe("config.openFile", () => {
  ...
  it("opens the configured file without shell interpolation", async () => {
    process.env.Durar_CONFIG_PATH = "/tmp/config $(touch pwned).json";
  ...
  it("returns a generic error and logs details when the opener fails", async () => {
    process.env.Durar_CONFIG_PATH = "/tmp/config.json";
    vi.mocked(execFile).mockImplementation(((...args: unknown[]) => {
