...
let lastCreateDurarToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/durar-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateDurarToolsContext?.agentTo,
          agentThreadId: lastCreateDurarToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createDurarTools: (ctx: Record<string, unknown>) => {
      lastCreateDurarToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateDurarToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-Durar-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-Durar-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateDurarToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateDurarToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-Durar-message-to": "channel:24514",
        "x-Durar-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as owner on /tools/invoke", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-Durar-scopes": "operator.approvals",
      },
      tool: "owner_only_test",
      sessionKey: "main",
    });
