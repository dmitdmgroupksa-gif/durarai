...
type GatewayInstallAuthMode = NonNullable<NonNullable<DurarConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: DurarConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.Durar_GATEWAY_PASSWORD?.trim() ||
