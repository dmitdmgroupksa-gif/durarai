...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("Durar doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with Durar_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with Durar_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.Durar_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://Durar-gateway.ai:18789",
      onConnectError,
    });
