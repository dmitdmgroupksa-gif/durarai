...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: DurarConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.Durar_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.Durar_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set Durar_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: Durar doctor --fix",
