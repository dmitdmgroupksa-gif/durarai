...
import { createDurarTools } from "../agents/Durar-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-Durar-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-Durar-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-Durar-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-Durar-thread-id")?.trim() || undefined;
  ...
  const allTools = createDurarTools({
