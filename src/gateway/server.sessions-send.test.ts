...
type SessionSendTool = ReturnType<typeof createDurarTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createDurarTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["Durar_GATEWAY_PORT", "Durar_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.Durar_GATEWAY_PORT = String(gatewayPort);
  process.env.Durar_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("Durar_CONFIG_PATH missing in gateway test environment");
      }
