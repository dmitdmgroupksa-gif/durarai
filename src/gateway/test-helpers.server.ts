...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "Durar_STATE_DIR",
  "Durar_CONFIG_PATH",
  "Durar_SKIP_BROWSER_CONTROL_SERVER",
  "Durar_SKIP_GMAIL_WATCHER",
  "Durar_SKIP_CANVAS_HOST",
  "Durar_BUNDLED_PLUGINS_DIR",
  "Durar_SKIP_CHANNELS",
  "Durar_SKIP_PROVIDERS",
  "Durar_SKIP_CRON",
  "Durar_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.Durar_CONFIG_PATH) {
    configPaths.add(process.env.Durar_CONFIG_PATH);
  }
  if (process.env.Durar_STATE_DIR) {
    configPaths.add(path.join(process.env.Durar_STATE_DIR, "Durar.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.Durar_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.Durar_SKIP_GMAIL_WATCHER = "1";
  process.env.Durar_SKIP_CANVAS_HOST = "1";
  process.env.Durar_SKIP_CHANNELS = "1";
  process.env.Durar_SKIP_PROVIDERS = "1";
  process.env.Durar_SKIP_CRON = "1";
  process.env.Durar_TEST_MINIMAL_GATEWAY = "1";
  process.env.Durar_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "Durar-test-no-bundled-extensions")
    : "Durar-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.Durar_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".Durar-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".Durar-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".Durar-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>Durar-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__DurarTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__DurarTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["Durar_GATEWAY_TOKEN"]);
  const prev = process.env.Durar_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.Durar_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.Durar_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
