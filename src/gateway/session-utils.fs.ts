...
export function attachDurarTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__Durar && typeof record.__Durar === "object" && !Array.isArray(record.__Durar)
      ? (record.__Durar as Record<string, unknown>)
      : {};
  return {
    ...record,
    __Durar: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachDurarTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "Durar" && model === "delivery-mirror";
