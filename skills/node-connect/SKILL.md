---
name: node-connect
description: Diagnose Durar node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Durar is advertising that route, then fix pairing/auth.
...
- whether `Durar devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `Durar qr --json`. It uses the same setup-code payload Android scans.

```bash
Durar config get gateway.mode
Durar config get gateway.bind
Durar config get gateway.tailscale.mode
Durar config get gateway.remote.url
Durar config get gateway.auth.mode
Durar config get gateway.auth.allowTailscale
Durar config get plugins.entries.device-pair.config.publicUrl
Durar qr --json
Durar devices list
Durar nodes status
```

If this Durar instance is pointed at a remote gateway, also run:

```bash
Durar qr --remote --json
```
...
If `Durar qr --json` says `Gateway is only bound to loopback`:
...
- `Durar devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run Durar qr again, rescan, then approve the pending device pairing.`
