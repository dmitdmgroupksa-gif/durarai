---
summary: "Get Durar installed and run your first chat in minutes."
read_when:
...
Install Durar, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.Durar/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.Durar/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Durar as a service account or want custom paths:

- `Durar_HOME` — home directory for internal path resolution
- `Durar_STATE_DIR` — override the state directory
- `Durar_CONFIG_PATH` — override the config file path
