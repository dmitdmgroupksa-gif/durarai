...
- Pool note:
  ...
  - Opt back into Vitest file isolation with `Durar_TEST_ISOLATE=1 pnpm test`.
  - `Durar_TEST_NO_ISOLATE=0` or `Durar_TEST_NO_ISOLATE=false` also force isolated runs.
- Fast-local iteration note:
  ...
  - The wrapper keeps `Durar_VITEST_FS_MODULE_CACHE` enabled on supported hosts, but assigns a lane-local `Durar_VITEST_FS_MODULE_CACHE_PATH` so concurrent Vitest processes do not race on one shared experimental cache directory.
  - Set `Durar_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want one explicit cache location for direct single-run profiling.
- Perf-debug note:
...
- How to select models:
  ...
  - or `Durar_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
...
- Token sources (pick one):
  - Profile: `Durar_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - Raw token: `Durar_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- Model override (optional):
  - `Durar_LIVE_SETUP_TOKEN_MODEL=anthropic/claude-opus-4-6`
...
- `Durar_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` to keep Claude Code CLI MCP config enabled (default disables MCP config with a temporary empty file).
...
- Overrides:
  ...
  - `Durar_LIVE_ACP_BIND_ACPX_COMMAND=/full/path/to/acpx`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `Durar_LIVE_ACP_BIND_ACPX_COMMAND` is unset, the test uses the configured/bundled acpx command. If your harness auth depends on env vars from `~/.profile`, prefer a custom `acpx` command that preserves provider env.
...
- Inside Docker, the runner sets `Durar_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx` so acpx keeps provider env vars from the sourced profile available to the child harness CLI.
...
- Tool calling across several providers:
  - `Durar_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Profile store: `~/.Durar/credentials/` (preferred; what “profile keys” means in the tests)
