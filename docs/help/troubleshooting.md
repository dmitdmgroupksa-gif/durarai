---
summary: "Symptom first troubleshooting hub for Durar"
read_when:
  - Durar is not working and you need the fastest path to a fix
...
- `Durar status` → shows configured channels and no obvious auth errors.
- `Durar status --all` → full report is present and shareable.
- `Durar gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `Durar gateway status` → `Runtime: running` and `RPC probe: ok`.
- `Durar doctor` → no blocking config/service errors.
- `Durar channels status --probe` → reachable gateway returns live per-account
...
- `Durar logs --follow` → steady activity, no repeating fatal errors.
...
## Plugin install fails with missing Durar extensions

If install fails with `package.json missing Durar.extensions`, the plugin package
is using an old shape that Durar no longer accepts.

Fix in the plugin package:

1. Add `Durar.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `Durar plugins install <package>` again.

Example:

```json
{
  "name": "@Durar/my-plugin",
  "version": "1.2.3",
  "Durar": {
...
```mermaid
flowchart TD
  A[Durar is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `Durar gateway status`
    - `RPC probe: ok`
...
- `requests-in-flight` → main lane busy; heartbeat wake was deferred. - `unknown accountId` → heartbeat delivery target account does not exist.
    ...
    <Accordion title="Exec suddenly asks for approval">
      ```bash
      Durar config get tools.exec.host
      Durar config get tools.exec.security
      Durar config get tools.exec.ask
    ...
    <Accordion title="Browser tool fails">
      ...
      - `Durar` starts, or `user` can see local Chrome tabs.
      ...
      - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `Durar browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
