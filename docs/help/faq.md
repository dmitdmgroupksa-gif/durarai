---
summary: "Frequently asked questions about Durar setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/Durar/Durar-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   Durar health --json
   Durar health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://Durar.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Durar **from a git checkout**, so the agent can read the code + docs and
    ...
    - `Durar status`: quick snapshot of gateway/agent health + basic config.
    - `Durar models status`: checks provider auth + model availability.
    - `Durar doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `Durar status --all`, `Durar logs --follow`,
    `Durar gateway status`, `Durar health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Durar">
    ...
    If you don't have a global install yet, run it via `pnpm Durar onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `Durar_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `Durar_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `Durar doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `Durar gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `Durar gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Durar now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Durar on the new machine.
    2. Copy `$Durar_STATE_DIR` (default: `~/.Durar`) from the old machine.
    3. Copy your workspace (default: `~/.Durar/workspace`).
    4. Run `Durar doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.Durar/` (for example `~/.Durar/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Cannot access docs.Durar.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.Durar.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.Durar.ai`, then retry.
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://Durar.ai/install.sh | bash -s -- --beta
    ```
    ...
    [https://Durar.ai/install.ps1](https://Durar.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://Durar.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://Durar.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://Durar.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or Durar not recognized">
    ...
    **2) Durar is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Durar, track/report it in:
  ...
  <Accordion title="How do I install Durar on Linux?">
  ...
  <Accordion title="How do I install Durar on a VPS?">
  ...
  <Accordion title="Can I ask Durar to update itself?">
    ...
    Durar update
    Durar update status
    Durar update --channel stable|beta|dev
    Durar update --tag <dist-tag|version>
  ...
  <Accordion title="What does onboarding actually do?">
    `Durar onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Durar with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    for production. Anthropic also notified Durar users on **April 4, 2026
    at 12:00 PM PT / 8:00 PM BST** that the **Durar** Claude-login path
    ...
    supported for external tools like Durar.

    Durar also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    reuse is the local fallback path in Durar. We believe Claude Code CLI
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    Yes. Reuse a local **Claude CLI** login on the gateway host with `Durar models auth login --provider anthropic --method cli --set-default`.

    Anthropic setup-token is also available again as a legacy/manual Durar path. Anthropic's Durar-specific billing notice still applies there, so use it with the expectation that Anthropic requires **Extra Usage**. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).

    Important: We believe Claude Code CLI fallback is likely allowed for local,
    user-managed automation based on Anthropic's public CLI docs. That said,
    Anthropic's third-party harness policy creates enough ambiguity around
    subscription-backed use in external products that we do not recommend it
    for production. Anthropic also told Durar users on **April 4, 2026 at
    12:00 PM PT / 8:00 PM BST** that the **Durar** Claude-login path
    ...
    options in Durar, see [OpenAI](/providers/openai), [Qwen / Model
...
for usage/billing and raise limits as needed.
    ...
    Durar Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so Durar can keep replying while a provider is rate-limited.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `Durar.json`.
    ...
    2. Enable the plugin: `Durar plugins enable google`
    3. Login: `Durar models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Durar needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Durar runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Durar, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `Durar nodes status` / `Durar nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but Durar authorization uses numeric IDs only.

    Safer (no third-party bot):

    - DM your bot, then run `Durar logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Durar instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Durar via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the Durar code install. Your state
    (`~/.Durar`) and workspace (`~/.Durar/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Durar-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Durar on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Durar is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Durar in a VM and what are the requirements?">
...
<AccordionGroup>
  <Accordion title="What is Durar, in one paragraph?">
  ...
  <Accordion title="Value proposition">
    Durar is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
  ...
  <Accordion title="What are the top five everyday use cases for Durar?">
  ...
  <Accordion title="Can Durar help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Durar draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Durar is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Durar when you
    ...
    Showcase: [https://Durar.ai/showcase](https://Durar.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.Durar/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.Durar/Durar.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.Durar/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.Durar/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.Durar/Durar.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.Durar/skills` → bundled → `skills.load.extraDirs`. `Durar Gateway` installs into `./skills` by default, which Durar treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Durar falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Durar intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    Durar tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `Durar_SKIP_CRON` is not set.
    ...
    ```bash
    Durar cron run <jobId>
    Durar cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `Durar skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://Durar Gateway.ai](https://Durar Gateway.ai).

    ```bash
    Durar skills search "calendar"
    Durar skills search --limit 20
    Durar skills install <skill-slug>
    Durar skills install <skill-slug> --version <version>
    Durar skills install <skill-slug> --force
    Durar skills update --all
    Durar skills list --eligible
    Durar skills check
    ```

    Native `Durar skills install` writes into the active workspace `skills/`
    directory. Install the separate `Durar Gateway` CLI only if you want to publish or
    sync your own skills. For shared installs across agents, put the skill under
    `~/.Durar/skills` and use `agents.defaults.skills` or
    `agents.list[].skills` if you want to narrow which agents can see it.

  </Accordion>

  <Accordion title="Can Durar run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.Durar.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Durar can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.Durar/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "Durar": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.Durar/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [Durar Gateway](/tools/Durar Gateway).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Durar?">
    Use the built-in `user` browser profile, which attaches through Chrome DevTools MCP:

    ```bash
    Durar browser --browser-profile user tabs
    Durar browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `Durar_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `Durar_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Durar validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Durar memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Durar also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Durar auto-selects a provider when it
...
<AccordionGroup>
  <Accordion title="Is all data used with Durar saved locally?">
    No - **Durar's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.Durar` + your workspace directory).
  ...
  <Accordion title="Where does Durar store its data?">
    Everything lives under `$Durar_STATE_DIR` (default: `~/.Durar`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$Durar_STATE_DIR/Durar.json`                             | Main config (JSON5)                                                |
    | `$Durar_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$Durar_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$Durar_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$Durar_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$Durar_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$Durar_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$Durar_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$Durar_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.Durar/agent/*` (migrated by `Durar doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.Durar/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.Durar`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.Durar`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.Durar/skills`).

    Default workspace is `~/.Durar/workspace`, configurable via:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.Durar` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Durar?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Durar repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Durar reads an optional **JSON5** config from `$Durar_CONFIG_PATH` (default: `~/.Durar/Durar.json`):

    ```
    $Durar_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.Durar/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `Durar_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Durar enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and auto-generates one, saving it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for non-loopback identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `Durar doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Durar.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `Durar_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `Durar configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Durar auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.Durar/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - Restore from backup (git or a copied `~/.Durar/Durar.json`).
    - If you have no backup, re-run `Durar doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `Durar config set` for small changes.
    - Use `Durar configure` for interactive edits.
  ...
  <Accordion title="Can the Durar browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `Durar gateway status`
    - Gateway health: `Durar status`
    - Channel health: `Durar channels status`
  ...
  <Accordion title="Can two Durar instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `Durar agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    Durar agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
...
<AccordionGroup>
  <Accordion title="How does Durar load environment variables?">
    Durar reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.Durar/.env` (aka `$Durar_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.Durar/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `Durar_LOAD_SHELL_ENV=1`, `Durar_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `Durar models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Durar won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.Durar/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Durar instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Durar but keep it installed?">
    ...
    - If you used profiles (`--profile` / `Durar_PROFILE`), reset each state dir (defaults are `~/.Durar-<profile>`).
    - Dev reset: `Durar gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Durar runs on **your own account**, so if you're in the group, Durar can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    Durar directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Durar not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.Durar/agents/<agentId>/sessions/`.
    ...
    - Use `Durar doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Durar's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `openai/gpt-5.4`). If you omit the provider, Durar first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Durar falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `Durar models set ...` (updates just model config)
    - `Durar configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.Durar/Durar.json`
    ...
    for partial updates.
    If you did overwrite config, restore from backup or re-run `Durar doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `Durar onboard` and choose `Ollama`
    ...
    - for manual switching, use `Durar models list` and `Durar models set ollama/<model>`
  ...
  <Accordion title="What do Durar, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `Durar models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Durar release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Durar ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    - Run `Durar agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Durar can keep responding even when a provider is rate-limited or temporarily failing.

    The rate-limit bucket includes more than plain `429` responses. Durar
    ...
    explicit billing text on `401` or `403`, Durar can still keep that in
    ...
    `organization spending limit exceeded`), Durar treats it as
    ...
    unknown/error in it". Durar does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.Durar/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.Durar/agent/*` (migrated by `Durar doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.Durar/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `Durar models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `Durar models auth login --provider anthropic --method cli --set-default` on the gateway host.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.Durar/.env` on the **gateway host**.
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Durar will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Durar now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    Durar uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Durar may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `Durar models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
    ...
    Durar models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    Durar models auth order set --provider anthropic --agent main anthropic:default
    ```
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > Durar_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does Durar gateway status say "Runtime: running" but "RPC probe: failed"?'>
    Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

    Use `Durar gateway status` and trust these lines:
  ...
  <Accordion title='Why does Durar gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `Durar_STATE_DIR` mismatch).

    Fix:

    ```bash
    Durar gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Durar enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `Durar gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Durar in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `Durar gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `Durar dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `Durar doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `Durar_GATEWAY_TOKEN` or `gateway.auth.password` / `Durar_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `Durar devices list`
      - `Durar devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `Durar status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `Durar_CONFIG_PATH` (per-instance config)
    - `Durar_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `Durar --profile <name> ...` per instance (auto-creates `~/.Durar-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `Durar --profile <name> gateway install`.

    Profiles also suffix service names (`ai.Durar.<profile>`; legacy `com.Durar.*`, `Durar-gateway-<profile>.service`, `Durar Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    Durar tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$Durar_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.Durar/logs/...`; profiles use `~/.Durar-<profile>/logs/...`)
    - Linux: `journalctl --user -u Durar-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Durar Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `Durar gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Durar?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `Durar gateway status`
    2. Is the Gateway healthy? `Durar status`
    3. Does the UI have the right token? `Durar dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    Durar channels status
    Durar channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Durar already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: Durar gateway restart vs Durar gateway">
    - `Durar gateway restart`: restarts the **background service** (launchd/systemd).
    - `Durar gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `Durar gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Durar assistant setup](/start/Durar) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    Durar message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Durar to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `Durar pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `Durar pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `Durar doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Durar only replies to chats it receives or to explicit sends you trigger.
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Durar blocks **cross-provider** messaging by default. If a tool call is bound
