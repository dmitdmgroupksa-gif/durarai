...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    [https://github.com/dmitdmgroupksa-gif/durarai/issues](https://github.com/dmitdmgroupksa-gif/durarai/issues)
    [https://github.com/dmitdmgroupksa-gif/durarai/pulls](https://github.com/dmitdmgroupksa-gif/durarai/pulls)
  ...
  <Accordion title="Recommended way to install and set up Durar">
    ...
    ```bash
    git clone https://github.com/dmitdmgroupksa-gif/durarai.git
    cd Durar
    pnpm install
    pnpm build
  ...
  <Accordion title="How do I authenticate the dashboard (token) on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - If it asks for auth, paste the token from `gateway.auth.token` (or `Durar_GATEWAY_TOKEN`) into Control UI settings.
    - Retrieve it from the gateway host: `Durar config get gateway.auth.token` (or generate one: `Durar doctor --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `Durar gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
    - **Tailnet bind**: run `Durar gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/dmitdmgroupksa-gif/durarai/blob/main/CHANGELOG.md](https://github.com/dmitdmgroupksa-gif/durarai/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.Durar.ai (SSL error)">
    ...
    [https://github.com/dmitdmgroupksa-gif/durarai/tree/main/docs](https://github.com/dmitdmgroupksa-gif/durarai/tree/main/docs)
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    - [Issue #30640](https://github.com/dmitdmgroupksa-gif/durarai/issues/30640)
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    ...
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    OpenAI Codex OAuth is explicitly supported for external tools like Durar.
  ...
  <Accordion title="How does Anthropic setup-token auth work?">
    `claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `Durar models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
  </Accordion>
  ...
  <Accordion title="Where do I find an Anthropic setup-token?">
    ...
    Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `Durar models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `Durar models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    - use a **setup-token**
    - reuse a local **Claude CLI** login on the gateway host with `Durar models auth login --provider anthropic --method cli --set-default`
...
for usage/billing and raise limits as needed.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    1. Enable the plugin: `Durar plugins enable google`
    2. Login: `Durar models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Durar-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.Durar/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.Durar/Durar.json`). Precedence is `<workspace>/skills` > `~/.Durar/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.Durar/Durar.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.Durar/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Durar treats as `<workspace>/skills` on the next session.
  </Accordion>
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.Durar/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
<AccordionGroup>
  ...
  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    ...
    - **State dir (`~/.Durar`)**: config, credentials, auth profiles, sessions, logs,
...
<AccordionGroup>
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `Durar_GATEWAY_TOKEN`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Durar enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `Durar doctor --generate-gateway-token`.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    ...
    Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Durar currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
<AccordionGroup>
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use a setup-token**
      - Run `claude setup-token`, then paste it with `Durar models auth setup-token --provider anthropic`.
      - If the token was created on another machine, use `Durar models auth paste-token --provider anthropic`.
...
<AccordionGroup>
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Set `gateway.auth.token` (or `Durar_GATEWAY_TOKEN`) on the gateway host.
...
Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/dmitdmgroupksa-gif/durarai/discussions).
