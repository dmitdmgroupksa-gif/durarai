...
Durar can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Durar tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
Durar agent --message "hi" --model claude-cli/claude-sonnet-4-6
```

Codex CLI also works out of the box (via the bundled OpenAI plugin):

```bash
Durar agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, Durar now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Durar will
...
2. **Builds a system prompt** using the same Durar prompt + workspace context.
...
- Durar drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
Durar will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Durar appends the
...
- For Gemini CLI JSON output, Durar reads reply text from `response` and
...
- `stats.cached` is normalized into Durar `cacheRead`.
- If `stats.input` is missing, Durar derives input tokens from
...
CLI backends do **not** receive Durar tool calls directly, but a backend can
...
When bundle MCP is enabled, Durar:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`Durar_MCP_TOKEN`)
...
user-level or global MCP servers. If no MCP servers are enabled, Durar still
injects a strict empty config so background runs stay isolated.

## Limitations

- **No direct Durar tool calls.** Durar does not inject tool calls into
  ...
  so Claude CLI can invoke Durar tools via its native MCP support.
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. Durar sessions still work
  normally.
