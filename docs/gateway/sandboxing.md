---
summary: "How Durar sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Durar can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`Durar-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Durar emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Use `backend: "ssh"` when you want Durar to sandbox `exec`, file tools, and media reads on
...
- Durar creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, Durar seeds that remote workspace from the local workspace once.
...
- Durar does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. Durar resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside Durar after the seed step are not visible remotely until you recreate the sandbox.
- `Durar sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
- Browser sandboxing is not supported on the SSH backend.
- `sandbox.docker.*` settings do not apply to the SSH backend.

### OpenShell backend

Use `backend: "openshell"` when you want Durar to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. Durar syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. Durar seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- Durar asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- you edit files locally outside Durar and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, Durar seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- Durar does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside Durar after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `Durar sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `Durar sandbox recreate` deletes the current runtime and lets Durar recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.Durar/sandboxes`.
...
Durar mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Durar blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- Durar also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. Durar normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
Default Docker image: `Durar-sandbox:bookworm-slim`
...
`Durar-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `Durar_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `Durar_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `Durar_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `Durar_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `Durar_DOCKER_SOCKET`. Full setup and env
...
- Use `Durar sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
