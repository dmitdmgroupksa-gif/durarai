---
summary: "Schema-accurate configuration examples for common Durar setups"
read_when:
  - Learning how to configure Durar
...
```json5
{
  agent: { workspace: "~/.Durar/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.Durar/Durar.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.Durar/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/Durar" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.Durar/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
