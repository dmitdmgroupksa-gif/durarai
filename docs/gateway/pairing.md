...
Durar nodes status
Durar nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.Durar`):

- `~/.Durar/nodes/paired.json`
- `~/.Durar/nodes/pending.json`

If you override `Durar_STATE_DIR`, the `nodes/` folder moves with it.
