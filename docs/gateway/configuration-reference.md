...
summary: "Complete reference for every Durar config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.Durar/Durar.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Durar uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `Durar doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `Durar doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["Durar-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-Durar",
          requireMention: false,
...
- Durar additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `Durar plugins install @Durar/mattermost`.
...
- `commands.callbackUrl` must resolve to the Durar gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, Durar rejects callbacks with
...
Durar spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `Durar channels add` (or channel onboarding) while still on a single-account top-level channel config, Durar promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `Durar doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@Durar", "Durar"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@Durar"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `Durar.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Durar auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/Durar" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.4`). If you omit the provider, Durar tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, Durar falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Durar aborts it. Default: `900`.
    ...
    defaults: {
      sandbox: {
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/Durar-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "Durar-sandbox-browser:bookworm-slim",
          network: "Durar-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Durar materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.Durar/sandboxes`
...
In `remote` mode, host-local edits made outside Durar are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `Durar.json`.
noVNC observer access uses VNC auth by default and Durar emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `Durar-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from Durar_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `Durar_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `Durar_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `Durar_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.Durar/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@Durar"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
For `type: "acp"` entries, Durar resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Durar starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Durar treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:Durar`   | All built-in tools (excludes provider plugins)                                                                          |
...
- Files are materialized into the child workspace at `.Durar/attachments/<uuid>/` with a `.manifest.json`.
...
Durar uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.Durar/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `Durar_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Durar forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `Durar onboard --auth-choice opencode-zen` or `Durar onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `Durar onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `Durar onboard --auth-choice moonshot-api-key-cn`.

Native Moonshot endpoints advertise streaming usage compatibility on the shared
`openai-completions` transport, and Durar now keys that off endpoint
...
Anthropic-compatible, built-in provider. Shortcut: `Durar onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `Durar onboard --auth-choice synthetic-api-key`.
...
`Durar onboard --auth-choice minimax-global-api` or
`Durar onboard --auth-choice minimax-cn-api`.
The model catalog now defaults to M2.7 only.
On the Anthropic-compatible streaming path, Durar disables MiniMax thinking
...
- `install.nodeManager`: node installer preference for `metadata.Durar.install`
...
- Loaded from `~/.Durar/extensions`, `<workspace>/.Durar/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Durar plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Durar plugin schema when available).
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Durar applies those as sanitized agent settings, not as raw Durar config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `Durar plugins update`.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Durar to discover `/json/version`; use WS(S)
...
- Local managed `Durar` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or Durar_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `Durar_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `Durar_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `Durar_APNS_RELAY_BASE_URL` / `Durar_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `Durar_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
Durar_CONFIG_PATH=~/.Durar/a.json \
Durar_STATE_DIR=~/.Durar-a \
Durar gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.Durar-dev` + port `19001`), `--profile <name>` (uses `~/.Durar-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/Durar/tls/server.crt",
      keyPath: "/etc/Durar/tls/server.key",
      caPath: "/etc/Durar/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.Durar/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-Durar-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `Durar_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.Durar/workspace/canvas",
    liveReload: true,
    // enabled: false, // or Durar_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__Durar__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__Durar__/a2ui/`
...
- Also serves A2UI at `/__Durar__/a2ui/`.
...
- Hostname defaults to `Durar`. Override with `Durar_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.Durar/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `Durar dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.Durar/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${Durar_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `Durar.json` credential paths.
...
- Legacy OAuth imports from `~/.Durar/credentials/oauth.json`.
...
- Default log file: `/tmp/Durar/Durar-YYYY-MM-DD.log`.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.Durar/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$Durar_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Durar.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `Durar_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `Durar doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `Durar.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
