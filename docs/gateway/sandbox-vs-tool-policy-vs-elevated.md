...
Durar has three related (but different) controls:
...
Use the inspector to see what Durar is _actually_ doing:
...
- Durar validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:Durar`: all built-in Durar tools (excludes provider plugins)
