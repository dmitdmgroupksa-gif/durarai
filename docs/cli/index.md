...
- `Durar security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `Durar secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `Durar secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `Durar secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `Durar secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).
...
- `Durar plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
...
- `Durar memory status` — show index stats.
...
- `config schema`: print the generated JSON schema for `Durar.json`.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `Durar health` or `Durar status --deep` for gateway health probes).
...
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Durar moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
Tip: use `Durar skills search`, `Durar skills install`, and `Durar skills update` for ClawHub-backed skills.
