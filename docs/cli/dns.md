---
summary: "CLI reference for `Durar dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: Durar.internal)
title: "dns"
---
...
```bash
Durar dns setup
Durar dns setup --domain Durar.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `Durar.internal`)
...
- zone file path
- current tailnet IPs
- recommended `Durar.json` discovery config
...
- If `--domain` is omitted, Durar uses `discovery.wideArea.domain` from config.
