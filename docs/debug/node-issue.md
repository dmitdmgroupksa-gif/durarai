...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Durar
...
Running Durar via Node with `tsx` fails at startup with:

```
[Durar] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `Durar status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch Durar.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node Durar.mjs status` works on Node 25.
