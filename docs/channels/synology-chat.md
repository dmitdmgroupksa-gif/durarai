---
summary: "Synology Chat webhook setup and Durar config"
read_when:
  - Setting up Synology Chat with Durar
...
Synology Chat ships as a bundled plugin in current Durar releases, so normal
...
```bash
Durar plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `Durar onboard` now shows Synology Chat in the same channel setup list as `Durar channels add`.
   - Non-interactive setup: `Durar channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Durar gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Durar.
   - Guided: `Durar onboard`
   - Direct: `Durar channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Durar accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `Durar pairing list synology-chat`
  - `Durar pairing approve synology-chat <CODE>`
...
```bash
Durar message send --channel synology-chat --target 123456 --text "Hello from Durar"
Durar message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Durar now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Durar
- `Rate limit exceeded`:
