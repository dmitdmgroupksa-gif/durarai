---
summary: "Mattermost bot setup and Durar config"
read_when:
...
Mattermost ships as a bundled plugin in current Durar releases, so normal
...
```bash
Durar plugins install @Durar/mattermost
```

Local checkout (when running from a git repo):

```bash
Durar plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Durar and start the gateway.
...
Native slash commands are opt-in. When enabled, Durar registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Durar derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when Durar registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Durar.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Durar.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Durar, not `404`.
...
- Approve via:
  - `Durar pairing list mattermost`
  - `Durar pairing approve mattermost <CODE>`
...
Use these target formats with `Durar message send` or cron/webhooks:
...
Durar resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Durar sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Durar sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Durar derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Durar run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="Durar-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"Durar-mattermost-interactions",
...
`Durar message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: Durar
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as Durar. Set an
