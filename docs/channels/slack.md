...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Durar HTTP mode">
...
```json
{
  "display_information": {
    "name": "Durar",
    "description": "Slack connector for Durar"
  },
  "features": {
    "bot_user": {
      "display_name": "Durar",
    ...
    "slash_commands": [
      {
        "command": "/Durar",
        "description": "Send a message to Durar",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `Durar pairing approve slack <code>`.
...
- If streaming fails mid-reply, Durar falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Durar is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are Durar-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Durar falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `Durar channels status --probe --json` shows `botTokenStatus` or
