...
read_when:
  - Setting up Twitch chat integration for Durar
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. Durar connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current Durar releases, so normal
...
```bash
Durar plugins install @Durar/twitch
```

Local checkout (when running from a git repo):

```bash
Durar plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `Durar_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "Durar", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use Durar_TWITCH_ACCESS_TOKEN env var)
...
```bash
Durar_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
