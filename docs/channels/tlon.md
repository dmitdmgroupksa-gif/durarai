...
Tlon is a decentralized messenger built on Urbit. Durar connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Durar releases, so normal packaged
...
```bash
Durar plugins install @Durar/tlon
```

Local checkout (when running from a git repo):

```bash
Durar plugins install ./path/to/local/tlon-plugin
```
...
By default, Durar blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `Durar message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Durar replies in-thread.
