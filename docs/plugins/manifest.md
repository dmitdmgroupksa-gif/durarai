...
supported hook packs when the layout matches Durar runtime expectations.
...
`imageGenerationProviders` are deprecated. Use `Durar doctor --fix` to move
...
| `package.json`         | npm metadata, dependency installation, and the `Durar` block used for entrypoints and setup or catalog metadata |
