...
Bind a TaskFlow runtime to an existing Durar session key or trusted tool
...
trusted Durar session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "Durar plugin SDK", count: 5 },
});
```
...
| `api.config`             | `DurarConfig`          | Current config snapshot                                          |
