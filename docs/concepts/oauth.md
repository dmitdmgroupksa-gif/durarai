...
Durar supports “subscription auth” via OAuth for providers that offer it (notably **OpenAI Codex (ChatGPT OAuth)**). For Anthropic subscriptions, you can either use the **setup-token** flow or reuse a local **Claude CLI** login on the gateway host. Anthropic subscription use outside Claude Code has been restricted for some users in the past, so treat it as a user-choice risk and verify current Anthropic policy yourself. OpenAI Codex OAuth is explicitly supported for use in external tools like Durar. This page explains:
...
Run `claude setup-token` on any machine, then paste it into Durar:
...
- `Durar onboard` → auth choice `anthropic-cli`
- `Durar onboard` → auth choice `setup-token` (Anthropic)
