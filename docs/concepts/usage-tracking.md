...
- `/usage cost` in chats: local cost summary aggregated from Durar session logs.
- CLI: `Durar status --usage` prints a full per-provider breakdown.
- CLI: `Durar channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Durar treats
  ...
  quota, so Durar inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Durar prefers the
...
can supply plugin-specific usage auth logic; otherwise Durar falls back to
