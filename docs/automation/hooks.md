...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `Durar hooks`, while hook-pack installation and updates now go through `Durar plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Durar. See [Webhook Hooks](/automation/webhook) or use `Durar webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `Durar hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but Durar keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend Durar's behavior without modifying core code

## Getting Started

### Bundled Hooks

Durar ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.Durar/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.Durar/logs/commands.log`
...
During onboarding (`Durar onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so Durar requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with Durar; located at `<Durar>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.Durar/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `Durar.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.Durar/hooks/<id>`.
Each `Durar.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `Durar plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.Durar.ai/automation/hooks#my-hook
metadata:
...
The `metadata.Durar` object supports:
...
```typescript
{
  ...
  cfg: DurarConfig            // Current gateway config
}
```
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Durar persists them.
...
Runs after the built-in install security scan and before installation continues. Durar fires this hook for interactive skill installs as well as plugin bundle, package, and single-file installs.

Default behavior differs by target type:

- Plugin installs fail closed on built-in scan `critical` findings and scan errors unless the operator explicitly uses `Durar plugins install --dangerously-force-unsafe-install`.
...
- **`origin`**: Normalized install origin when available (for example `Durar-bundled`, `Durar-workspace`, `plugin-bundle`, `plugin-package`, or `plugin-file`)
...
```json
{
  ...
  "sourcePath": "/var/folders/.../Durar-plugin-acme-audit/package",
  "sourcePathKind": "directory",
  "origin": "plugin-package",
  "request": {
    "kind": "plugin-npm",
    "mode": "install",
    "requestedSpecifier": "@acme/Durar-plugin-audit@1.4.2"
  },
  "builtinScan": {
    "status": "ok",
    "scannedFiles": 12,
  ...
  "plugin": {
    "pluginId": "acme-audit",
    "contentType": "package",
    "packageName": "@acme/Durar-plugin-audit",
...
- **Managed hooks** (`~/.Durar/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.Durar/hooks/my-hook
cd ~/.Durar/hooks/my-hook
```
...
metadata: { "Durar": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
Durar hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.Durar/workspace`)
...
```yaml
metadata: { "Durar": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "Durar": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.Durar/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.Durar/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.Durar/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/dmitdmgroupksa-gif/durarai/tree/main/src/hooks/bundled)
