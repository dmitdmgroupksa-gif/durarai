...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into Durar
...
- Jobs persist at `~/.Durar/cron/jobs.json` so restarts do not lose schedules.
...
together`, and similar hints) and no descendant subagent run is still
  responsible for the final answer, Durar re-prompts once for the actual
...
running, Durar suppresses that partial parent update instead of announcing it.
...
Wire Gmail inbox triggers to Durar via Google PubSub.

**Prerequisites**: `gcloud` CLI, `gog` (gogcli), Durar hooks enabled, Tailscale for the public HTTPS endpoint.

### Wizard setup (recommended)

```bash
Durar webhooks gmail setup --account Durar@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `Durar_SKIP_GMAIL_WATCHER=1` to opt out.
...
# Edit a job
Durar cron edit <jobId> --message "Updated prompt" --model "opus"
...
- `Durar cron add|edit --model ...` changes the job's selected model.
...
Disable cron: `cron.enabled: false` or `Durar_SKIP_CRON=1`.
...
- Check `cron.enabled` and `Durar_SKIP_CRON` env var.
- Confirm the Gateway is running continuously.
- For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
- `reason: not-due` in run output means manual run was checked with `Durar cron run <jobId> --due` and the job was not due yet.
...
- If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`),
  Durar suppresses direct outbound delivery and also suppresses the fallback
