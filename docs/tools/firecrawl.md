...
Durar can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `Durar configure --section web` enables the bundled Firecrawl plugin automatically.
...
Durar always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Durar
