...
Plugins extend Durar with new capabilities: channels, model providers, tools,
skills, speech, image generation, and more. Some plugins are **core** (shipped
with Durar), others are **external** (published on npm by the community).
...
```text
/plugin install clawhub:@Durar/voice-call
...
Durar plugins status                  # operational summary
Durar plugins doctor                  # diagnostics

Durar plugins install <package>        # install (ClawHub first, then npm)
Durar plugins install clawhub:<pkg>   # install from ClawHub only
...
instead, while `Durar skills install` remains the separate ClawHub skill
