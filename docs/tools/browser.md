...
- If browser control is enabled and no auth is configured, Durar auto-generates `gateway.auth.token` on startup and persists it to config.
...
error. ARIA snapshots and basic screenshots still work for Durar-managed Chrome.
