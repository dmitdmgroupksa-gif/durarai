...
- `gateway`/`node` approvals are controlled by `~/.Durar/exec-approvals.json`.
...
- Durar sets `Durar_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.Durar/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#no-approval-yolo-mode).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Durar prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`Durar security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `Durar doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`Durar security audit` and `Durar doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
