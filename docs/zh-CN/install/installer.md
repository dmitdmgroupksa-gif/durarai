...
Durar 提供三个安装脚本，由 `Durar.ai` 提供。

| 脚本                               | 平台                  | 功能                                                                          |
| ---------------------------------- | --------------------- | ----------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL   | 如有需要则安装 Node，通过 npm（默认）或 git 安装 Durar，并可运行新手引导。 |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL   | 将 Node + Durar 安装到本地前缀（`~/.Durar`）中。无需 root。             |
| [`install.ps1`](#installps1)       | Windows（PowerShell） | 如有需要则安装 Node，通过 npm（默认）或 git 安装 Durar，并可运行新手引导。 |
...
<Note>
如果安装成功但在新终端中找不到 `Durar`，请参见 [Node.js 故障排除](/install/node#troubleshooting)。
</Note>
...
<Steps>
  ...
  <Step title="默认确保使用 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（macOS 上使用 Homebrew，Linux apt/dnf/yum 上使用 NodeSource 设置脚本）。为了兼容性，Durar 仍支持 Node 22 LTS，目前为 `22.16+`。
  </Step>
  <Step title="确保安装 Git">
    如果缺少 Git，则安装它。
  </Step>
  <Step title="安装 Durar">
    - `npm` 方法（默认）：全局 npm 安装
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装依赖，构建，然后将包装器安装到 `~/.local/bin/Durar`
  </Step>
  <Step title="安装后任务">
    - 在升级和 git 安装时运行 `Durar doctor --non-interactive`（尽力而为）
...
如果在 Durar 检出目录中运行（`package.json` + `pnpm-workspace.yaml`），脚本会提供：
...
| `--git-dir <path>`                    | 检出目录（默认：`~/Durar`）。别名：`--dir`     |
...
| 变量                                                    | 说明                                 |
| ------------------------------------------------------- | ------------------------------------ |
| `Durar_INSTALL_METHOD=git\|npm`                      | 安装方法                             |
| `Durar_VERSION=latest\|next\|main\|<semver>\|<spec>` | npm 版本、dist-tag 或包规范          |
| `Durar_BETA=0\|1`                                    | 如有可用则使用 beta                  |
| `Durar_GIT_DIR=<path>`                               | 检出目录                             |
| `Durar_GIT_UPDATE=0\|1`                              | 切换 git 更新                        |
| `Durar_NO_PROMPT=1`                                  | 禁用提示                             |
| `Durar_NO_ONBOARD=1`                                 | 跳过新手引导                         |
| `Durar_DRY_RUN=1`                                    | 试运行模式                           |
| `Durar_VERBOSE=1`                                    | 调试模式                             |
| `Durar_NPM_LOGLEVEL=error\|warn\|notice`             | npm 日志级别                         |
...
<Info>
适用于你希望所有内容都放在本地前缀（默认 `~/.Durar`）下，并且不依赖系统 Node 的环境。
</Info>
...
<Steps>
  ...
  <Step title="在前缀下安装 Durar">
    使用 `--prefix <prefix>` 通过 npm 安装，然后将包装器写入 `<prefix>/bin/Durar`。
  </Step>
</Steps>
...
| 标志                   | 说明                                                                   |
| ---------------------- | ---------------------------------------------------------------------- |
| `--prefix <path>`      | 安装前缀（默认：`~/.Durar`）                                        |
| `--version <ver>`      | Durar 版本或 dist-tag（默认：`latest`）                             |
| `--node-version <ver>` | Node 版本（默认：`22.22.0`）                                           |
| `--json`               | 输出 NDJSON 事件                                                       |
| `--onboard`            | 安装后运行 `Durar onboard`                                          |
...
| 变量                                        | 说明                                                   |
| ------------------------------------------- | ------------------------------------------------------ |
| `Durar_PREFIX=<path>`                    | 安装前缀                                               |
| `Durar_VERSION=<ver>`                    | Durar 版本或 dist-tag                               |
| `Durar_NODE_VERSION=<ver>`               | Node 版本                                              |
| `Durar_NO_ONBOARD=1`                     | 跳过新手引导                                           |
| `Durar_NPM_LOGLEVEL=error\|warn\|notice` | npm 日志级别                                           |
| `Durar_GIT_DIR=<path>`                   | 旧版清理查找路径（用于删除旧的 `Peekaboo` 子模块检出） |
| `SHARP_IGNORE_GLOBAL_LIBVIPS=0\|1`          | 控制 sharp/libvips 行为（默认：`1`）                   |
...
<Steps>
  ...
  <Step title="安装 Durar">
    ...
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装/构建，并将包装器安装到 `%USERPROFILE%\.local\bin\Durar.cmd`
  </Step>
  <Step title="安装后任务">
    在可能情况下将所需 bin 目录添加到用户 PATH，然后在升级和 git 安装时运行 `Durar doctor --non-interactive`（尽力而为）。
  </Step>
</Steps>
...
| `-GitDir <path>`            | 检出目录（默认：`%USERPROFILE%\Durar`）   |
| `-NoOnboard`                | 跳过新手引导                                 |
...
| `Durar_GIT_DIR=<path>`          | 检出目录      |
| `Durar_NO_ONBOARD=1`            | 跳过新手引导  |
| `Durar_GIT_UPDATE=0`            | 禁用 git pull |
| `Durar_DRY_RUN=1`               | 试运行模式    |
