...
所有日志配置都在 `~/.Durar/Durar.json` 的 `logging` 下。
...
- Durar 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `Durar plugins enable diagnostics-otel` 启用插件。
...
- `Durar.tokens`（计数器，属性：`Durar.token`、`Durar.channel`、`Durar.provider`、`Durar.model`）
- `Durar.cost.usd`（计数器，属性：`Durar.channel`、`Durar.provider`、`Durar.model`）
- `Durar.run.duration_ms`（直方图，属性：`Durar.channel`、`Durar.provider`、`Durar.model`）
- `Durar.context.tokens`（直方图，属性：`Durar.context`、`Durar.channel`、`Durar.provider`、`Durar.model`）

消息流：

- `Durar.webhook.received`（计数器，属性：`Durar.channel`、`Durar.webhook`）
- `Durar.webhook.error`（计数器，属性：`Durar.channel`、`Durar.webhook`）
- `Durar.webhook.duration_ms`（直方图，属性：`Durar.channel`、`Durar.webhook`）
- `Durar.message.queued`（计数器，属性：`Durar.channel`、`Durar.source`）
- `Durar.message.processed`（计数器，属性：`Durar.channel`、`Durar.outcome`）
- `Durar.message.duration_ms`（直方图，属性：`Durar.channel`、`Durar.outcome`）

队列 + 会话：

- `Durar.queue.lane.enqueue`（计数器，属性：`Durar.lane`）
- `Durar.queue.lane.dequeue`（计数器，属性：`Durar.lane`）
- `Durar.queue.depth`（直方图，属性：`Durar.lane` 或 `Durar.channel=heartbeat`）
- `Durar.queue.wait_ms`（直方图，属性：`Durar.lane`）
- `Durar.session.state`（计数器，属性：`Durar.state`、`Durar.reason`）
- `Durar.session.stuck`（计数器，属性：`Durar.state`）
- `Durar.session.stuck_age_ms`（直方图，属性：`Durar.state`）
- `Durar.run.attempt`（计数器，属性：`Durar.attempt`）
...
- `Durar.webhook.error`
  - `Durar.channel`、`Durar.webhook`、`Durar.chatId`、`Durar.error`
- `Durar.message.processed`
  - `Durar.channel`、`Durar.outcome`、`Durar.chatId`、`Durar.messageId`、`Durar.sessionKey`、`Durar.sessionId`、`Durar.reason`
- `Durar.session.stuck`
  - `Durar.state`、`Durar.ageMs`、`Durar.queueDepth`、`Durar.sessionKey`、`Durar.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `Durar doctor`。
