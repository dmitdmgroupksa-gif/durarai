...
read_when:
  ...
  - 你想通过 OpenRouter 在 Durar 中运行模型
summary: 使用 OpenRouter 的统一 API 在 Durar 中访问许多模型
title: OpenRouter
x-i18n:
...
```bash
Durar onboard --auth-choice apiKey --token-provider openrouter --token "$OPENROUTER_API_KEY"
```
