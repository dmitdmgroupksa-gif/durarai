...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.Durar/credentials/oauth.json`; auth profiles live in `~/.Durar/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.Durar/credentials/oauth.json` (or `$Durar_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  ...
  <Step title="Health check">
    ...
    - Tip: `Durar status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
