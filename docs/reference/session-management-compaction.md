...
This document explains how Durar manages sessions end-to-end:
...
Durar is designed around a single **Gateway process** that owns session state.
...
Durar persists sessions in two layers:
...
Durar resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Durar reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.Durar/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Durar intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Durar splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Durar
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Durar preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Durar consumes the events, but Pi decides when to compact).
...
Durar also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Durar bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Durar leaves it alone.
...
Durar supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Durar strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Durar also suppresses **draft/typing streaming** when a
...
Durar uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Durar’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `Durar status`.
