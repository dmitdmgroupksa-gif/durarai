...
- Same path: run the app from a fixed location (for Durar, `dist/Durar.app`).
...
```bash
sudo tccutil reset Accessibility ai.Durar.mac
sudo tccutil reset ScreenCapture ai.Durar.mac
...
Workaround: move files into the Durar workspace (`~/.Durar/workspace`) if you want to avoid per-folder grants.
