...
> **Note:** The Android app has not been publicly released yet. The source code is available in the [Durar repository](https://github.com/dmitdmgroupksa-gif/durarai) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/dmitdmgroupksa-gif/durarai/blob/main/apps/android/README.md) for build instructions.
...
- Set `gateway.bind: "tailnet"` in `~/.Durar/Durar.json` on the gateway host.
