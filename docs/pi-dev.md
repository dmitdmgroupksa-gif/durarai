...
This guide summarizes a sane workflow for working on the pi integration in Durar.
...
```bash
Durar_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm Durar agent --message "Hello" --thinking low`
...
State lives under the Durar state directory. Default is `~/.Durar`. If `Durar_STATE_DIR` is set, use that directory instead.

To reset everything:

- `Durar.json` for config
