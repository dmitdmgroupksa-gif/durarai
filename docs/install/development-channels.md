...
Durar ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/Durar`, override with
  `Durar_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
Durar update --tag 2026.4.1-beta.1
...
Durar update --tag Durar@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `Durar update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Durar prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `Durar update` would do without making changes:

```bash
Durar update --dry-run
Durar update --channel beta --dry-run
Durar update --tag 2026.4.1-beta.1 --dry-run
Durar update --dry-run --json
```
...
When you switch channels with `Durar update`, Durar also syncs plugin
sources:
