...
Tip: keep this VM **stateful**. Durar stores state under `~/.Durar/` and `~/.Durar/workspace/`.
...
token from `gateway.auth.token` on the VM (retrieve with `Durar config get gateway.auth.token`, or generate one
with `Durar doctor --generate-gateway-token`). Approve devices with `Durar devices list` and
`Durar devices approve <requestId>`. When in doubt, use Shelley from your browser!
