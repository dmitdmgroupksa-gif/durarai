summary: "Use Qwen Cloud via Durar's bundled qwen provider"
read_when:

- You want to use Qwen with Durar
...
Durar now treats Qwen as a first-class bundled provider with canonical id
...
Durar onboard --auth-choice qwen-api-key

# China Coding Plan endpoint
Durar onboard --auth-choice qwen-api-key-cn

# Global Standard (pay-as-you-go) endpoint
Durar onboard --auth-choice qwen-standard-api-key

# China Standard (pay-as-you-go) endpoint
Durar onboard --auth-choice qwen-standard-api-key-cn
```
...
For video generation, Durar maps the configured Qwen region to the matching
