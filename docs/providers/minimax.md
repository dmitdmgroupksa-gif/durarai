---
summary: "Use MiniMax models in Durar"
read_when:
  - You want MiniMax models in Durar
...
Durar's MiniMax provider defaults to **MiniMax M2.7**.
...
entries, Durar materializes `MiniMax-M2.7` and
...
```bash
Durar onboard --auth-choice minimax-global-oauth
# or
Durar onboard --auth-choice minimax-cn-oauth
```
...
See the MiniMax plugin package README in the Durar repo for details.
...
```bash
Durar onboard --auth-choice minimax-global-api
# or
...
On the Anthropic-compatible streaming path, Durar now disables MiniMax
...
- On `api: "anthropic-messages"`, Durar injects
...
- Durar normalizes MiniMax coding-plan usage to the same `% left` display
  used by other providers. MiniMax's raw `usage_percent` / `usagePercent`
  fields are remaining quota, not consumed quota, so Durar inverts them.
  Count-based fields win when present. When the API returns `model_remains`,
  Durar prefers the chat-model entry, derives the window label from
...
- Use `Durar models list` to confirm the current provider id, then switch with
  `Durar models set minimax/MiniMax-M2.7` or
  `Durar models set minimax-portal/MiniMax-M2.7`.
...
- Running `Durar configure` and selecting a **MiniMax** auth option, or
