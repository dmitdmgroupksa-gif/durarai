---
summary: "Use Anthropic Claude via API keys, setup-token, or Claude CLI in Durar"
read_when:
...
In Durar you can authenticate with an API key or a **setup-token**.
...
Durar supports Anthropic's prompt caching feature. This is **API-only**; subscription auth does not honor cache settings.
...
Durar includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
...
with subscription auth, Durar logs a warning and falls back to the standard
...
1. Durar launches `claude -p --output-format json ...` on the **gateway
   host**.
...
- Tools are disabled on the Durar side for CLI backend runs.
- Text in, text out. No Durar streaming handoff.
...
Paste the token into Durar (wizard: **Anthropic token (paste setup-token)**), or run it on the gateway host:
...
Durar onboard --auth-choice setup-token
```
...
- Generate the setup-token with `claude setup-token` and paste it, or run `Durar models auth setup-token` on the gateway host.
...
- If the Claude CLI login lives on a different machine, use
  `Durar models auth paste-token --provider anthropic` on the gateway host.

**No API key found for provider "anthropic"**

- Auth is **per agent**. New agents don’t inherit the main agent’s keys.
- Re-run onboarding for that agent, or paste a setup-token / API key on the
  gateway host, then verify with `Durar models status`.
