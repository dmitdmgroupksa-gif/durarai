---
summary: "Use Anthropic Claude via API keys or Claude CLI in Durar"
read_when:
  - You want to use Anthropic models in Durar
...
In Durar, new Anthropic setup should use an API key or the local Claude CLI
...
Separately, Anthropic notified Durar users on **April 4, 2026 at 12:00 PM
PT / 8:00 PM BST** that **Durar counts as a third-party harness**. Their
stated policy is that Durar-driven Claude-login traffic no longer uses the
included Claude subscription pool and instead requires **Extra Usage**
(pay-as-you-go, billed separately from the subscription).

That policy distinction is about **Durar-driven Claude CLI reuse**, not
...
Durar also supports other subscription-style options, including [OpenAI
...
# or non-interactive
Durar onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
```
...
- Anthropic Claude 4.6 models default to `adaptive` thinking in Durar when no explicit thinking level is set.
...
Durar's shared `/fast` toggle also supports direct public Anthropic traffic, including API-key and OAuth-authenticated requests sent to `api.anthropic.com`.
...
- Durar only injects Anthropic service tiers for direct `api.anthropic.com` requests. If you route `anthropic/*` through a proxy or gateway, `/fast` leaves `service_tier` untouched.
...
Durar supports Anthropic's prompt caching feature. This is **API-only**; legacy Anthropic token auth does not honor cache settings.
...
When using Anthropic API Key authentication, Durar automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
Anthropic's 1M context window is beta-gated. In Durar, enable it per model
...
Durar maps this to `anthropic-beta: context-1m-2025-08-07` on Anthropic
requests.
...
(typically API key billing, or Durar's Claude-login path / legacy token auth
...
`context1m: true` with that legacy auth mode, Durar logs a warning and
...
production. Anthropic also told Durar users that **Durar-driven** Claude
...
the Anthropic API directly. Durar treats it as a **CLI backend provider**
...
1. Durar launches `claude -p --output-format stream-json --include-partial-messages ...`
...
4. Your chat messages still go through the normal Durar message pipeline, but
...
- Durar auto-loads the bundled Anthropic plugin at gateway startup when your
...
- Normal Durar message/session routing
...
switch the same gateway host to Claude CLI, Durar supports that as a normal
provider-auth migration path.

Prerequisites:

- Claude CLI installed on the **same gateway host** that runs Durar
...
Interactive `Durar onboard` and `Durar configure` now prefer **Anthropic
...
- Durar does not inject tool calls directly. Claude CLI receives gateway
...
- Auth is read at runtime from live Claude CLI credentials and is not persisted
  to Durar profiles. Keychain prompts are suppressed in non-interactive
  contexts.
...
- Anthropic's public Claude Code docs still document direct CLI usage such as
  ...
  Anthropic's separate notice to Durar users says the **Durar**
...
- Anthropic setup-token is available again in Durar as a legacy/manual path. Anthropic's Durar-specific billing notice still applies, so use it with the expectation that Anthropic requires **Extra Usage** for this path.
...
- Re-run onboarding for that agent, or configure an API key on the gateway
  host, then verify with `Durar models status`.

**No credentials found for profile `anthropic:default`**

- Run `Durar models status` to see which auth profile is active.
- Re-run onboarding, or configure an API key or Claude CLI for that profile path.

**No available auth profile (all in cooldown/unavailable)**

- Check `Durar models status --json` for `auth.unusableProfiles`.
