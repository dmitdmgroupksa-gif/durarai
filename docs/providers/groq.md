...
read_when:
  - You want to use Groq with Durar
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Durar connects
...
media-understanding provider, Durar uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.Durar/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `Durar models list | grep groq`
