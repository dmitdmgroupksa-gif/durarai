---
summary: "Run Durar with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Durar against a local SGLang server
...
Durar can connect to SGLang using the `openai-completions` API.

Durar can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Durar will query:
