...
read_when:
  - You want to use Google Gemini models with Durar
...
```bash
Durar onboard --auth-choice gemini-api-key
```
...
```bash
Durar models auth login --provider google-gemini-cli --set-default
```

Environment variables:

- `Durar_GEMINI_OAUTH_CLIENT_ID`
- `Durar_GEMINI_OAUTH_CLIENT_SECRET`
...
command is installed and on `PATH`. Durar supports both Homebrew installs
...
For direct Gemini API runs (`api: "google-generative-ai"`), Durar now
...
- Gemini cache-hit usage is normalized into Durar `cacheRead` from
