---
summary: "Use StepFun models with Durar"
read_when:
  - You want StepFun models in Durar
...
Durar includes a bundled StepFun provider plugin with two provider ids:
...
```bash
Durar onboard --auth-choice stepfun-standard-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
Durar onboard --auth-choice stepfun-plan-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
```
...
- The provider is bundled with Durar, so there is no separate plugin install step.
...
- Use `Durar models list` and `Durar models set <provider/model>` to inspect or switch models.
