---
summary: "Use Synthetic's Anthropic-compatible API in Durar"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Durar registers it as the
...
```bash
Durar onboard --auth-choice synthetic-api-key
```
...
Note: Durar's Anthropic client appends `/v1` to the base URL, so use
