...
struct ChatMessageBubble: View {
    let message: DurarChatMessage
    let style: DurarChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? DurarChatTheme.userText : DurarChatTheme.assistantText
    ...
    private var inlineAttachments: [DurarChatMessageContent] {
    ...
    private var toolCalls: [DurarChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [DurarChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? DurarChatTheme.userBubble
        }
        if self.style == .onboarding {
            return DurarChatTheme.onboardingAssistantBubble
        }
        return DurarChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return DurarChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: DurarChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? DurarChatTheme.userText : DurarChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: DurarChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(DurarChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [DurarChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: DurarChatTheme.assistantText)
            }
        }
    }
}
