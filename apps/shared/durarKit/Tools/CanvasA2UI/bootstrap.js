...
import "@a2ui/lit/ui";
import { themeContext } from "@Durar/a2ui-theme-context";
...
class DurarA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.DurarA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["Durar:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.DurarCanvasA2UIAction ??
      globalThis.DurarCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.DurarCanvasA2UIAction) {
...
if (!customElements.get("Durar-a2ui-host")) {
  customElements.define("Durar-a2ui-host", DurarA2UIHost);
}
