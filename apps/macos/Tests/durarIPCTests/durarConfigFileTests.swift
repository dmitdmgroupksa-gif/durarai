...
@Suite(.serialized)
struct DurarConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("Durar-config-\(UUID().uuidString)")
            .appendingPathComponent("Durar.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["Durar_CONFIG_PATH": override]) {
            #expect(DurarConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["Durar_CONFIG_PATH": override]) {
            ...
            #expect(DurarConfigFile.remoteGatewayPort() == 19999)
            #expect(DurarConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(DurarConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(DurarConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["Durar_CONFIG_PATH": override]) {
            ...
            DurarConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = DurarConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["Durar_CONFIG_PATH": override]) {
            ...
            DurarConfigFile.clearRemoteGatewayUrl()
            let root = DurarConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("Durar-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "Durar_CONFIG_PATH": nil,
            "Durar_STATE_DIR": dir,
        ]) {
            #expect(DurarConfigFile.stateDirURL().path == dir)
            #expect(DurarConfigFile.url().path == "\(dir)/Durar.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("Durar-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("Durar.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "Durar_STATE_DIR": stateDir.path,
            "Durar_CONFIG_PATH": configPath.path,
        ]) {
            DurarConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-Durar-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = DurarConfigFile.loadDict()
