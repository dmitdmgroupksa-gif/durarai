...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: DurarProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: DurarProtocol.AnyCodable] = [
            "presence": .init([DurarProtocol.AnyCodable(entry)]),
        ]
        let payload = DurarProtocol.AnyCodable(payloadMap)
