...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.Durar.mac`) even if the shipped
    ...
    {
        if environment["Durar_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "Durar.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "Durar.nixMode") { return true }
