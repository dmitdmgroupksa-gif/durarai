...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: DurarProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.Durar", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: DurarKit.AnyCodable]()) {
                $0[$1.key] = DurarKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: DurarProtocol.AnyCodable?) -> [String: DurarProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: DurarProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: DurarKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: DurarProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("Durar.control.heartbeat")
    static let controlAgentEvent = Notification.Name("Durar.control.agent")
}
