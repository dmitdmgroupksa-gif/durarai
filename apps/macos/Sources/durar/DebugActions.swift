...
enum DebugActions {
    private static let verboseDefaultsKey = "Durar.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "Durar.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = DurarPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Durar", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = DurarPaths.configURL
        guard
