    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so Durar can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = DurarLocationMode.off.rawValue
    ...
    private var locationMode: DurarLocationMode {
        DurarLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: DurarLocationMode) async -> Bool {
