...
enum CommandResolver {
    private static let projectRootDefaultsKey = "Durar.gatewayProjectRootPath"
    private static let helperName = "Durar"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let DurarEntry = root.appendingPathComponent("Durar.mjs").path
        if FileManager().isReadableFile(atPath: DurarEntry) { return DurarEntry }
        let binEntry = root.appendingPathComponent("bin/Durar.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/Durar")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let DurarPaths = self.DurarManagedPaths(home: home)
        if !DurarPaths.isEmpty {
            extras.insert(contentsOf: DurarPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + DurarPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func DurarManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".Durar"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func DurarExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectDurarExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("Durar.mjs").path,
            root.appendingPathComponent("bin/Durar.js").path,
        ]
    ...
    static func hasAnyDurarInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.DurarExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let DurarPath = self.projectDurarExecutable(projectRoot: root) {
            return [DurarPath, subcommand] + extraArgs
        }
        if let DurarPath = self.DurarExecutable(searchPaths: searchPaths) {
            return [DurarPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "Durar", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            Durar entrypoint missing (looked for dist/index.js or Durar.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real Durar CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/Durar"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/Durar.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/Durar.mjs"
            node "$PRJ/Durar.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/Durar.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/Durar.js"
            node "$PRJ/bin/Durar.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent Durar"
          pnpm --silent Durar \(quotedArgs);
        else
          echo "Durar CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? DurarConfigFile.loadDict()
