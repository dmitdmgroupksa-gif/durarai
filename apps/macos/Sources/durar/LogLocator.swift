...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["Durar_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/Durar")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("Durar-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("Durar-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/Durar/ (rolling or stdout), or nil if none exist.
