...
import ai.Durar.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      DurarCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      DurarCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      DurarCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      DurarCanvasA2UICommand.Push.rawValue, DurarCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      DurarCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      DurarCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      DurarCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      DurarLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      DurarDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      DurarDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      DurarDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      DurarDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      DurarNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      DurarNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      DurarSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.Durar.app.protocol.DurarPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      DurarContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      DurarContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      DurarCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      DurarCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      DurarMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      DurarMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      DurarSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      DurarSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      DurarCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
