...
private const val NOTIFICATION_CHANNEL_BASE_ID = "Durar.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Durar Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Durar Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Durar Active")
      }
