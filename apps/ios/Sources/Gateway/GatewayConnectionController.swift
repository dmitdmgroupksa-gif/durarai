...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [DurarCapability.canvas.rawValue, DurarCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(DurarCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(DurarCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = DurarLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(DurarCapability.location.rawValue) }

        caps.append(DurarCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(DurarCapability.watch.rawValue)
        }
        caps.append(DurarCapability.photos.rawValue)
        caps.append(DurarCapability.contacts.rawValue)
        caps.append(DurarCapability.calendar.rawValue)
        caps.append(DurarCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(DurarCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            DurarScreenCommand.record.rawValue,
            DurarSystemCommand.notify.rawValue,
            DurarChatCommand.push.rawValue,
        ...
        if caps.contains(DurarCapability.camera.rawValue) {
            commands.append(DurarCameraCommand.list.rawValue)
            commands.append(DurarCameraCommand.snap.rawValue)
            commands.append(DurarCameraCommand.clip.rawValue)
        }
        if caps.contains(DurarCapability.location.rawValue) {
            commands.append(DurarLocationCommand.get.rawValue)
        }
        if caps.contains(DurarCapability.device.rawValue) {
            commands.append(DurarDeviceCommand.status.rawValue)
            commands.append(DurarDeviceCommand.info.rawValue)
        }
        if caps.contains(DurarCapability.watch.rawValue) {
            commands.append(DurarWatchCommand.status.rawValue)
            commands.append(DurarWatchCommand.notify.rawValue)
        }
        if caps.contains(DurarCapability.photos.rawValue) {
            commands.append(DurarPhotosCommand.latest.rawValue)
        }
        if caps.contains(DurarCapability.contacts.rawValue) {
            commands.append(DurarContactsCommand.search.rawValue)
            commands.append(DurarContactsCommand.add.rawValue)
        }
        if caps.contains(DurarCapability.calendar.rawValue) {
            commands.append(DurarCalendarCommand.events.rawValue)
            commands.append(DurarCalendarCommand.add.rawValue)
        }
        if caps.contains(DurarCapability.reminders.rawValue) {
            commands.append(DurarRemindersCommand.list.rawValue)
            commands.append(DurarRemindersCommand.add.rawValue)
        }
        if caps.contains(DurarCapability.motion.rawValue) {
            commands.append(DurarMotionCommand.activity.rawValue)
            commands.append(DurarMotionCommand.pedometer.rawValue)
        }
