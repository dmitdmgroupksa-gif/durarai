...
final class RemindersService: RemindersServicing {
    func list(params: DurarRemindersListParams) async throws -> DurarRemindersListPayload {
        ...
        let payload: [DurarReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return DurarRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: DurarRemindersAddParams) async throws -> DurarRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = DurarReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return DurarRemindersAddPayload(reminder: payload)
    }
