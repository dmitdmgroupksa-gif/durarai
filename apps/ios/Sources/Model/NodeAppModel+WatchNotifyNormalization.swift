...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: DurarWatchNotifyParams) -> DurarWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [DurarWatchAction]?,
        kind: String?,
        promptId: String?) -> [DurarWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> DurarWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return DurarWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                DurarWatchAction(id: "approve", label: "Approve"),
                DurarWatchAction(id: "decline", label: "Decline", style: "destructive"),
                DurarWatchAction(id: "open_phone", label: "Open iPhone"),
                DurarWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            DurarWatchAction(id: "done", label: "Done"),
            DurarWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            DurarWatchAction(id: "open_phone", label: "Open iPhone"),
            DurarWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: DurarWatchRisk?,
        priority: DurarNotificationPriority?) -> DurarWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: DurarNotificationPriority?,
        risk: DurarWatchRisk?) -> DurarNotificationPriority?
    {
