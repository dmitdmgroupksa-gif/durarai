...
final class MotionService: MotionServicing {
    func activities(params: DurarMotionActivityParams) async throws -> DurarMotionActivityPayload {
        ...
        let mapped: [DurarMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return DurarMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: DurarPedometerParams) async throws -> DurarPedometerPayload {
        ...
        let payload: DurarPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = DurarPedometerPayload(
