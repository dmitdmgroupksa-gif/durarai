---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Durar deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Durar cron scheduling for periodic checks, or version status checks on a machine running Durar (laptop, workstation, Pi, VPS).
---

# Durar Host Hardening

## Overview

Assess and harden the host running Durar, then align it to a user-defined risk tolerance without breaking access. Use Durar security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Durar changes the host firewall, SSH, or OS updates; it does not.
...
5. Durar gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Durar can access sensitive data.
...
### 2) Run Durar security audits (read-only)

As part of the default read-only checks, run `Durar security audit --deep`. Only offer alternatives if the user requests them:

1. `Durar security audit` (faster, non-probing)
2. `Durar security audit --json` (structured output)

Offer to apply Durar safe defaults (numbered):

1. `Durar security audit --fix`

Be explicit that `--fix` only tightens Durar defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Durar version/update status (read-only)

As part of the default read-only checks, run `Durar update status`.
...
- Credential hygiene notes (location of Durar creds, prefer disk encryption)
...
After Durar install or first hardening pass, run at least one baseline audit and version check:

- `Durar security audit`
- `Durar security audit --deep`
- `Durar update status`

Ongoing monitoring is recommended. Use the Durar cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `Durar cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `Durar update status`
...
Before creating, `Durar cron list` and match on exact `name`. If found, `Durar cron edit <id> ...`.
If not found, `Durar cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `Durar update status` (preferred for source checkouts and channels)
2. `npm view Durar version` (published npm version)

## Durar command accuracy

Use only supported commands and flags:

- `Durar security audit [--deep] [--fix] [--json]`
- `Durar status` / `Durar status --deep`
- `Durar health --json`
- `Durar update status`
- `Durar cron add|list|runs|run`

Do not invent CLI flags or imply Durar enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Durar compaction:
