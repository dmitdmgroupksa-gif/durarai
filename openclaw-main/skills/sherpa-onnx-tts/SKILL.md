...
1. Download the runtime for your OS (extracts into `$Durar_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.Durar/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$Durar_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.Durar/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Durar config file (`$Durar_CONFIG_PATH`, default `~/.Durar/Durar.json`):
