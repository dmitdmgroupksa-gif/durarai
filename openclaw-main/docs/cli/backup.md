...
Create a local backup archive for Durar state, config, credentials, sessions, and optionally workspaces.
...
If you use `--only-config`, Durar skips state, credentials, and workspace discovery and archives only the active config file path.

Durar canonicalizes paths before building the archive. If config, credentials, or a workspace already live inside the state directory, they are not duplicated as separate top-level backup sources. Missing paths are skipped.
