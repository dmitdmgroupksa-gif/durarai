...
Durar supports **camera capture** for agent workflows:
...
```bash
Durar nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
Durar nodes camera snap --node <id> --facing front
Durar nodes camera clip --node <id> --duration 3000
Durar nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`Durar.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `Durar` CLI to invoke camera commands on the macOS node.

Examples:

```bash
Durar nodes camera list --node <id>            # list camera ids
Durar nodes camera snap --node <id>            # prints MEDIA:<path>
Durar nodes camera snap --node <id> --max-width 1280
Durar nodes camera snap --node <id> --delay-ms 2000
Durar nodes camera snap --node <id> --device-id <id>
Durar nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
Durar nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
Durar nodes camera clip --node <id> --device-id <id>
Durar nodes camera clip --node <id> --no-audio
```

Notes:

- `Durar nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
Durar nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
