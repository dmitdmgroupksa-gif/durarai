...
- `sessionTarget: "isolated"` or `sessionTarget: "session:..."` creates a visible task that shows up in `Durar tasks` with delivery notifications.
...
- Jobs persist under `~/.Durar/cron/` so restarts don’t lose schedules.
...
- For upgrades, `Durar doctor --fix` can normalize legacy cron store fields, including old top-level delivery hints such as `threadId`.
...
Cron jobs are persisted on the Gateway host at `~/.Durar/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `Durar cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Durar applies a
...
Main-session cron jobs create [background task](/automation/tasks) records with `silent` notify policy (no notifications by default). They appear in `Durar tasks list` but do not generate delivery messages.
...
These detached runs create [background task](/automation/tasks) records visible in `Durar tasks` and subject to task audit and maintenance.
...
If `delivery` is omitted for isolated jobs, Durar defaults to `announce`.
...
- CLI equivalents: `Durar cron add --light-context ...` and `Durar cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `Durar cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.Durar/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.Durar/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Durar classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Durar archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `Durar cron runs` and adjust retention before logs become large
...
Durar cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
Durar cron edit <jobId> --agent ops
Durar cron edit <jobId> --clear-agent
```
...
Durar cron add --name "Scoped job" --cron "0 8 * * *" --session isolated --message "Run scoped checks" --tools exec,read

# Update an existing job's tool allowlist
Durar cron edit <jobId> --tools exec,read,write

# Remove a tool allowlist (use all tools)
Durar cron edit <jobId> --clear-tools
```
...
```bash
Durar system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`Durar system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `Durar_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Durar applies exponential retry backoff for recurring jobs after consecutive errors:
