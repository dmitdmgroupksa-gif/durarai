...
- **Cron (isolated)** is a scheduled **background run** — creates a task record tracked in `Durar tasks`.
...
- **Task tracking**: isolated jobs create [background task](/automation/tasks) records visible in `Durar tasks` and `Durar tasks audit`.
...
Durar cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
Durar cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
Durar cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```
...
| [Tasks](/automation/tasks) | No task record                  | Task record (silent)     | Task record (visible in `Durar tasks`)       |
