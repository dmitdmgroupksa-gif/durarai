...
read_when:
  ...
  - Wiring external systems into Durar
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Durar config directory (typically `~/.Durar/hooks/transforms`).
...
- `Durar webhooks gmail setup` writes `hooks.gmail` config for `Durar webhooks gmail run`.
