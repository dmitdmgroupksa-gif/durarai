...
<Steps>
  ...
  <Step title="Health check">
    ...
    - `Durar status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.Durar/credentials/oauth.json`
...
`~/.Durar/credentials/oauth.json` (or `$Durar_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
