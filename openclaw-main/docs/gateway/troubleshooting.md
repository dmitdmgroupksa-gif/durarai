...
- `Durar channels status --probe` shows connected/ready channels.
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `Durar configure`). If you are running Durar via Podman, the default config path is `~/.Durar/Durar.json`.
