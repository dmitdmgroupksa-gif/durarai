...
- Strict command paths (for example `Durar memory` remote-memory paths and `Durar qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
