---
summary: "ClawHub guide: public registry, native Durar install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Durar skills and plugins**.
...
```bash
Durar plugins install clawhub:<package>
...
If you want to add new capabilities to your Durar agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
directory. `Durar plugins install clawhub:...` records a normal managed
...
current working directory. If an Durar workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Durar loads workspace skills from `<workspace>/skills`
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/Durar/skills` and `~/.Durar/skills`). This is designed to find older skill installs without extra flags.
