...
read_when:
  - You want to use Vercel AI Gateway with Durar
...
- Durar auto-discovers the Gateway `/v1/models` catalog, so `/models vercel-ai-gateway`
...
```bash
Durar onboard --auth-choice ai-gateway-api-key
```
...
Durar accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
