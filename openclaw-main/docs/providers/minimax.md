...
```bash
Durar plugins enable minimax  # skip if already loaded.
Durar gateway restart  # restart if gateway is already running
Durar onboard --auth-choice minimax-portal
```
...
- Use `Durar models list` and `Durar models set minimax/MiniMax-M2.7` to switch.
