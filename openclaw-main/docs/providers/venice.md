---
summary: "Use Venice AI privacy-focused models in Durar"
read_when:
  - You want privacy-focused inference in Durar
...
```bash
Durar onboard --auth-choice venice-api-key
```
...
```bash
Durar agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Durar shows all available Venice models. Pick based on your needs:
...
```bash
Durar models set venice/kimi-k2-5
Durar models set venice/claude-opus-4-6
```

List all available models:

```bash
Durar models list | grep venice
```

## Configure via `Durar configure`

1. Run `Durar configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Durar automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
Durar agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
Durar agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
Durar agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
Durar agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
Durar agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `Durar models list` to see currently available models. Some models may be temporarily offline.
