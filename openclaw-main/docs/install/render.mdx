---
summary: "Deploy Durar on Render with Infrastructure-as-Code"
read_when:
  - Deploying Durar to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Durar on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/dmitdmgroupksa-gif/durarai)
...
repository configures everything needed to run Durar:
...
services:
  - type: web
    ...
    envVars:
      - key: Durar_GATEWAY_PORT
        value: "8080"
      - key: Durar_STATE_DIR
        value: /data/.Durar
      - key: Durar_WORKSPACE_DIR
        value: /data/workspace
      - key: Durar_GATEWAY_TOKEN
...
Connect using the `Durar_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original Durar repository, Render will not auto-deploy your Durar. To update it, run a manual Blueprint sync from the dashboard.
...
For Durar, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any Durar host. See [Backup](/cli/backup) for details.
...
- Missing `Durar_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `Durar_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `Durar backup create` in the Render shell.
