---
summary: "Move (migrate) an Durar install from one machine to another"
read_when:
  - You are moving Durar to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Durar to a New Machine

This guide moves an Durar gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.Durar/` by default) and your **workspace**, you preserve:

- **Config** -- `Durar.json` and all gateway settings
...
<Tip>
Run `Durar status` on the old machine to confirm your state directory path.
Custom profiles use `~/.Durar-<profile>/` or a path set via `Durar_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    Durar gateway stop
    cd ~
    tar -czf Durar-state.tgz .Durar
    ```

    If you use multiple profiles (e.g. `~/.Durar-work`), archive each separately.

  </Step>

  <Step title="Install Durar on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.Durar/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `Durar_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `Durar doctor`.
  </Accordion>

  <Accordion title="Copying only Durar.json">
...
- [ ] `Durar status` shows the gateway running
