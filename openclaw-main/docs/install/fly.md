...
<Steps>
  ...
  <Step title="Set secrets">
    ...
    **Notes:**
    ...
    - Non-loopback binds (`--bind lan`) require `Durar_GATEWAY_TOKEN` for security.
  ...
  <Step title="Access the Gateway">
    ...
    Paste your gateway token (the one from `Durar_GATEWAY_TOKEN`) to authenticate.
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/Durar.json` should be read on restart.
