---
read_when:
  - 在 DigitalOcean 上设置 Durar
  - 寻找适合 Durar 的低价 VPS 托管
summary: 在 DigitalOcean 上运行 Durar（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Durar

## 目标

在 DigitalOcean 上以 **每月 6 美元**（或预留定价时每月 4 美元）运行一个持久化的 Durar Gateway 网关。
...
- `~/.Durar/` — 配置、凭证、会话数据
- `~/.Durar/workspace/` — 工作区（`SOUL.md`、memory 等）
...
journalctl -u Durar --no-pager -n 50
```
