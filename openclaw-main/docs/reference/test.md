...
- `pnpm test:e2e`: Runs gateway end-to-end smoke tests (multi-instance WS/HTTP/node pairing). Defaults to `forks` + adaptive workers in `vitest.e2e.config.ts`; tune with `Durar_E2E_WORKERS=<n>` and set `Durar_E2E_VERBOSE=1` for verbose logs.
...
Script: [`scripts/bench-model.ts`](https://github.com/dmitdmgroupksa-gif/durarai/blob/main/scripts/bench-model.ts)
...
Script: [`scripts/bench-cli-startup.ts`](https://github.com/dmitdmgroupksa-gif/durarai/blob/main/scripts/bench-cli-startup.ts)
