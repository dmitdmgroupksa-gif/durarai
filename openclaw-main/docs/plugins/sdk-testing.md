...
summary: "Testing utilities and patterns for Durar plugins"
read_when:
...
Reference for test utilities, patterns, and lint enforcement for Durar
plugins.
...
**Import:** `Durar/plugin-sdk/testing`
...
import { installCommonResolveTargetErrorCases } from "Durar/plugin-sdk/testing";
...
import type { PluginRuntime } from "Durar/plugin-sdk/runtime-store";
...
1. **No monolithic root imports** -- `Durar/plugin-sdk` root barrel is rejected
...
Durar uses Vitest with V8 coverage thresholds. For plugin tests:
...
```bash
Durar_TEST_PROFILE=low Durar_TEST_SERIAL_GATEWAY=1 pnpm test
```
