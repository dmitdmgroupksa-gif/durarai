# Durar iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Durar Gateway as a `role: node`.
...
- Beta release uses canonical `ai.Durar.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `DurarPushTransport=relay`, `DurarPushDistribution=official`, and `DurarPushAPNsEnvironment=production`.
...
- `Durar_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
- `apps/ios/Sources/Durar.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `DurarPushTransport=direct` and `DurarPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `DurarPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `Durar.json`. `Durar_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Durar official builds and to ensure a
