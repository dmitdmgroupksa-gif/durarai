...
@MainActor
final class DurarAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.Durar.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.Durar.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.Durar.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "Durar.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "Durar.watch.promptId"
    static let sessionKeyKey = "Durar.watch.sessionKey"
    static let actionPrimaryIDKey = "Durar.watch.action.primary.id"
    static let actionPrimaryLabelKey = "Durar.watch.action.primary.label"
    static let actionSecondaryIDKey = "Durar.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "Durar.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "Durar.watch.action.primary"
    static let actionSecondaryIdentifier = "Durar.watch.action.secondary"
    static let actionIdentifierPrefix = "Durar.watch.action."
    static let actionIDKeyPrefix = "Durar.watch.action.id."
    static let actionLabelKeyPrefix = "Durar.watch.action.label."
    static let categoryPrefix = "Durar.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: DurarWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> DurarWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return DurarWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Durar" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [DurarWatchAction]) -> [UNNotificationAction] {
...
struct DurarApp: App {
    ...
    @UIApplicationDelegateAdaptor(DurarAppDelegate.self) private var appDelegate
...
extension DurarApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Durar: installing uncaught exception handler")
