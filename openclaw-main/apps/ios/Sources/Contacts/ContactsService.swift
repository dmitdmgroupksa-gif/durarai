...
final class ContactsService: ContactsServicing {
    ...
    func search(params: DurarContactsSearchParams) async throws -> DurarContactsSearchPayload {
        ...
        return DurarContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: DurarContactsAddParams) async throws -> DurarContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return DurarContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return DurarContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> DurarContactPayload {
        DurarContactPayload(
