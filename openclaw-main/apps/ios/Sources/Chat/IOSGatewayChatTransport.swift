...
struct IOSGatewayChatTransport: DurarChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.Durar", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> DurarChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(DurarChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> DurarChatHistoryPayload {
        ...
        return try JSONDecoder().decode(DurarChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [DurarChatAttachmentPayload]) async throws -> DurarChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [DurarChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(DurarChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(DurarGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DurarGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DurarChatEventPayload.self)
                        {
