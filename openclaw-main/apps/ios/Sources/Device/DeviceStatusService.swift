...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> DurarDeviceStatusPayload {
        ...
        return DurarDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> DurarBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: DurarBatteryState = switch device.batteryState {
        ...
        return DurarBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> DurarThermalStatusPayload {
        let state: DurarThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return DurarThermalStatusPayload(state: state)
    }

    private func storageStatus() -> DurarStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return DurarStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
