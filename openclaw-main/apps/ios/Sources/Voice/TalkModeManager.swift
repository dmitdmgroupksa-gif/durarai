...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__Durar_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<DurarTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.Durar", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = DurarTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> DurarTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return DurarTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> DurarTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> DurarTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return DurarTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> DurarTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = DurarTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: DurarTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: DurarAgentEventPayload.self
            ) else {
                continue
            }
