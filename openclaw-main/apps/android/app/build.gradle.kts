...
val androidStoreFile = providers.gradleProperty("Durar_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("Durar_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("Durar_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("Durar_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set Durar_ANDROID_STORE_FILE, " +
            "Durar_ANDROID_STORE_PASSWORD, Durar_ANDROID_KEY_ALIAS, and " +
            "Durar_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/DurarKit/Sources/DurarKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.Durar.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "Durar_ENABLE_SMS", "false")
            buildConfigField("boolean", "Durar_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "Durar_ENABLE_SMS", "true")
            buildConfigField("boolean", "Durar_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "Durar-$versionName-$buildType.apk"
                    } else {
                        "Durar-$versionName-$flavorName-$buildType.apk"
                    }
