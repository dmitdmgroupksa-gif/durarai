...
internal fun DurarAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Durar Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `Durar devices list` should show a pending pairing request
    - if more signal is needed, ask for `Durar qr --json`, `Durar devices list`, and `Durar nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${DurarAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Durar gateway diagnostics", report))
