...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> DurarChatHistoryPayload
{
    DurarChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> DurarChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> DurarChatSessionEntry
{
    DurarChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> DurarChatModelChoice {
    DurarChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [DurarChatHistoryPayload],
    sessionsResponses: [DurarChatSessionsListResponse] = [],
    modelResponses: [[DurarChatModelChoice]] = [],
    ...
    -> (TestChatTransport, DurarChatViewModel)
{
...
private func sendUserMessage(_ vm: DurarChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, DurarChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [DurarChatHistoryPayload]
    private let sessionsResponses: [DurarChatSessionsListResponse]
    private let modelResponses: [[DurarChatModelChoice]]
    ...
    private let stream: AsyncStream<DurarChatTransportEvent>
    private let continuation: AsyncStream<DurarChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<DurarChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> DurarChatHistoryPayload {
        ...
        return self.historyResponses.last ?? DurarChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [DurarChatAttachmentPayload]) async throws -> DurarChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return DurarChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> DurarChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? DurarChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: DurarChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = DurarChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = DurarChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: DurarChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = DurarChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: DurarChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(DurarChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == DurarChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = DurarChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: DurarChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = DurarChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = DurarChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = DurarChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { DurarChatViewModel(sessionKey: "main", transport: transport) }
