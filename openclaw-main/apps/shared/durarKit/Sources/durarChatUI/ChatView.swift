...
@MainActor
public struct DurarChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: DurarChatViewModel
    ...
    private var visibleMessages: [DurarChatMessage] {
        let base: [DurarChatMessage]
    ...
    private func mergeToolResults(in messages: [DurarChatMessage]) -> [DurarChatMessage] {
    ...
    private func isToolResultMessage(_ message: DurarChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: DurarChatMessage) -> Bool {
    ...
    private func primaryText(in message: DurarChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: DurarChatMessage) -> Bool {
    ...
    private func toolCalls(in message: DurarChatMessage) -> [DurarChatMessageContent] {
    ...
    private func inlineToolResults(in message: DurarChatMessage) -> [DurarChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: DurarChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: DurarChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(DurarChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
