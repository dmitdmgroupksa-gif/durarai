...
private let chatUILogger = Logger(subsystem: "ai.Durar", category: "DurarChatUI")

@MainActor
@Observable
public final class DurarChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [DurarChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [DurarChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [DurarPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [DurarChatPendingToolCall] = []
    public private(set) var sessions: [DurarChatSessionEntry] = []
    private let transport: any DurarChatTransport
    private var sessionDefaults: DurarChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: DurarChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any DurarChatTransport,
    ...
    public var sessionChoices: [DurarChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [DurarChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: DurarPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [DurarChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: DurarChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: DurarChatMessage) -> DurarChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> DurarChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return DurarChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: DurarChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: DurarChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: DurarChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [DurarChatMessage],
        incoming: [DurarChatMessage]) -> [DurarChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [DurarChatMessage]) -> [DurarChatMessage] {
        var result: [DurarChatMessage] = []
    ...
    private static func dedupeKey(for message: DurarChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [DurarChatMessageContent] = [
            DurarChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> DurarChatAttachmentPayload in
            DurarChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> DurarChatSessionEntry {
        DurarChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = DurarChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: DurarChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: DurarChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: DurarAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = DurarChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> DurarPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
