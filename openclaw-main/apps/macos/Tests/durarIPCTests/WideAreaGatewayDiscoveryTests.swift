...
import Foundation
import Testing
@testable import DurarDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("Durar_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("Durar_WIDE_AREA_DOMAIN", "Durar.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("Durar_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("Durar_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._Durar-gw._tcp.Durar.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.Durar.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Durar)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/Durar/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Durar)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/Durar/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._Durar-gw._tcp.Durar.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
