...
struct DurarMacCLI {
    static func main() async {
        ...
        default:
            fputs("Durar-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      Durar-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      Durar-mac discover [--timeout <ms>] [--json] [--include-local]
      Durar-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      Durar-mac connect
      Durar-mac connect --url ws://127.0.0.1:18789 --json
      Durar-mac discover --timeout 3000 --json
      Durar-mac wizard --mode local
    """)
}
