...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.Durar/Durar.json. Prefer Durar_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.Durar/Durar.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.Durar/Durar.json. Restart the gateway to apply."
        }
