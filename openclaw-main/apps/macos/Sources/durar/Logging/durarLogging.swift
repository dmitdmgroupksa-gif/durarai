...
enum DurarLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = DurarOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = DurarFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        DurarLogging.bootstrapIfNeeded()
        let label = DurarLogging.makeLabel(subsystem: subsystem, category: category)
...
struct DurarOSLogHandler: AppLogLevelBackedHandler {
...
struct DurarFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = DurarLogging.parseLabel(self.label)
