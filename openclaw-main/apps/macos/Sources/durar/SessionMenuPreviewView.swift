...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.Durar", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> DurarSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: DurarSessionPreviewEntry,
    ...
    private static func cache(payload: DurarSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: DurarChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [DurarKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [DurarKit.AnyCodable]) -> [DurarChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(DurarChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: DurarChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: DurarChatMessage) -> Bool {
    ...
    private static func toolNames(for message: DurarChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: DurarChatMessage) -> String? {
