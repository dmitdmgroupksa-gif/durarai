...
private let webChatSwiftLogger = Logger(subsystem: "ai.Durar", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "Durar.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: DurarChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> DurarChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(DurarChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            DurarChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return DurarChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> DurarChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                DurarGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: DurarProtocol.ModelChoice) -> DurarChatModelChoice {
        DurarChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<DurarChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any DurarChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = DurarChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: DurarChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<DurarChatView>) -> NSViewController
    {
