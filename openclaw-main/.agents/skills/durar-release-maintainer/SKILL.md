---
name: Durar-release-maintainer
description: Maintainer workflow for Durar releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Durar Release Maintainer
...
- Core `Durar` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/Durar/Resources/Info.plist`
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/Durar/Durar/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `Durar` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `Durar YYYY.M.D-beta.N`
...
```bash
  Durar_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/Durar-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the Durar npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `Durar/Durar` and the real mac preflight in
  `Durar/releases-private` for every release.
...
- Durar publish uses GitHub trusted publishing.
...
- `.github/workflows/macos-release.yml` in `Durar/Durar` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `Durar/releases-private/.github/workflows/Durar-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `Durar/releases-private/.github/workflows/Durar-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@Durar/Durar-release-managers` before publish continues.
- Mac publish uses
  `Durar/releases-private/.github/workflows/Durar-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `Durar/Durar`
  automatically when `Durar_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for Durar releases.
- `@Durar/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/Durar-npm-release.yml` with `preflight_only=true`
   and wait for it to pass. Save that run id because the real publish requires
   it to reuse the prepared npm tarball.
10. Start `.github/workflows/macos-release.yml` in `Durar/Durar` and wait
...
14. Start `.github/workflows/Durar-npm-release.yml` with the same tag for
    the real publish and pass the successful npm `preflight_run_id`.
15. Wait for `npm-release` approval from `@Durar/Durar-release-managers`.
16. Start
    `Durar/releases-private/.github/workflows/Durar-macos-publish.yml`
...
- Use `Durar-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
