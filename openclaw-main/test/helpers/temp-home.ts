...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked Durar_HOME.
  delete process.env.Durar_HOME;
  process.env.Durar_STATE_DIR = path.join(base, ".Durar");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "Durar-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".Durar", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
