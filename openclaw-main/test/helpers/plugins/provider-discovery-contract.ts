...
function installDiscoveryHooks(state: DiscoveryState) {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("Durar/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("Durar/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("Durar/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
