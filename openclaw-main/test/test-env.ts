...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.Durar_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "Durar_TEST_FAST", value: process.env.Durar_TEST_FAST },
    ...
    { key: "Durar_STATE_DIR", value: process.env.Durar_STATE_DIR },
    { key: "Durar_CONFIG_PATH", value: process.env.Durar_CONFIG_PATH },
    { key: "Durar_GATEWAY_PORT", value: process.env.Durar_GATEWAY_PORT },
    { key: "Durar_BRIDGE_ENABLED", value: process.env.Durar_BRIDGE_ENABLED },
    { key: "Durar_BRIDGE_HOST", value: process.env.Durar_BRIDGE_HOST },
    { key: "Durar_BRIDGE_PORT", value: process.env.Durar_BRIDGE_PORT },
    { key: "Durar_CANVAS_HOST_PORT", value: process.env.Durar_CANVAS_HOST_PORT },
    { key: "Durar_TEST_HOME", value: process.env.Durar_TEST_HOME },
    { key: "Durar_AGENT_DIR", value: process.env.Durar_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.Durar_TEST_HOME = tempHome;
  process.env.Durar_TEST_FAST = "1";
  ...
  delete process.env.Durar_BRIDGE_ENABLED;
  delete process.env.Durar_BRIDGE_HOST;
  delete process.env.Durar_BRIDGE_PORT;
  delete process.env.Durar_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.Durar_STATE_DIR = path.join(tempHome, ".Durar");
  }
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.Durar_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".Durar");
  const priorIsolatedHome = params.env.Durar_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".Durar")
  ) {
    realStateDir = path.join(params.realHome, ".Durar");
  }
  const tempStateDir = path.join(params.tempHome, ".Durar");
  fs.mkdirSync(tempStateDir, { recursive: true });

  const realConfigPath = params.env.Durar_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.Durar_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "Durar.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "Durar.json"),
...
export function installTestEnv(): { cleanup: () => void; tempHome: string } {
  const live =
    process.env.LIVE === "1" ||
    process.env.Durar_LIVE_TEST === "1" ||
    process.env.Durar_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.Durar_LIVE_USE_REAL_HOME);
