...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("passes through the filesystem module cache path when provided", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          Durar_VITEST_FS_MODULE_CACHE_PATH: "/tmp/Durar-vitest-cache",
        },
        "linux",
      ),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
        fsModuleCachePath: "/tmp/Durar-vitest-cache",
      },
    });
  });
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          Durar_VITEST_IMPORT_DURATIONS: "true",
          Durar_VITEST_PRINT_IMPORT_BREAKDOWN: "1",
        },
        "linux",
      ),
    ).toEqual({
