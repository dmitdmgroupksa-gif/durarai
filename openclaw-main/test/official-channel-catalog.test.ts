...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("Durar-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@Durar/whatsapp",
      version: "2026.3.23",
      description: "Durar WhatsApp channel plugin",
      Durar: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("Durar-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@Durar/whatsapp",
      Durar: {
        channel: {
