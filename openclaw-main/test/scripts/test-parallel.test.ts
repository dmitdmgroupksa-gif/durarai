...
const clearPlannerShardEnv = (env) => {
  const nextEnv = { ...env };
  delete nextEnv.Durar_TEST_SHARDS;
  delete nextEnv.Durar_TEST_SHARD_INDEX;
  delete nextEnv.Durar_TEST_FORCE_THREADS;
  delete nextEnv.Durar_TEST_FORCE_FORKS;
  delete nextEnv.Durar_TEST_DISABLE_THREAD_EXPANSION;
  delete nextEnv.Durar_TEST_SHOW_POOL_DECISION;
  delete nextEnv.Durar_TEST_PROFILE;
  delete nextEnv.Durar_TEST_WORKERS;
  delete nextEnv.Durar_TEST_SKIP_DEFAULT;
  delete nextEnv.Durar_TEST_INCLUDE_EXTENSIONS;
  delete nextEnv.Durar_TEST_INCLUDE_CHANNELS;
  delete nextEnv.Durar_TEST_INCLUDE_GATEWAY;
  delete nextEnv.Durar_TEST_UNIT_FAST_BATCH_TARGET_MS;
  return nextEnv;
};
...
const HIGH_MEMORY_LOCAL_PLANNER_ENV = {
  RUNNER_OS: "macOS",
  Durar_TEST_HOST_CPU_COUNT: "12",
  Durar_TEST_HOST_MEMORY_GIB: "128",
} as const;
...
function runManifestOutputWriter(workflow: string, envOverrides: NodeJS.ProcessEnv = {}): string {
  const outputPath = path.join(os.tmpdir(), `Durar-${workflow}-output-${Date.now()}.txt`);
  try {
...
describe("scripts/test-parallel lane planning", () => {
  ...
  it("recycles default local unit-fast runs into bounded batches", () => {
    const output = runPlannerPlan(["--plan"], {
      CI: "",
      Durar_TEST_UNIT_FAST_LANES: "1",
      Durar_TEST_UNIT_FAST_BATCH_TARGET_MS: "1",
    });
  ...
  it("reports capability-derived output for mid-memory local macOS hosts", () => {
    const output = runPlannerPlan(
      ...
      createLocalPlannerEnv({
        RUNNER_OS: "macOS",
        Durar_TEST_HOST_CPU_COUNT: "10",
        Durar_TEST_HOST_MEMORY_GIB: "64",
      }),
    );
  ...
  it("prints the planner-backed CI manifest as JSON", () => {
    const output = runPlannerPlan(["--ci-manifest"], {
      ...
      Durar_CI_DOCS_ONLY: "false",
      Durar_CI_DOCS_CHANGED: "false",
      Durar_CI_RUN_NODE: "true",
      Durar_CI_RUN_MACOS: "true",
      Durar_CI_RUN_ANDROID: "false",
      Durar_CI_RUN_WINDOWS: "true",
      Durar_CI_RUN_SKILLS_PYTHON: "false",
      Durar_CI_HAS_CHANGED_EXTENSIONS: "false",
      Durar_CI_CHANGED_EXTENSIONS_MATRIX: '{"include":[]}',
    });
  ...
  it("writes install-smoke outputs in install-smoke mode", () => {
    const outputs = runManifestOutputWriter("install-smoke", {
      Durar_CI_DOCS_ONLY: "false",
      Durar_CI_RUN_CHANGED_SMOKE: "true",
    });
  ...
  it("rejects explicit existing files that are not known test files", () => {
    const tempFilePath = path.join(os.tmpdir(), `Durar-non-test-${Date.now()}.ts`);
