...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://Durar.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://Durar.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://Durar.ai/abc-123", baseHref)).toBe(
      "blob:https://Durar.ai/abc-123",
    );
  });
