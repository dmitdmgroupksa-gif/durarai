...
export async function handleWhatsAppStart(host: DurarApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: DurarApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: DurarApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: DurarApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: DurarApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: DurarApp): string {
...
function resolveGatewayHttpAuthHeader(host: DurarApp): string | null {
...
function buildGatewayHttpHeaders(host: DurarApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: DurarApp) {
...
export function handleNostrProfileToggleAdvanced(host: DurarApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: DurarApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: DurarApp) {
  const state = host.nostrProfileFormState;
