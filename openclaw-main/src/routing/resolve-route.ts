...
function listAgents(cfg: DurarConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: DurarConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<DurarConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: DurarConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: DurarConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: DurarConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<DurarConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  DurarConfig,
  {
    bindingsRef: DurarConfig["bindings"];
    agentsRef: DurarConfig["agents"];
    sessionRef: DurarConfig["session"];
...
function resolveRouteCacheForConfig(cfg: DurarConfig): Map<string, ResolvedAgentRoute> {
