...
const TEMP_MEDIA_PREFIX = "Durar-media-";
...
function createGroqAudioConfig(): DurarConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): DurarConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): DurarConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: DurarConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredDurarTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        Durar_AGENT_DIR: isolatedAgentDir,
