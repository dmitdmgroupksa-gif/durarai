...
export function triggerDurarRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.Durar_SYSTEMD_UNIT,
      process.env.Durar_PROFILE,
    );
  ...
  const label =
    process.env.Durar_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.Durar_PROFILE);
