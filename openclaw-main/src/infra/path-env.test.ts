...
import { ensureDurarCliOnPath } from "./path-env.js";
...
describe("ensureDurarCliOnPath", () => {
  const envKeys = [
    "PATH",
    "Durar_PATH_BOOTSTRAPPED",
    "Durar_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/Durar-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "Durar");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureDurarCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.Durar_PATH_BOOTSTRAPPED;
    delete process.env.Durar_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling Durar exists", () => {
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "Durar");
      ...
      if (envValue === undefined) {
        ...
        process.env.Durar_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/Durar-path/case-linuxbrew");
