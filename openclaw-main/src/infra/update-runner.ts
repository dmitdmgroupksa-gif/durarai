...
const DEFAULT_PACKAGE_NAME = "Durar";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["Durar", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "Durar.mjs");
    ...
    const doctorStep = await runStep(
      step("Durar doctor", doctorArgv, gitRoot, { Durar_UPDATE_IN_PROGRESS: "1" }),
    );
