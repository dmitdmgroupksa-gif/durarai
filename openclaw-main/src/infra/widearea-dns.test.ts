...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "Durar.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Durar)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "Durar.internal", expected: "Durar.internal." },
    { value: "Durar.internal.", expected: "Durar.internal." },
    { value: "  Durar.internal  ", expected: "Durar.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { Durar_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { Durar_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("Durar.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "Durar.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderZoneText({
      ...
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/Durar",
    });

    expectZoneRecords(txt, [
      `$ORIGIN Durar.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_Durar-gw._tcp IN PTR studio-london._Durar-gw._tcp`,
      `studio-london._Durar-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Durar)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/Durar`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN Durar.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._Durar-gw._tcp IN TXT`,
        `displayName=Mac Studio (Durar)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("Durar.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("Durar.internal."),
