 ...
 * returning control to the caller (typically `triggerDurarRestart` →
...
/**
 * Parse Durar gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("Durar")) {
 ...
 * Returns only PIDs that belong to Durar gateway processes (not the current process).
 */
