...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __DurarDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__DurarDiagnosticEventsState) {
    globalStore.__DurarDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__DurarDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: DurarConfig): boolean {
