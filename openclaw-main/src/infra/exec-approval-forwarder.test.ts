...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): DurarConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): DurarConfig {
  return {
