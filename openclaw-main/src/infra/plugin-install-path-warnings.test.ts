...
const MATRIX_REPO_INSTALL_COMMAND = `Durar plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/Durar-matrix-missing",
        installPath: "/tmp/Durar-matrix-missing",
      },
    });
    ...
    expect(
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/Durar-matrix-missing",
      'Reinstall with "Durar plugins install @Durar/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/Durar-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<Durar plugins install @Durar/matrix>" when you want to return to the standard Matrix plugin.',
