...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: DurarConfig | null = null;
  ...
  get(key: string, cfg: DurarConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: DurarConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: DurarConfig): void {
