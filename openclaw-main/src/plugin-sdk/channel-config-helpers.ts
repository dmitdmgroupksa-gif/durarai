...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`Durar pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`Durar pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends DurarConfig = DurarConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends DurarConfig = DurarConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends DurarConfig = DurarConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: DurarConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends DurarConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends DurarConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends DurarConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
