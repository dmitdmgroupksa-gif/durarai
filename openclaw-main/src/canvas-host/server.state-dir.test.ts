...
describe("canvas host state dir defaults", () => {
  ...
  it("uses Durar_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("Durar-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Durar Canvas");
      } finally {
        await handler.close();
      }
    });
