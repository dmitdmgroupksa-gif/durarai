...
import {
  ...
  Durar_NODE_EXTRA_CA_CERTS_READY,
  Durar_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(plan?.env[Durar_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(plan?.env[Durar_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "Durar", "gateway", "run"],
        env: {
          [Durar_NODE_EXTRA_CA_CERTS_READY]: "1",
          [Durar_NODE_OPTIONS_READY]: "1",
        },
