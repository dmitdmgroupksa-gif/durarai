...
const Durar_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of Durar_COMPILED_SECRET_TARGETS) {
...
const Durar_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: Durar_COMPILED_SECRET_TARGETS,
    entriesById: Durar_TARGETS_BY_ID,
  });
