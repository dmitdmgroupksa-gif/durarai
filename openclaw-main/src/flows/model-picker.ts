...
export type PromptDefaultModelResult = { model?: string; config?: DurarConfig };
...
function resolveConfiguredModelRaw(cfg: DurarConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: DurarConfig): string[] {
...
export function applyModelAllowlist(cfg: DurarConfig, models: string[]): DurarConfig {
