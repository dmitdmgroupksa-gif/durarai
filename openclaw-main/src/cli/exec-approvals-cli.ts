...
import { readBestEffortConfig, type DurarConfig } from "../config/config.js";
import {
...
export function registerExecApprovalsCli(program: Command) {
  ...
  const approvals = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.Durar.ai/cli/approvals")}\n`,
    );
