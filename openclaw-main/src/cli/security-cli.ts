...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Durar security audit", "Run a local security audit."],
          ["Durar security audit --deep", "Include best-effort live Gateway probe checks."],
          ["Durar security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "Durar security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["Durar security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["Durar security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.Durar.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Durar security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("Durar security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("Durar security audit --fix")}`));
        if (!fixResult) {
