    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Durar nodes status", "List known nodes with live status."],
          ["Durar nodes pairing pending", "Show pending node pairing requests."],
          [
            'Durar nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["Durar nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.Durar.ai/cli/nodes")}\n`,
    );
