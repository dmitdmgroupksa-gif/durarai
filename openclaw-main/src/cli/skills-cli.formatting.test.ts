...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-bundled-skills-test-"));
    process.env.Durar_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
