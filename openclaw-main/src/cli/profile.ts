...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.Durar${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.Durar_PROFILE = profile;

  const stateDir = env.Durar_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.Durar_STATE_DIR?.trim()) {
    env.Durar_STATE_DIR = stateDir;
  }

  if (!env.Durar_CONFIG_PATH?.trim()) {
    env.Durar_CONFIG_PATH = path.join(stateDir, "Durar.json");
  }

  if (profile === "dev" && !env.Durar_GATEWAY_PORT?.trim()) {
    env.Durar_GATEWAY_PORT = "19001";
  }
}
