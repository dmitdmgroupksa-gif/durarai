...
const STEP_LABELS: Record<string, string> = {
  ...
  "Durar doctor entry": "Checking doctor entrypoint",
  "Durar doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g Durar@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g Durar@latest --omit=optional");
  }
