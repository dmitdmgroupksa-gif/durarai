...
${formatHelpExamples([
  ['Durar agent --to +15555550123 --message "status update"', "Start a new session."],
  ['Durar agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['Durar agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.Durar.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.Durar.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['Durar agents set-identity --agent main --name "Durar" --emoji "🦞"', "Set name + emoji."],
  ["Durar agents set-identity --agent main --avatar avatars/Durar.png", "Set avatar path."],
  [
    "Durar agents set-identity --workspace ~/.Durar/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "Durar agents set-identity --identity-file ~/.Durar/workspace/IDENTITY.md --agent main",
