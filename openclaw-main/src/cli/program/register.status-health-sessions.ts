...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Durar status", "Show channel health + session summary."],
          ["Durar status --all", "Full diagnosis (read-only)."],
          ["Durar status --json", "Machine-readable output."],
          ["Durar status --usage", "Show model provider usage/quota snapshots."],
          [
            "Durar status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["Durar status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.Durar.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.Durar.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Durar sessions", "List all sessions."],
          ["Durar sessions --agent work", "List sessions for one agent."],
          ["Durar sessions --all-agents", "Aggregate sessions across agents."],
          ["Durar sessions --active 120", "Only last 2 hours."],
          ["Durar sessions --json", "Machine-readable output."],
          ["Durar sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.Durar.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Durar sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "Durar sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["Durar sessions cleanup --enforce", "Apply maintenance now."],
          ["Durar sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["Durar sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "Durar sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  const flowsCmd = program
    .command("flows")
    .description("Inspect durable TaskFlow state (alias for `Durar tasks flow`)")
    .option("--json", "Output as JSON", false)
    .option(
      "--status <name>",
      "Filter by status (queued, running, waiting, blocked, succeeded, failed, cancelled, lost)",
