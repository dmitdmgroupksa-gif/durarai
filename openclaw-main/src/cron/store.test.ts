...
const { makeStorePath } = createCronStoreHarness({ prefix: "Durar-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses Durar_HOME for tilde expansion", () => {
    vi.stubEnv("Durar_HOME", "/srv/Durar-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/Durar-home", "cron", "jobs.json"));
  });
});
