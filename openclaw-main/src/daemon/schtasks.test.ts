...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when Durar_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".Durar", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when Durar_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", Durar_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".Durar-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers Durar_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        Durar_PROFILE: "rescue",
        Durar_STATE_DIR: "C:\\State\\Durar",
      },
      expected: path.join("C:\\State\\Durar", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", Durar_PROFILE: "default" },
      expected: path.join("/home/test", ".Durar", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Durar Gateway",
          "cd /d C:\\Projects\\Durar",
          "set NODE_ENV=production",
          "set Durar_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\Durar",
          environment: {
            NODE_ENV: "production",
            Durar_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\Durar\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\Durar\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Durar Share\\node.exe" "\\\\fileserver\\Durar Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Durar Share\\node.exe",
            "\\\\fileserver\\Durar Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from Durar_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ Durar_STATE_DIR: path.join(tmpDir, "custom-state") }),
