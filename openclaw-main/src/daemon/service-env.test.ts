...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.Durar_GATEWAY_PORT).toBe("18789");
    expect(env.Durar_GATEWAY_TOKEN).toBeUndefined();
    expect(env.Durar_SERVICE_MARKER).toBe("Durar");
    expect(env.Durar_SERVICE_KIND).toBe("gateway");
    expect(typeof env.Durar_SERVICE_VERSION).toBe("string");
    expect(env.Durar_SYSTEMD_UNIT).toBe("Durar-gateway.service");
    expect(env.Durar_WINDOWS_TASK_NAME).toBe("Durar Gateway");
    if (process.platform === "darwin") {
      expect(env.Durar_LAUNCHD_LABEL).toBe("ai.Durar.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", Durar_PROFILE: "work" },
      port: 18789,
    });
    expect(env.Durar_SYSTEMD_UNIT).toBe("Durar-gateway-work.service");
    expect(env.Durar_WINDOWS_TASK_NAME).toBe("Durar Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.Durar_LAUNCHD_LABEL).toBe("ai.Durar.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through Durar_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", Durar_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.Durar_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits Durar_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        Durar_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".Durar"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", Durar_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".Durar-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", Durar_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".Durar"));
  });

  it("uses Durar_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", Durar_STATE_DIR: "/var/lib/Durar" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/Durar"));
  });

  it("expands ~ in Durar_STATE_DIR", () => {
    const env = { HOME: "/Users/test", Durar_STATE_DIR: "~/Durar-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/Durar-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { Durar_STATE_DIR: "C:\\State\\Durar" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\Durar");
  });
});
