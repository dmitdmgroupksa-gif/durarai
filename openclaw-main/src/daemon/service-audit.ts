...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds Durar_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `Durar gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service Durar_GATEWAY_TOKEN does not match gateway.auth.token in Durar.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.Durar_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.Durar_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `Durar gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
