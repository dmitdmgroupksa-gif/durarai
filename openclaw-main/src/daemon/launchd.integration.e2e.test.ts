...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `Durar-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      Durar_LAUNCHD_LABEL: `ai.Durar.launchd-int-${testId}`,
      Durar_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
