...
const GOOGLE_LIVE_SINGLE_KEY = "Durar_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "Durar_LIVE_ANTHROPIC_KEY",
    listVar: "Durar_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "Durar_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `Durar_LIVE_${base}_KEY`;
