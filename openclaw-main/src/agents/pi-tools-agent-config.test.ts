...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: DurarConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<DurarConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<DurarConfig["agents"]>["list"]>[number]["tools"];
  }): DurarConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createDurarCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createDurarCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createDurarCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createDurarCodingTools({
      config: cfg,
    ...
    const bobTools = createDurarCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createDurarCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createDurarCodingTools({
      config: cfg,
