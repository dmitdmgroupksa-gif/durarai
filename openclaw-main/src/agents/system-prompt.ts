...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Durar handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Durar docs: ${docsPath}`,
    "Mirror: https://docs.Durar.ai",
    "Source: https://github.com/dmitdmgroupksa-gif/durarai",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For Durar behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `Durar status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Durar process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Durar agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Durar agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Durar.";
  }

  const lines = [
    "You are a personal assistant running inside Durar.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Durar's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## Durar CLI Quick Reference",
    "Durar is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- Durar gateway status",
    "- Durar gateway start",
    "- Durar gateway stop",
    "- Durar gateway restart",
    "If unsure, ask the user to run `Durar help` (or `Durar gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Durar Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Durar pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Durar and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Durar treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
