...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseDurarManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyDurarManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveDurarMetadata(
  frontmatter: ParsedSkillFrontmatter,
): DurarSkillMetadata | undefined {
  const metadataObj = resolveDurarManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveDurarManifestRequires(metadataObj);
  const install = resolveDurarManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveDurarManifestOs(metadataObj);
  return {
