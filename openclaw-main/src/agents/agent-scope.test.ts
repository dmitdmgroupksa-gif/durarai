...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: DurarConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: DurarConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: DurarConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: DurarConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: DurarConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: DurarConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/Durar");
  });

  it("uses Durar_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "Durar-home");
    vi.stubEnv("Durar_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as DurarConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".Durar", "workspace"));
  });

  it("uses Durar_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "Durar-home");
    vi.stubEnv("Durar_HOME", home);
    // Clear state dir so it falls back to Durar_HOME
    vi.stubEnv("Durar_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as DurarConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".Durar", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/Durar-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/Durar-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-agent-scope-"));
