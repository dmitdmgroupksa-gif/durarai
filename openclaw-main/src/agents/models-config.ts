...
import { planDurarModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  config: DurarConfig;
  sourceConfigForSecrets: DurarConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: DurarConfig): {
  config: DurarConfig;
  sourceConfigForSecrets: DurarConfig;
} {
...
export async function ensureDurarModelsJson(
  config?: DurarConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveDurarAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planDurarModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
