...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["Durar_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-subagent-"));
  ...
  it("uses isolated temp state when Durar_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "Durar-test-state"));
  });
});
