...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.Durar_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".Durar", `workspace-${profile}`);
  }
  return path.join(home, ".Durar", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".Durar";
