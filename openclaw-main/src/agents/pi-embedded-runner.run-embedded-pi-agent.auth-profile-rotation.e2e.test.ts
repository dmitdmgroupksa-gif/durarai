...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureDurarModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): DurarConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): DurarConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `Durar-auth-rotation-${Date.now()}.log`),
    });
