...
function resolveAttachmentLimits(config: DurarConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".Durar", "attachments");
  const relDir = path.posix.join(".Durar", "attachments", attachmentId);
