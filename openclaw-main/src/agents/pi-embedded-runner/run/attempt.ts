...
import { resolveDurarDocsPath } from "../../docs-path.js";
...
import { createDurarCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveDurarAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createDurarCodingTools({
    ...
    const docsPath = await resolveDurarDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/dmitdmgroupksa-gif/durarai/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("Durar:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/dmitdmgroupksa-gif/durarai/issues/8643
