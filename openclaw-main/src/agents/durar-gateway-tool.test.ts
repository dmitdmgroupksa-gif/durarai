...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshDurarToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-test-"));

    try {
      await withEnvAsync(
        { Durar_STATE_DIR: stateDir, Durar_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: Durar --profile isolated doctor --non-interactive",
          );
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/Durar" } } }',
      }),
    ).rejects.toThrow(
