...
let configuredConfig: DurarConfig | undefined;
...
function isLikelyDurarCliProcess(argv: string[] = process.argv): boolean {
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Durar CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureDurarModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyDurarCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): DurarConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureDurarModelsJson(cfg);
    } catch {
