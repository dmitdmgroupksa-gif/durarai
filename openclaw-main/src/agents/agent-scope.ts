...
type AgentEntry = NonNullable<NonNullable<DurarConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: DurarConfig): AgentEntry[] {
...
export function listAgentIds(cfg: DurarConfig): string[] {
...
export function resolveDefaultAgentId(cfg: DurarConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: DurarConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: DurarConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: DurarConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
