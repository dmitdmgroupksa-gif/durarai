...
function mockContextDeps(params: {
  ...
  const ensureDurarModelsJson = vi.fn(async () => {});
  vi.doMock("../config/config.js", () => ({
    loadConfig: params.loadConfig,
  }));
  vi.doMock("./models-config.js", () => ({
    ensureDurarModelsJson,
  }));
  vi.doMock("./agent-paths.js", () => ({
    resolveDurarAgentDir: () => "/tmp/Durar-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real Durar startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "Durar", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "Durar", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "Durar", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureDurarModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureDurarModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
