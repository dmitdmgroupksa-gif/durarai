...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "Durar-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    Durar_GATEWAY_TOKEN: "test-token",
    Durar_CONFIG_DIR: join(sandbox.rootDir, "config"),
    Durar_WORKSPACE_DIR: join(sandbox.rootDir, "Durar"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("Durar-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    Durar_GATEWAY_TOKEN: undefined,
    Durar_CONFIG_DIR: configDir,
    Durar_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      Durar_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      Durar_EXTRA_MOUNTS: undefined,
      Durar_HOME_VOLUME: "Durar-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("Durar_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("Durar_EXTRA_MOUNTS=");
    expect(envFile).toContain("Durar_HOME_VOLUME=Durar-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("Durar-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("Durar-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg Durar_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node Durar-gateway dist/index.js onboard --mode local --no-install-daemon",
    );
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node Durar-gateway dist/index.js config set gateway.mode local",
    );
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node Durar-gateway dist/index.js config set gateway.bind lan",
    );
    expect(log).toContain(
      'run --rm --no-deps --entrypoint node Durar-gateway dist/index.js config set gateway.controlUi.allowedOrigins ["http://localhost:18789","http://127.0.0.1:18789"] --strict-json',
    );
    expect(log).not.toContain("run --rm Durar-cli onboard --mode local --no-install-daemon");
  });

  it("avoids shared-network Durar-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(prestartLines.some((line) => /\bcompose\b.*\brun\b.*\bDurar-cli\b/.test(line))).toBe(
      false,
    );
  });
  ...
  it("writes Durar_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      Durar_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("Durar_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh Durar-gateway -c");
  });

  it("reuses existing config token when Durar_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("Durar_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when Durar_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "Durar_GATEWAY_TOKEN=dotenv-token-123\nDurar_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("Durar_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        "Durar_GATEWAY_TOKEN=",
        "Durar_GATEWAY_TOKEN=first-token",
        "Durar_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("Durar_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("Durar_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats Durar_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("Durar_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg Durar_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg Durar_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      Durar_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker Durar-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        Durar_SANDBOX: "1",
        Durar_DOCKER_SOCKET: socketPath,
      ...
      expect(log).toContain(
        "run --rm --no-deps Durar-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate Durar-gateway"));
  ...
  it("rejects injected multiline Durar_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      Durar_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("Durar_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid Durar_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      Durar_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid Durar_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      Durar_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("Durar_HOME_VOLUME must match");
  });

  it("rejects Durar_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("Durar_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:Durar-gateway"');
    expect(compose).toContain("depends_on:\n      - Durar-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/Durar_GATEWAY_TOKEN: \$\{Durar_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{Durar_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
