...
import { z } from "zod";
import { createDurarChannelMcpServer, DurarChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createDurarChannelMcpServer({
...
describe("Durar channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new DurarChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
