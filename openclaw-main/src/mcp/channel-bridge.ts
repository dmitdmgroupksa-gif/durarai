...
export class DurarChannelBridge {
  ...
  async start(): Promise<void> {
    ...
    const gatewayUrlOverrideSource =
      connection.urlSource === "cli --url"
        ? "cli"
        : connection.urlSource === "env Durar_GATEWAY_URL"
          ? "env"
          : undefined;
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`Durar mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `Durar mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
