...
import type { DurarConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: DurarConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: DurarConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Durar default (full)";
  const defaultRequestedAskSource = "Durar default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("Durar approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: DurarConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: DurarConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("Durar security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.Durar/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("Durar approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.Durar.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("Durar configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("Durar config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("Durar doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("Durar config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('Durar config set session.dmScope "per-channel-peer"') +
