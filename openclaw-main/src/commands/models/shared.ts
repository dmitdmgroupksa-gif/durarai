...
export async function loadValidConfigOrThrow(): Promise<DurarConfig> {
...
export async function updateConfig(
  mutator: (cfg: DurarConfig) => DurarConfig,
): Promise<DurarConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: DurarConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: DurarConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("Durar agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
