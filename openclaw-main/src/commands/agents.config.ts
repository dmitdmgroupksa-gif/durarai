...
function resolveAgentName(cfg: DurarConfig, agentId: string) {
...
function resolveAgentModel(cfg: DurarConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DurarConfig): AgentSummary[] {
