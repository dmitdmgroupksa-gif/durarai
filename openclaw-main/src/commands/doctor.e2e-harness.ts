...
export const resolveDurarPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadDurarPlugins: () => createEmptyPluginRegistry(),
}));
...
beforeEach(() => {
  ...
  resolveDurarPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.Durar_STATE_DIR;
  originalUpdateInProgress = process.env.Durar_UPDATE_IN_PROGRESS;
  process.env.Durar_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-doctor-state-"));
