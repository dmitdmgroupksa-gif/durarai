 ...
 * Runs as part of `Durar doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("Durar config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("Durar config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("Durar memory status --deep")}`,
        ]
          .filter(Boolean)
          .join("\n"),
        "Memory search",
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`Durar config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("Durar configure --section model")}`,
        `- To disable: ${formatCliCommand("Durar config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("Durar memory status --deep")}`,
      ].join("\n"),
      "Memory search",
