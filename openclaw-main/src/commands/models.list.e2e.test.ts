...
const ensureDurarModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveDurarAgentDir = vi.fn().mockReturnValue("/tmp/Durar-agent");
...
beforeEach(() => {
  ...
  ensureDurarModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureDurarModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureDurarModelsJson).toHaveBeenCalled();
    expect(ensureDurarModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
