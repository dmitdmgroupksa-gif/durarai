...
import { DurarSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: DurarConfig): {
  config: DurarConfig;
  removed: string[];
} {
  const parsed = DurarSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: DurarConfig): void {
