...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.Durar_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("Durar gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
