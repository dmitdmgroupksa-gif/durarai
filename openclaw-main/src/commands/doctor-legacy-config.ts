...
export function normalizeCompatibilityConfigValues(cfg: DurarConfig): {
  config: DurarConfig;
  changes: string[];
} {
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as DurarConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as DurarConfig["browser"],
    };
    changes.push(
  ...
  const normalizeLegacyNanoBananaSkill = () => {
    type ModelProviderEntry = Partial<
      NonNullable<NonNullable<DurarConfig["models"]>["providers"]>[string]
    >;
    type ModelsConfigPatch = Partial<NonNullable<DurarConfig["models"]>>;
    ...
    if (!hasGoogleApiKey && legacyApiKey) {
      ...
      rawModels.providers = rawProviders as NonNullable<DurarConfig["models"]>["providers"];
      next = {
        ...next,
        models: rawModels as DurarConfig["models"],
      };
      changes.push(
  ...
  const normalizeLegacyTalkConfig = () => {
    ...
    const normalizedTalk = normalizeTalkSection(rawTalk as DurarConfig["talk"]);
    if (!normalizedTalk) {
      return;
    }
  ...
  const normalizeLegacyMediaProviderOptions = () => {
    ...
    next = {
      ...
      tools: {
        ...next.tools,
        media: nextMedia as NonNullable<DurarConfig["tools"]>["media"],
      },
    };
  };
  ...
  const normalizeLegacyMistralModelMaxTokens = () => {
    ...
    next = {
      ...
      models: {
        ...next.models,
        providers: nextProviders as NonNullable<DurarConfig["models"]>["providers"],
      },
    };
  };
