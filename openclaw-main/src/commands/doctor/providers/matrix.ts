...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "Durar doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "Durar doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: DurarConfig): Promise<string[]> {
...
function hasConfiguredMatrixChannel(cfg: DurarConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: DurarConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: DurarConfig, env: NodeJS.ProcessEnv): boolean {
  return (
