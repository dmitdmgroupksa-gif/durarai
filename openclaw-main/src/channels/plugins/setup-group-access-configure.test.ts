...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: DurarConfig, policy: ChannelAccessPolicy) => DurarConfig;
  resolveAllowlist: (params: { cfg: DurarConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: DurarConfig; resolved: TResolved }) => DurarConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: DurarConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: DurarConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: DurarConfig, policy: ChannelAccessPolicy): DurarConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: DurarConfig, policy: ChannelAccessPolicy): DurarConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: DurarConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: DurarConfig; resolved: string[] }) => {
      calls.push("apply");
