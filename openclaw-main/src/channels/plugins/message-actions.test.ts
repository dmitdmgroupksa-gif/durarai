...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as DurarConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as DurarConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as DurarConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as DurarConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as DurarConfig, channel: "demo-buttons" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as DurarConfig, channel: "demo-cards" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as DurarConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as DurarConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as DurarConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as DurarConfig,
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as DurarConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as DurarConfig)).toEqual([]);
