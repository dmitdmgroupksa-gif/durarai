  ...
  it.each([
    {
      ...
      setup: () => {
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-catalog-"));
        const catalogPath = path.join(dir, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@Durar/demo-channel",
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-channel-catalog-state-"));
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-catalog-state-"));
  ...
  it.each([
    {
      ...
      setup: () => {
        const home = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-catalog-home-"));
        const catalogPath = path.join(home, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@Durar/env-demo-channel",
        ...
        return {
          env: {
            ...process.env,
            Durar_PLUGIN_CATALOG_PATHS: "~/catalog.json",
            Durar_HOME: home,
            HOME: home,
          },
    ...
    },
    {
      ...
      setup: () => {
        ...
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@Durar/default-env-demo",
