...
type SetupContractEntry = {
  ...
  cases: Array<{
    ...
    beforeTest?: () => void;
    assertPatchedConfig?: (cfg: DurarConfig) => void;
    assertResolvedAccount?: (account: unknown, cfg: DurarConfig) => void;
  }>;
};
...
setBundledChannelRuntime("line", {
  channel: {
    line: {
      listLineAccountIds,
      resolveDefaultLineAccountId,
      resolveLineAccount: ({ cfg, accountId }: { cfg: DurarConfig; accountId?: string }) =>
...
const matrixSessionBindingStateDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "Durar-matrix-session-binding-contract-"),
);
