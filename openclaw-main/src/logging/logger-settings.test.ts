...
let originalDurarLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.Durar_TEST_FILE_LOG;
  originalDurarLogLevel = process.env.Durar_LOG_LEVEL;
  delete process.env.Durar_TEST_FILE_LOG;
  delete process.env.Durar_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.Durar_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalDurarLogLevel === undefined) {
    delete process.env.Durar_LOG_LEVEL;
  } else {
    process.env.Durar_LOG_LEVEL = originalDurarLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
