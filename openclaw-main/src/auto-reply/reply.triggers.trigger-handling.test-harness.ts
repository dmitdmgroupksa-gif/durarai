...
const piEmbeddedMocks = getSharedMocks("Durar.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("Durar.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("Durar.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("Durar.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    DurarHome: process.env.Durar_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("Durar_HOME", snapshot.DurarHome);
  restoreKey("Durar_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.Durar_STATE_DIR = join(home, ".Durar");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "Durar-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".Durar", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): DurarConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "Durar"),
