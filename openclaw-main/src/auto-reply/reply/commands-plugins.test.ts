...
const workspaceHarness = createCommandWorkspaceHarness("Durar-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".Durar", "extensions", params.pluginId);
