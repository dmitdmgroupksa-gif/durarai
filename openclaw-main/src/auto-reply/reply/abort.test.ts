...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop Durar!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@Durar_bot", { botUsername: "Durar_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@Durar_bot", { botUsername: "Durar_bot" })).toBe(true);
