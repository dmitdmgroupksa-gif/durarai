...
import { scheduleGatewaySigusr1Restart, triggerDurarRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Durar in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerDurarRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Durar via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
