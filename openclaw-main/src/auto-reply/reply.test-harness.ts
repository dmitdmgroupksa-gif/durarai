...
type HomeEnvSnapshot = {
  ...
  Durar_STATE_DIR: string | undefined;
  Durar_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    Durar_AGENT_DIR: process.env.Durar_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".Durar", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.Durar_STATE_DIR = path.join(home, ".Durar");
    process.env.Durar_AGENT_DIR = path.join(home, ".Durar", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".Durar", "agent");
