...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set Durar_GATEWAY_TOKEN/Durar_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.Durar_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.Durar_GATEWAY_PASSWORD);
