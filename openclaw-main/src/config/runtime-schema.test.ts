...
import type { ConfigFileSnapshot, DurarConfig } from "./types.js";

const mockLoadConfig = vi.hoisted(() => vi.fn<() => DurarConfig>());
...
function makeSnapshot(params: { valid: boolean; config?: DurarConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/Durar.json",
    exists: true,
    raw: "{}",
