...
import { buildWebSearchProviderConfig, withTempHome, writeDurarConfig } from "./test-helpers.js";
import { DurarSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = DurarSchema.safeParse({
      $schema: "https://Durar.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://Durar.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = DurarSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = DurarSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeDurarConfig(home, {
        gateway: { bind: "${Durar_BIND}" },
      });

      const prev = process.env.Durar_BIND;
      process.env.Durar_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.Durar_BIND;
        } else {
          process.env.Durar_BIND = prev;
        }
      }
    });
  });
