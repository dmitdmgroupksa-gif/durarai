...
import type { DurarConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  Durar config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  Durar config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: DurarConfig,
  pathSegments: string[],
): { changed: boolean; next: DurarConfig } {
...
function coerceConfig(value: unknown): DurarConfig {
...
function stampConfigVersion(cfg: DurarConfig): DurarConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: DurarConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer Durar (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as DurarConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): DurarConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as DurarConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(
    ...
    let cfgToWrite = persistCandidate as DurarConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as DurarConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.Durar_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.Durar_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.Durar_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.Durar_WATCH_SESSION === "string" &&
        deps.env.Durar_WATCH_SESSION.trim().length > 0
          ? deps.env.Durar_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.Durar_WATCH_COMMAND === "string" &&
        deps.env.Durar_WATCH_COMMAND.trim().length > 0
          ? deps.env.Durar_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `Durar_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: DurarConfig | null = null;
let runtimeConfigSourceSnapshot: DurarConfig | null = null;
...
export function getRuntimeConfigSourceSnapshot(): DurarConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: DurarConfig;
  candidate: DurarConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: DurarConfig): DurarConfig {
...
export function loadConfig(): DurarConfig {
  ...
  // reparsing Durar.json on hot code paths.
  setRuntimeConfigSnapshot(config);
  return runtimeConfigSnapshot ?? config;
}

export function getRuntimeConfig(): DurarConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<DurarConfig> {
