...
export function parseDurarVersion(raw: string | null | undefined): DurarVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeDurarVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseDurarVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameDurarStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseDurarVersion(a);
  const parsedB = parseDurarVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareDurarVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseDurarVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseDurarVersion(current);
  const parsedTouched = parseDurarVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameDurarStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareDurarVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: DurarVersion): number {
