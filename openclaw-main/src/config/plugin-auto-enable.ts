...
const ENV_CATALOG_PATHS = ["Durar_PLUGIN_CATALOG_PATHS", "Durar_MPM_CATALOG_PATHS"];
...
function collectModelRefs(cfg: DurarConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: DurarConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: DurarConfig, pluginId: string): boolean {
...
function hasPluginOwnedWebFetchConfig(cfg: DurarConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  if (!isRecord(pluginConfig)) {
    return false;
  }
...
function hasPluginOwnedToolConfig(cfg: DurarConfig, pluginId: string): boolean {
  if (pluginId === "xai") {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.Durar) || !isRecord(entry.Durar.channel)) {
      continue;
    }
    const channel = entry.Durar.channel;
...
function collectCandidateChannelIds(cfg: DurarConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: DurarConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: DurarConfig): boolean {
  const entries = cfg.plugins?.entries;
  if (!entries || typeof entries !== "object") {
    return false;
  }
...
function configMayNeedPluginManifestRegistry(cfg: DurarConfig): boolean {
...
function configMayNeedPluginAutoEnable(cfg: DurarConfig, env: NodeJS.ProcessEnv): boolean {
...
function hasBrowserToolReference(cfg: DurarConfig): boolean {
...
function hasExplicitBrowserPluginEntry(cfg: DurarConfig): boolean {
  return Boolean(
    cfg.plugins?.entries && Object.prototype.hasOwnProperty.call(cfg.plugins.entries, "browser"),
  );
}

function resolveBrowserAutoEnableReason(cfg: DurarConfig): string | null {
...
function isPluginExplicitlyDisabled(cfg: DurarConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: DurarConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: DurarConfig, pluginId: string): DurarConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function applyPluginAutoEnable(params: {
  ...
  const config = params.config ?? ({} as DurarConfig);
