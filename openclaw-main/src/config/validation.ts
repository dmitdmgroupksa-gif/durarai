...
import type { DurarConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.Durar.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: DurarConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: DurarConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: DurarConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DurarSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as DurarConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: DurarConfig | null | undefined;

  const ensureCompatConfig = (): DurarConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Durar config schema.
