...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "Durar", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "Durar", version: "2.3.4" });
  ...
  it("ignores non-Durar package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers Durar_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        Durar_VERSION: "9.9.9",
        Durar_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale Durar_VERSION for compatibility checks", () => {
    const previous = process.env.Durar_VERSION;
    const previousService = process.env.Durar_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.Durar_VERSION = "2026.3.25";
      process.env.Durar_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.Durar_VERSION = previous;
      process.env.Durar_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        Durar_VERSION: "2026.3.99",
        Durar_SERVICE_VERSION: "2026.3.98",
