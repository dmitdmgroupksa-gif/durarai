...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("Durar-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("Durar-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("Durar-lid-a-", async (first) => {
      await withTempDir("Durar-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.Durar when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "Durar-config-dir-"));
    try {
  ...
  it("expands Durar_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/Durar-home",
      Durar_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/Durar-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers Durar_HOME over HOME", () => {
    vi.stubEnv("Durar_HOME", "/srv/Durar-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/Durar-home"));
...
describe("shortenHomePath", () => {
  it("uses $Durar_HOME prefix when Durar_HOME is set", () => {
    vi.stubEnv("Durar_HOME", "/srv/Durar-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/Durar-home")}/.Durar/Durar.json`)).toBe(
      "$Durar_HOME/.Durar/Durar.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $Durar_HOME replacement when Durar_HOME is set", () => {
    vi.stubEnv("Durar_HOME", "/srv/Durar-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/Durar-home")}/.Durar/Durar.json`),
    ).toBe("config: $Durar_HOME/.Durar/Durar.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/Durar", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "Durar"),
    );
  });
  ...
  it("prefers Durar_HOME for tilde expansion", () => {
    vi.stubEnv("Durar_HOME", "/srv/Durar-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/Durar")).toBe(path.resolve("/srv/Durar-home", "Durar"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/Durar", env)).toBe(path.resolve("/srv/Durar-home", "Durar"));
  });
