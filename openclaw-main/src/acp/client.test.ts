...
const createTempDir = () => tempDirs.make("Durar-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets Durar_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "Durar",
    });

    expect(env.Durar_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("Durar");
  });

  it("overrides pre-existing Durar_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      Durar_SHELL: "wrong",
    });
    expect(env.Durar_SHELL).toBe("acp-client");
  ...
  it("preserves Durar_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        Durar_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["Durar_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Durar bridge", () => {
    ...
    expect(env.Durar_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.Durar_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("Durar_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "Durar", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "Durar", "dist", "entry.js");
    const shimPath = path.join(dir, "Durar.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\Durar\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/Durar-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/Durar-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/Durar-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
