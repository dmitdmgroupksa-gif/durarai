...
type ConfiguredBinding = NonNullable<DurarConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: DurarConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/Durar" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
