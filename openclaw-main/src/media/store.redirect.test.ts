...
const HOME = path.join(os.tmpdir(), "Durar-home-redirect");
...
describe("media store redirects", () => {
  ...
  beforeAll(async () => {
    envSnapshot = captureEnv(["Durar_STATE_DIR"]);
    await fs.rm(HOME, { recursive: true, force: true });
    process.env.Durar_STATE_DIR = HOME;
  });
  ...
  it("strips sensitive headers when a redirect crosses origins", async () => {
    ...
    expect(secondHeaders.get("user-agent")).toBe("Durar-Test/1.0");
  });
