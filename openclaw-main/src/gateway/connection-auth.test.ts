...
function cfg(input: Partial<DurarConfig>): DurarConfig {
  return input as DurarConfig;
}
...
const DEFAULT_ENV = {
  Durar_GATEWAY_TOKEN: "env-token",
  Durar_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when Durar env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when Durar env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when Durar env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
