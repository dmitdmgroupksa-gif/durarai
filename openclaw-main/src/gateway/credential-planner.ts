...
type GatewaySecretDefaults = NonNullable<DurarConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${Durar_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.Durar_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.Durar_GATEWAY_PASSWORD);
}
