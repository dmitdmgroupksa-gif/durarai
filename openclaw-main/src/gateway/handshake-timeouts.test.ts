...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers Durar_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        Durar_HANDSHAKE_TIMEOUT_MS: "75",
        Durar_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        Durar_HANDSHAKE_TIMEOUT_MS: "",
        Durar_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
