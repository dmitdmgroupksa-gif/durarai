...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([Durar_MODEL_ID, Durar_DEFAULT_MODEL_ID]);
  ids.add(`Durar/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`Durar/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== Durar_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
