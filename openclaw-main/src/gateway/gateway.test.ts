...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".Durar", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "Durar");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".Durar");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "Durar.json");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.Durar_STATE_DIR = path.join(tempHome, ".Durar");
      delete process.env.Durar_CONFIG_PATH;
      process.env.Durar_SKIP_CHANNELS = "1";
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-wizard-home-"));
      process.env.HOME = tempHome;
