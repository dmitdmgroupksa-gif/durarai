...
export function resolveHooksConfig(cfg: DurarConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: DurarConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-Durar-token"] === "string"
      ? req.headers["x-Durar-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-Durar-idempotency-key"]) ||
