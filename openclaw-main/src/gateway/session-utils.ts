...
import { type DurarConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: DurarConfig): string[] {
...
export function listAgentsForGateway(cfg: DurarConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: DurarConfig): string {
...
function resolveSessionStoreAgentId(cfg: DurarConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: DurarConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: DurarConfig): GatewaySessionsDefaults {
