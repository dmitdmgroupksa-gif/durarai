...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("Durar_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "Durar", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `Durar` or `Durar/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "Durar", input: "hi" },
        { "x-Durar-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "Durar/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "Durar/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "Durar", input: "hi" },
        { "x-Durar-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-Durar-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats HTTP callers as non-owner regardless of requested scopes", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "Durar", input: "hi" },
      { "x-Durar-scopes": "operator.admin, operator.write" },
    );
