...
type PluginLoadConfig = NonNullable<Parameters<typeof loadDurarPlugins>[0]>["config"];
type PluginRegistry = ReturnType<typeof loadDurarPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "Durar-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.Durar_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "Durar.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function writeBundledPlugin(params: {
  ...
  process.env.Durar_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".Durar", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ Durar_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.Durar_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadDurarPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          Durar: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.Durar_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadDurarPlugins>) {
...
function useNoBundledPlugins() {
  process.env.Durar_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadDurarPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadDurarPlugins>[0], "cache" | "config">,
) {
  return loadDurarPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "Durar.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function loadBundleFixture(params: {
  ...
  const stateDir = makeTempDir();
  const bundleRoot = path.join(workspaceDir, ".Durar", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ Durar_STATE_DIR: stateDir, ...params.env }, () =>
    loadDurarPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadDurarPlugins>,
  pluginId: string,
) {
  expect(
    registry.diagnostics.some(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadDurarPlugins>;
  loadSecond: () => ReturnType<typeof loadDurarPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadDurarPlugins>;
  loadVariant: () => ReturnType<typeof loadDurarPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const DurarHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(DurarHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    Durar_HOME: DurarHome,
    HOME: ignoredHome,
    Durar_STATE_DIR: stateDir,
    Durar_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadDurarPlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.Durar_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.Durar_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".Durar", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ Durar_STATE_DIR: stateDir }, () =>
      loadDurarPlugins({
        workspaceDir,
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadDurarPlugins>) => {
...
module.exports = { id: "manifest-only-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "Durar.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__Durar_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadDurarPlugins(options);
        const scoped = loadDurarPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadDurarPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadDurarPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadDurarPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadDurarPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadDurarPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadDurarPlugins({
              ...
              env: {
                ...process.env,
                Durar_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadDurarPlugins({
              ...
              env: {
                ...process.env,
                Durar_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadDurarPlugins({
              ...
              env: {
                ...
                Durar_STATE_DIR: stateDir,
                Durar_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadDurarPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(DurarHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadDurarPlugins(options),
          loadVariant: () =>
            loadDurarPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadDurarPlugins({
      env: {
        ...
        HOME: homeDir,
        Durar_HOME: undefined,
        Durar_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers Durar_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const DurarHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "Durar-home-demo",
      dir: path.join(DurarHome, "plugins", "Durar-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "Durar-home-demo", register() {} };`,
    });
    ...
    const registry = loadDurarPlugins({
      ...
      config: {
        plugins: {
          allow: ["Durar-home-demo"],
          entries: {
            "Durar-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/Durar-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "Durar-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadDurarPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadDurarPlugins>) => {
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "Durar.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "Durar.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const broadSetupRegistry = loadDurarPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadDurarPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@Durar/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@Durar/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@Durar/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@Durar/setup-runtime-not-preferred-test",
  ...
  register(api) {
    ...
    const registry = await loadDurarPluginCliRegistry({
      config: {
        plugins: {
          load: { paths: [plugin.file] },
          allow: ["config-cli"],
  ...
  it("uses the real channel entry in cli-metadata mode for CLI metadata capture", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@Durar/cli-metadata-channel",
          Durar: { extensions: ["./index.cjs"], setupEntry: "./setup-entry.cjs" },
        },
        null,
        2,
      ),
    ...
    fs.writeFileSync(
      path.join(pluginDir, "index.cjs"),
      `const { defineChannelPluginEntry } = require("Durar/plugin-sdk/core");
require("node:fs").writeFileSync(${JSON.stringify(fullMarker)}, "loaded", "utf-8");
module.exports = {
  ...
  it("collects channel CLI metadata during full plugin loads", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@Durar/full-cli-metadata-channel",
          Durar: { extensions: ["./index.cjs"] },
        },
        null,
        2,
      ),
...
module.exports = {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "Durar.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "Durar.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ Durar_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("Durar/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ Durar_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadDurarPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__DurarLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("Durar/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("Durar/plugin-sdk");
