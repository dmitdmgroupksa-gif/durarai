...
const APPROVALS_PATH = "~/.Durar/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "Durar-app-server:thread:",
  "Durar-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("Durar.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("Durar.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Durar instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
