...
import type { DurarPluginService, DurarPluginServiceContext } from "./types.js";
...
function createRegistry(services: DurarPluginService[]) {
...
function expectServiceLogger(ctx: DurarPluginServiceContext) {
...
function expectServiceContexts(
  contexts: DurarPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: DurarPluginServiceContext[] = [];
