...
let loadDurarPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/Durar.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/Durar.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectRuntimeProviderResolution(
  ...
  expect(loadDurarPluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadDurarPluginsMock = vi
      .spyOn(loaderModule, "loadDurarPlugins")
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: (_config: { plugins?: Record<string, unknown> }, env: NodeJS.ProcessEnv) => {
        env.Durar_HOME = "/tmp/Durar-home-b";
      },
    },
  ] as const)("$name", ({ mutate }) => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({ Durar_HOME: "/tmp/Durar-home-a" });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        Durar_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadDurarPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      Durar_PLUGIN_DISCOVERY_CACHE_MS: "5",
      Durar_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadDurarPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      Durar_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.Durar_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
