...
type LoadDurarPlugins = typeof import("./loader.js").loadDurarPlugins;
...
const loadDurarPluginsMock = vi.fn<LoadDurarPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/Durar.plugin.json`,
  };
}
...
function getLastLoadPluginsCall(): Record<string, unknown> {
  const call = loadDurarPluginsMock.mock.calls.at(-1)?.[0];
...
function createAutoEnabledProviderConfig() {
  const rawConfig: DurarConfig = {
    plugins: {},
  };
  const autoEnabledConfig: DurarConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    vi.resetModules();
    vi.doMock("./loader.js", () => ({
      loadDurarPlugins: (...args: Parameters<LoadDurarPlugins>) =>
        loadDurarPluginsMock(...args),
    }));
  ...
  beforeEach(() => {
    loadDurarPluginsMock.mockReset();
