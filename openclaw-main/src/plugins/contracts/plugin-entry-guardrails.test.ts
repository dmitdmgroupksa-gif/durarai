...
const CORE_PLUGIN_ENTRY_IMPORT_RE =
  /import\s*\{[^}]*\bdefinePluginEntry\b[^}]*\}\s*from\s*"Durar\/plugin-sdk\/core"/;
    ...
    for (const entry of readdirSync(EXTENSIONS_DIR, { withFileTypes: true })) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          Durar?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.Durar?.extensions) ? pkg.Durar.extensions : [];
        if (
          extensions.some(
