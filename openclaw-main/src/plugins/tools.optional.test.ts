...
const loadDurarPluginsMock = vi.fn();
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadDurarPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
