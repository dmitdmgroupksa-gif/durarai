...
import type { DurarPluginCliCommandDescriptor } from "./types.js";
...
function resolvePluginCliLoadContext(cfg?: DurarConfig, env?: NodeJS.ProcessEnv) {
...
async function loadPluginCliMetadataRegistry(
  ...
  return {
    ...context,
    registry: await loadDurarPluginCliRegistry({
...
async function loadPluginCliCommandRegistry(
  ...
  const context = resolvePluginCliLoadContext(cfg, env);
  const runtimeRegistry = loadDurarPlugins({
    config: context.config,
    activationSourceConfig: context.rawConfig,
    autoEnabledReasons: context.autoEnabledReasons,
    workspaceDir: context.workspaceDir,
  ...
  try {
    const metadataRegistry = await loadDurarPluginCliRegistry({
      config: context.config,
      activationSourceConfig: context.rawConfig,
      autoEnabledReasons: context.autoEnabledReasons,
      workspaceDir: context.workspaceDir,
...
export async function getPluginCliCommandDescriptors(
  cfg?: DurarConfig,
  env?: NodeJS.ProcessEnv,
): Promise<DurarPluginCliCommandDescriptor[]> {
  try {
    const { registry } = await loadPluginCliMetadataRegistry(cfg, env);
    const seen = new Set<string>();
    const descriptors: DurarPluginCliCommandDescriptor[] = [];
