...
SMOKE_PREVIOUS_VERSION="${Durar_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${Durar_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="Durar"
PACKAGE_NAME="${Durar_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (Durar_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${Durar_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${Durar_INSTALL_LATEST_OUT:-}"
fi
