...
- regenerates apps/ios/Durar.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${Durar_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid Durar_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid Durar_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid Durar_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid Durar_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing Durar_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(Durar_URL_SLASH)$(Durar_URL_SLASH)#g'
)"
...
Durar_CODE_SIGN_STYLE = Automatic
Durar_DEVELOPMENT_TEAM = ${TEAM_ID}
Durar_IOS_SELECTED_TEAM = ${TEAM_ID}
Durar_APP_BUNDLE_ID = ai.Durar.client
Durar_SHARE_BUNDLE_ID = ai.Durar.client.share
Durar_ACTIVITY_WIDGET_BUNDLE_ID = ai.Durar.client.activitywidget
Durar_WATCH_APP_BUNDLE_ID = ai.Durar.client.watchkitapp
Durar_WATCH_EXTENSION_BUNDLE_ID = ai.Durar.client.watchkitapp.extension
Durar_APP_PROFILE =
Durar_SHARE_PROFILE =
Durar_PUSH_TRANSPORT = relay
Durar_PUSH_DISTRIBUTION = official
Durar_URL_SLASH = /
Durar_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
Durar_PUSH_APNS_ENVIRONMENT = production
EOF
