...
BASE_IMAGE="${BASE_IMAGE:-Durar-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-Durar-sandbox-common:bookworm-slim}"
...
Durar_DOCKER_BUILD_USE_BUILDX="${Durar_DOCKER_BUILD_USE_BUILDX:-0}"
Durar_DOCKER_BUILD_CACHE_FROM="${Durar_DOCKER_BUILD_CACHE_FROM:-}"
Durar_DOCKER_BUILD_CACHE_TO="${Durar_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${Durar_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${Durar_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${Durar_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${Durar_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${Durar_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=Durar.sandbox=1)
NOTE
