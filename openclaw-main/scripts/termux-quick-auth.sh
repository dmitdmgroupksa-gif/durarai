...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/Durar/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Durar Re-Auth" -c "After getting key, run: ssh $SERVER '~/Durar/scripts/mobile-reauth.sh'" --id Durar-auth
        ;;
    *)
