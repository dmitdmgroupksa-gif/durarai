...
const warningFilterKey = Symbol.for("Durar.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadDurarPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadDurarPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "Durar.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'Durar/plugin-sdk';",
...
const registry = loadDurarPlugins({
  ...
  env: {
    ...process.env,
    Durar_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
