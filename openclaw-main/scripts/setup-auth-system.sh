#!/bin/bash
# Setup Durar Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'Durar-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Durar message: Send warning via Durar itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/Durar-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/Durar-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/Durar-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now Durar-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status Durar-auth-monitor.timer"
echo ""
echo "Quick commands:"
