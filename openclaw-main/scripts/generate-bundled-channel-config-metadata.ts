...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const DurarMeta =
    ...
    "Durar" in source.packageJson
      ? (source.packageJson.Durar as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    DurarMeta &&
    typeof DurarMeta.channel === "object" &&
    DurarMeta.channel &&
    !Array.isArray(DurarMeta.channel)
      ? (DurarMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
