#!/usr/bin/env bash
# ClawDock - Docker helpers for Durar
# Inspired by Simon Willison's "Running Durar in Docker"
# https://til.simonwillison.net/llms/Durar-docker
#
# Installation:
...
# Common paths to check for Durar
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/Durar"
  "${HOME}/workspace/Durar"
  "${HOME}/projects/Durar"
  "${HOME}/dev/Durar"
  "${HOME}/code/Durar"
  "${HOME}/src/Durar"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Durar at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/Durar"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Durar not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/dmitdmgroupksa-gif/durarai.git ~/Durar"
    echo "  cd ~/Durar && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^Durar_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d Durar-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart Durar-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f Durar-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.Durar"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show Durar.json
  if [[ -f "${config_dir}/Durar.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/Durar.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/Durar.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No Durar.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec Durar-gateway \
    bash -c 'echo "alias Durar=\"./Durar.mjs\"" > /tmp/.bashrc_Durar && bash --rcfile /tmp/.bashrc_Durar'
}
...
clawdock-cli() {
  _clawdock_compose run --rm Durar-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build Durar-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d Durar-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build Durar-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "Durar_GATEWAY_TOKEN=$token" Durar-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" Durar-gateway \
    bash -c './Durar.mjs config set gateway.remote.token "$TOKEN" && ./Durar.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec Durar-gateway \
    bash -c "./Durar.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart Durar-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm Durar-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec Durar-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'Durar config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Durar${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (Durar alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to Durar project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.Durar)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'Durar channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'Durar status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.Durar.ai${_CLR_RESET}"
  echo ""
}
