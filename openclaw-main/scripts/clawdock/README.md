...
Inspired by Simon Willison's [Running Durar in Docker](https://til.simonwillison.net/llms/Durar-docker).
...
Canonical docs page: https://docs.Durar.ai/install/clawdock
...
On first command, ClawDock auto-detects your Durar directory:

- Checks common paths (`~/Durar`, `~/workspace/Durar`, etc.)
...
| `clawdock-cli <command>`  | Run Durar CLI commands                      |
...
| `clawdock-config`      | Open the Durar config directory        |
...
| File                       | Purpose                                                                    |
| -------------------------- | -------------------------------------------------------------------------- |
| `Dockerfile`               | Builds the `Durar:local` image (Node 22, pnpm, non-root `node` user)    |
| `docker-compose.yml`       | Defines `Durar-gateway` and `Durar-cli` services, bind-mounts, ports |
...
| --------------------------- | ------------------------------------------------ | ------------------------------------------------------------------- |
| `<project>/.env`            | **Docker infra** — image, ports, gateway token   | `Durar_GATEWAY_TOKEN`, `Durar_IMAGE`, `Durar_GATEWAY_PORT` |
| `~/.Durar/.env`          | **Secrets** — API keys and bot tokens            | `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `TELEGRAM_BOT_TOKEN`         |
| `~/.Durar/Durar.json` | **Behavior config** — models, channels, policies | Model selection, WhatsApp allowlists, agent settings                |

**Do NOT** put API keys or bot tokens in `Durar.json`. Use `~/.Durar/.env` for all secrets.

### Initial Setup

`./docker-setup.sh` (in the project root) handles first-time Docker configuration:

- Builds the `Durar:local` image from `Dockerfile`
- Creates `<project>/.env` from `.env.example` with a generated gateway token
- Sets up `~/.Durar` directories if they don't exist
...
- `Durar_DOCKER_APT_PACKAGES` — extra apt packages to install (e.g. `ffmpeg`)
- `Durar_INSTALL_BROWSER=1` — pre-install Chromium for browser automation (adds ~300MB, but skips the 60-90s Playwright install on each container start)
...
- `~/.Durar/.env` is available inside the container at `/home/node/.Durar/.env` — Durar loads it automatically as the global env fallback
- `~/.Durar/Durar.json` is available at `/home/node/.Durar/Durar.json` — the gateway watches it and hot-reloads most changes
...
The project `.env` feeds Docker Compose directly (gateway token, image name, ports). The `~/.Durar/.env` feeds the Durar process inside the container.
...
```bash
Durar_CONFIG_DIR=/Users/you/.Durar
Durar_WORKSPACE_DIR=/Users/you/.Durar/workspace
Durar_GATEWAY_PORT=18789
Durar_BRIDGE_PORT=18790
Durar_GATEWAY_BIND=lan
Durar_GATEWAY_TOKEN=<generated-by-docker-setup>
Durar_IMAGE=Durar:local
```

### Env Precedence

Durar loads env vars in this order (highest wins, never overrides existing):
...
3. **`~/.Durar/.env`** — global secrets (API keys, bot tokens)
4. **`Durar.json` `env` block** — inline vars, applied only if still missing
5. **Shell env import** — optional login-shell scrape (`Durar_LOAD_SHELL_ENV=1`)

## Common Workflows

### Update Durar

> **Important:** `Durar update` does not work inside Docker.
...
```bash
Durar channels login --channel whatsapp --verbose
```
...
2. Configure it in the Durar config
...
- Bash or Zsh shell
- Durar project (run `scripts/docker/setup.sh`)
