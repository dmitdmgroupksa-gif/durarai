#!/usr/bin/env bash
# One-time host setup for rootless Durar in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.Durar by default, and uses the repo-local
# launch script at ./scripts/run-Durar-podman.sh.
#
...
#   Or set Durar_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-Durar-podman.sh launch
#   ./scripts/run-Durar-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start Durar.service
set -euo pipefail

REPO_PATH="${Durar_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-Durar-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/Durar.container.in"
Durar_USER="$(id -un)"
Durar_HOME="${HOME:-}"
Durar_CONFIG_DIR="${Durar_CONFIG_DIR:-}"
Durar_WORKSPACE_DIR="${Durar_WORKSPACE_DIR:-}"
Durar_IMAGE="${Durar_PODMAN_IMAGE:-${Durar_IMAGE:-Durar:local}}"
Durar_CONTAINER_NAME="${Durar_PODMAN_CONTAINER:-Durar}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${Durar_PODMAN_GATEWAY_HOST_PORT:-${Durar_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate Durar_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${Durar_PODMAN_QUADLET:-}" ]]; then
  case "${Durar_PODMAN_QUADLET,,}" in
...
if [[ "$Durar_IMAGE" == "Durar:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set Durar_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$Durar_HOME" ]]; then
  Durar_HOME="$(resolve_user_home "$Durar_USER")"
fi
if [[ -z "$Durar_HOME" ]]; then
  echo "Unable to resolve HOME for user $Durar_USER." >&2
  exit 1
fi
if [[ -z "$Durar_CONFIG_DIR" ]]; then
  Durar_CONFIG_DIR="$Durar_HOME/.Durar"
fi
if [[ -z "$Durar_WORKSPACE_DIR" ]]; then
  Durar_WORKSPACE_DIR="$Durar_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$Durar_HOME"
validate_mount_source_path "config directory" "$Durar_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$Durar_WORKSPACE_DIR"
validate_container_name "$Durar_CONTAINER_NAME"
validate_image_name "$Durar_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$Durar_CONFIG_DIR" "$Durar_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$Durar_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$Durar_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${Durar_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "Durar_DOCKER_APT_PACKAGES=${Durar_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${Durar_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "Durar_EXTENSIONS=${Durar_EXTENSIONS}")
fi

if [[ "$Durar_IMAGE" == "Durar:local" ]]; then
  echo "Building image $Durar_IMAGE ..."
  podman build -t "$Durar_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$Durar_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $Durar_IMAGE"
  else
    echo "Pulling image $Durar_IMAGE ..."
    podman pull "$Durar_IMAGE"
  fi
fi

ENV_FILE="$Durar_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
  TOKEN="$(generate_token_hex_32)"
  (
    umask 077
    write_file_atomically "$ENV_FILE" 600 <<EOF
Durar_GATEWAY_TOKEN=$TOKEN
EOF
  )
  echo "Generated Durar_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "Durar_PODMAN_CONTAINER" "$Durar_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "Durar_PODMAN_IMAGE" "$Durar_IMAGE"

CONFIG_JSON="$Durar_CONFIG_DIR/Durar.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$Durar_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/Durar.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  Durar_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$Durar_HOME")"
  Durar_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$Durar_CONFIG_DIR")"
  Durar_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$Durar_WORKSPACE_DIR")"
  Durar_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$Durar_IMAGE")"
  Durar_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$Durar_CONTAINER_NAME")"
  sed \
    -e "s|{{Durar_HOME}}|$Durar_HOME_ESCAPED|g" \
    -e "s|{{Durar_CONFIG_DIR}}|$Durar_CONFIG_ESCAPED|g" \
    -e "s|{{Durar_WORKSPACE_DIR}}|$Durar_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$Durar_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$Durar_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start Durar.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start Durar.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-Durar-podman.sh launch"
echo "  ./scripts/run-Durar-podman.sh launch setup"
echo "  Durar --container $Durar_CONTAINER_NAME dashboard --no-open"
