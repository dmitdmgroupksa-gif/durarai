...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/Durar-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: Durar@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/Durar-main-$CURRENT_HEAD_SHORT.tgz"
  cp "$MAIN_TGZ_DIR/$pkg" "$MAIN_TGZ_PATH"
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/Durar-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/Durar warnings do not
...
try {
  $env:PATH = "$env:LOCALAPPDATA\Durar\deps\portable-git\cmd;$env:LOCALAPPDATA\Durar\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\Durar\deps\portable-git\usr\bin;$env:PATH"
  $tgz = Join-Path $env:TEMP 'Durar-main-update.tgz'
  ...
  $version = Invoke-CaptureLogged 'Durar --version' { & $Durar --version }
  if ($version -notmatch [regex]::Escape($HeadShort)) {
    throw "version mismatch: expected substring $HeadShort"
  }
  Invoke-Logged 'Durar models set' { & $Durar models set $ModelId }
  ...
  Invoke-Logged 'Durar gateway restart' { & $Durar gateway restart }
  Start-Sleep -Seconds 5
  Invoke-Logged 'Durar gateway status' { & $Durar gateway status --deep --require-rpc }
  Invoke-CaptureLogged 'Durar agent' { & $Durar agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/Durar-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = [match for match in matches if re.search(r"Durar \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="Durar-update-$RANDOM-$RANDOM.ps1"
  log_name="Durar-update-$RANDOM-$RANDOM.log"
  done_name="Durar-update-$RANDOM-$RANDOM.done"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/Durar-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/Durar-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/Durar-main-update.tgz
version="\$(/opt/homebrew/bin/Durar --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/Durar models set "$MODEL_ID"
/opt/homebrew/bin/Durar gateway status --deep --require-rpc
/usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /opt/homebrew/bin/Durar agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/Durar-main-update.sh
}
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/Durar-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/Durar-main-update.tgz
npm install -g /tmp/Durar-main-update.tgz --no-fund --no-audit
version="\$(Durar --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
Durar models set "$MODEL_ID"
Durar agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/Durar-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="Durar@$LATEST_VERSION"
fi
