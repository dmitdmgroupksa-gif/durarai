...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-Durar-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://Durar.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/Durar-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-Durar-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://Durar.ai/install.ps1
...
\$Durar = Join-Path \$env:APPDATA 'npm\Durar.cmd'
\$args = $args_literal
if ('${env_name_q}' -ne '') {
  Set-Item -Path ('Env:' + '${env_name_q}') -Value '${env_value_q}'
}
# Durar.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$Durar @args 2>&1
if (\$null -ne \$output) {
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "Durar-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\Durar\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\Durar\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\Durar\\deps\" mkdir \"%LOCALAPPDATA%\\Durar\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\Durar\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\Durar\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\Durar\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Durar\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Durar\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/Durar-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\Durar.cmd') --version
EOF
)"
...
install_main_tgz() {
  ...
  run_windows_retry "main tgz install" 2 \
    guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\Durar\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Durar\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Durar\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\Durar.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_Durar "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/Durar-onboard-$PROVIDER.ps1"
...
try {
  \$Durar = Join-Path \$env:APPDATA 'npm\Durar.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$Durar, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="Durar-onboard-$RANDOM-$RANDOM.ps1"
  log_name="Durar-onboard-$RANDOM-$RANDOM.log"
  done_name="Durar-onboard-$RANDOM-$RANDOM.done"
...
verify_gateway() {
  guest_run_Durar "" "" gateway status --deep --require-rpc
}
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="Durar-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="Durar-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="Durar-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$Durar = Join-Path \$env:APPDATA 'npm\Durar.cmd'
  & \$Durar gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_Durar "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_Durar "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_Durar "" "" gateway status --deep
}

verify_turn() {
  guest_run_Durar "" "" models set "$MODEL_ID"
  guest_run_Durar "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "Durar-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "Durar-main-upgrade.tgz" || return $?
