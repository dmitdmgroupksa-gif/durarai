...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/Durar-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/Durar-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/Durar-install.sh
  guest_exec /usr/bin/env Durar_NO_ONBOARD=1 bash /tmp/Durar-install.sh "${version_args[@]}" --no-onboard
  guest_exec Durar --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec Durar --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" Durar onboard \
    --non-interactive \
    --mode local \
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "Durar gateway run" >/dev/null 2>&1 || true
rm -f /tmp/Durar-parallels-linux-gateway.log
setsid sh -lc 'exec env Durar_HOME=/root Durar_STATE_DIR=/root/.Durar Durar_CONFIG_PATH=/root/.Durar/Durar.json ${API_KEY_ENV}=${api_key_value_q} Durar gateway run --bind loopback --port 18789 --force >/tmp/Durar-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec Durar gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec Durar gateway status --deep --require-rpc
    return
  fi
  guest_exec Durar gateway status --deep
}

verify_local_turn() {
  guest_exec Durar models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" Durar agent \
    --local \
    --agent main \
    --message ping \
    --json
