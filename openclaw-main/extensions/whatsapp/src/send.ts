...
import { generateSecureUuid } from "Durar/plugin-sdk/core";
...
import { getChildLogger } from "Durar/plugin-sdk/text-runtime";
import { redactIdentifier } from "Durar/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "Durar/plugin-sdk/text-runtime";
import { markdownToWhatsApp } from "Durar/plugin-sdk/text-runtime";
import { toWhatsappJid } from "Durar/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: DurarConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
