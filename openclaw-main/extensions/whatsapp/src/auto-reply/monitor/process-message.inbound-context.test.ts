...
describe("web processMessage inbound context", () => {
  ...
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-process-message-"));
  ...
  it("does not force an [Durar] response prefix in self-chats when identity is unset", async () => {
    await processSelfDirectMessage({
      messages: {},
      session: { store: sessionStorePath },
    } as unknown as ReturnType<typeof import("Durar/plugin-sdk/config-runtime").loadConfig>);
