...
import { resolveReactionMessageId } from "Durar/plugin-sdk/channel-actions";
import { createChatChannelPlugin } from "Durar/plugin-sdk/core";
import { chunkText } from "Durar/plugin-sdk/reply-runtime";
import {
...
function areWhatsAppAgentReactionsEnabled(params: { cfg: DurarConfig; accountId?: string }) {
...
function hasAnyWhatsAppAccountWithAgentReactionsEnabled(cfg: DurarConfig) {
...
function resolveWhatsAppAgentReactionGuidance(params: { cfg: DurarConfig; accountId?: string }) {
  if (!params.cfg.channels?.whatsapp) {
    return undefined;
  }
  const gate = createActionGate(params.cfg.channels.whatsapp.actions);
