...
import { VERSION } from "Durar/plugin-sdk/cli-runtime";
import { danger, success } from "Durar/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "Durar/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "Durar/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("Durar channels login")}`,
              ),
            );
          }
        }
