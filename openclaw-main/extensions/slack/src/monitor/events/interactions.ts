...
// Prefix for Durar-generated action IDs to scope our handler
const Durar_ACTION_PREFIX = "Durar:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${Durar_ACTION_PREFIX}`);

  // Handle Durar modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
