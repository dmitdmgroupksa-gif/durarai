...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): DurarConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as DurarConfig;
  ...
  async function resolveReadToken(cfg: DurarConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: DurarConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
