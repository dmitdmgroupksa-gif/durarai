...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => DurarConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<DurarConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: DurarConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
