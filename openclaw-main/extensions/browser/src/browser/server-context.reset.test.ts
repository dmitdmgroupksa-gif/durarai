...
function localDurarProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "Durar",
...
function createLocalDurarResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localDurarProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveDurarUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-reset-"));
    const profileDir = path.join(tempRoot, "Durar");
    ...
    const ops = createLocalDurarResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveDurarUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Durar-reset-no-own-"));
    const profileDir = path.join(tempRoot, "Durar");
    fs.mkdirSync(profileDir, { recursive: true });
