...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".Durar-profile-decorated");
}
 ...
 */
export function decorateDurarProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_Durar_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_Durar_BROWSER_COLOR).toUpperCase();
