  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          Durar: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.Durar_GATEWAY_PORT;
    process.env.Durar_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.Durar_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.Durar_GATEWAY_PASSWORD;
