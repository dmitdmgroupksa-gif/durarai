...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.Durar_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "Durar",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/Durar" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    Durar: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchDurarChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.Durar_GATEWAY_PORT;
  process.env.Durar_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.Durar_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.Durar_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.Durar_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.Durar_GATEWAY_PASSWORD;
  } else {
    process.env.Durar_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
