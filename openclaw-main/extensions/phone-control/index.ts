...
function normalizeDenyList(cfg: DurarPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: DurarPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: DurarPluginApi) {
    ...
    const timerService: DurarPluginService = {
