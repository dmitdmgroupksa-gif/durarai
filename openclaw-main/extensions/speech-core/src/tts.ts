...
import { normalizeChannelId, type ChannelId } from "Durar/plugin-sdk/channel-runtime";
import type {
  DurarConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "Durar/plugin-sdk/logging-core";
...
import { isVerbose, logVerbose } from "Durar/plugin-sdk/runtime-env";
import { resolvePreferredDurarTmpDir } from "Durar/plugin-sdk/sandbox";
import { CONFIG_DIR, resolveUserPath, stripMarkdown } from "Durar/plugin-sdk/text-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.Durar_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: DurarConfig) {
...
function resolveRegistryDefaultSpeechProviderId(cfg?: DurarConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: DurarConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: DurarConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: DurarConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: DurarConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredDurarTmpDir();
