...
import { getRuntimeConfigSnapshot } from "Durar/plugin-sdk/config-runtime";
import type { DurarConfig } from "Durar/plugin-sdk/plugin-entry";
import {
  jsonResult,
  readCache,
  ...
  writeCache,
} from "Durar/plugin-sdk/provider-web-search";
import {
...
const X_SEARCH_CACHE_KEY = Symbol.for("Durar.xai.x-search.cache");
...
function readLegacyGrokApiKey(cfg?: DurarConfig): string | undefined {
...
function readPluginXaiWebSearchApiKey(cfg?: DurarConfig): string | undefined {
  return readConfiguredSecretString(
...
function resolveFallbackXaiApiKey(cfg?: DurarConfig): string | undefined {
  return readPluginXaiWebSearchApiKey(cfg) ?? readLegacyGrokApiKey(cfg);
}

function readLegacyXSearchApiKey(cfg?: DurarConfig): string | undefined {
...
function resolveXSearchConfig(cfg?: DurarConfig): Record<string, unknown> | undefined {
...
export function createXSearchTool(options?: {
  config?: DurarConfig;
  runtimeConfig?: DurarConfig | null;
}) {
