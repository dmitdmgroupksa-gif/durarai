...
`Durar.invoke --each`) without adding new Durar code per workflow.
...
This extension depends on Durar internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Durar extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.Durar/extensions` as a standalone plugin directory.
