...
metadata: { "Durar": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Durar `sessions_spawn`
- **File I/O** == Durar `read`/`write`
- **Remote fetch** == Durar `web_fetch` (or `exec` with curl when POST is required)
