...
function getChannelConfig(cfg: DurarConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: DurarConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: DurarConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: DurarConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Durar uses to send replies back to Chat.",
      ],
