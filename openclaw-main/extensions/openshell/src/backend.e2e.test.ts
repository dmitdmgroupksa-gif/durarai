...
import { createSandboxTestContext } from "Durar/plugin-sdk/testing";
...
const Durar_OPENSHELL_E2E = process.env.Durar_E2E_OPENSHELL === "1";
const Durar_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const Durar_OPENSHELL_COMMAND =
  process.env.Durar_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "Durar-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && Durar_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: Durar_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(Durar_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "Durar-openshell-e2e-"));
      ...
      const gatewayName = `Durar-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `Durar-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/Durar-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: Durar_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("Durar-openshell-e2e");
