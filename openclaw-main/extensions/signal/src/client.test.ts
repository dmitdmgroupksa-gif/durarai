...
vi.mock("Durar/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("Durar/plugin-sdk/core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/core")>();
  return {
    ...actual,
...
vi.mock("Durar/plugin-sdk/text-runtime", () => ({
