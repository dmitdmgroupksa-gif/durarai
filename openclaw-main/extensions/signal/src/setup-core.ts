...
export const signalDmPolicy = createTopLevelChannelDmPolicy({
  ...
  getCurrent: (cfg: DurarConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "Durar"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("Durar gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
