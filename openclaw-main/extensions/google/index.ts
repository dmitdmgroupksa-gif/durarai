import type { ImageGenerationProvider } from "Durar/plugin-sdk/image-generation";
...
import { createProviderApiKeyAuthMethod } from "Durar/plugin-sdk/provider-auth-api-key";
import type { ProviderPlugin } from "Durar/plugin-sdk/provider-model-shared";
import { createGoogleThinkingPayloadWrapper } from "Durar/plugin-sdk/provider-stream";
import {
...
async function loadGoogleGeminiCliProvider(): Promise<ProviderPlugin> {
  if (!googleGeminiCliProviderPromise) {
    googleGeminiCliProviderPromise = import("./gemini-cli-provider.js").then((mod) => {
      ...
      } as Pick<DurarPluginApi, "registerProvider"> as DurarPluginApi);
      if (!provider) {
