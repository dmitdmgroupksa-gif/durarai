...
import type { DurarPluginApi, DurarPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("Durar-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<DurarPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: DurarPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<DurarPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as DurarPluginApi);
