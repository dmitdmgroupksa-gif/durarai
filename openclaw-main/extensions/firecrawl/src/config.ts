...
type WebSearchConfig = NonNullable<DurarConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<DurarConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DurarConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: DurarConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: DurarConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: DurarConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: DurarConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: DurarConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: DurarConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: DurarConfig, override?: number): number {
