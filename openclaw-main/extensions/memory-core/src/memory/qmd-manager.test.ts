...
const MCPORTER_STATE_KEY = Symbol.for("Durar.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("Durar.qmdEmbedQueueTail");
...
vi.mock("Durar/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("Durar/plugin-sdk/memory-core-host-engine-foundation")
  >("Durar/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("Durar/plugin-sdk/file-lock", async (importOriginal) => {
  const actual = await importOriginal<typeof import("Durar/plugin-sdk/file-lock")>();
  return {
    ...actual,
...
import type { DurarConfig } from "Durar/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
  resolveMemoryBackendConfig,
} from "Durar/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: DurarConfig }) {
