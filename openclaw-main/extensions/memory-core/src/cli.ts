...
import {
  ...
  formatHelpExamples,
  theme,
} from "Durar/plugin-sdk/memory-core-host-runtime-cli";
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Durar memory status", "Show index and provider status."],
          ["Durar memory status --deep", "Probe embedding provider readiness."],
          ["Durar memory index --force", "Force a full reindex."],
          ['Durar memory search "meeting notes"', "Quick search using positional query."],
          [
            'Durar memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["Durar memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.Durar.ai/cli/memory")}\n`,
    );
