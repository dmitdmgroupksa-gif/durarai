...
import {
  ...
  resolveStateDir,
  type DurarConfig,
} from "Durar/plugin-sdk/memory-core-host-runtime-core";
import {
  listMemoryFiles,
  normalizeExtraMemoryPaths,
} from "Durar/plugin-sdk/memory-core-host-runtime-files";
...
function resolveAgent(cfg: DurarConfig, agent?: string) {
...
function resolveAgentIds(cfg: DurarConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
