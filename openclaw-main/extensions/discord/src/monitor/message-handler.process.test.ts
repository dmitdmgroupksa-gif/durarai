import { DEFAULT_EMOJIS } from "Durar/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/Durar-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("Durar/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("Durar/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("Durar/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/Durar-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/Durar-discord-process-test-sessions.json" },
      },
    });
