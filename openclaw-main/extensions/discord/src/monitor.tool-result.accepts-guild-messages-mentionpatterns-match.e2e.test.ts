  ...
  it("accepts guild messages when mentionPatterns match", async () => {
    const cfg = createMentionRequiredGuildConfig({
      messages: {
        ...
        groupChat: { mentionPatterns: ["\\bDurar\\b"] },
      },
    } as Partial<Config>);
    ...
    await handler(createGuildMessageEvent({ messageId: "m2", content: "Durar: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Durar" },
          },
        },
      }),
      client,
