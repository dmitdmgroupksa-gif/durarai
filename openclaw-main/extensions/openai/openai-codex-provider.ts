...
} from "Durar/plugin-sdk/plugin-entry";
import {
  CODEX_CLI_PROFILE_ID,
  ensureAuthProfileStore,
  listProfilesForProvider,
  type OAuthCredential,
} from "Durar/plugin-sdk/provider-auth";
import { buildOauthProviderAuthResult } from "Durar/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "Durar/plugin-sdk/provider-auth-login";
import {
  DEFAULT_CONTEXT_TOKENS,
  normalizeModelCompat,
  normalizeProviderId,
  type ProviderPlugin,
} from "Durar/plugin-sdk/provider-model-shared";
import { createOpenAIAttributionHeadersWrapper } from "Durar/plugin-sdk/provider-stream";
import { fetchCodexUsage } from "Durar/plugin-sdk/provider-usage";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `Durar models auth login --provider openai-codex` or `Durar configure`.";
}
