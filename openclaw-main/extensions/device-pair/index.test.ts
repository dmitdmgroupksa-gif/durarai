...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredDurarTmpDir: vi.fn(() => path.join(os.tmpdir(), "Durar-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredDurarTmpDir: pluginApiMocks.resolvePreferredDurarTmpDir,
...
function createApi(params?: {
  runtime?: DurarPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as DurarPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: DurarPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as DurarPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredDurarTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredDurarTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Durar iOS app:");
    expect(text).toContain("![Durar pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Durar iOS app:");
