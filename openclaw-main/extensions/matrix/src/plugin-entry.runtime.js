...
const Durar_PLUGIN_SDK_PREFIX = ["Durar", "plugin-sdk"].join("/");
...
function findDurarPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "Durar" && pkg.exports?.["./plugin-sdk"]) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findDurarPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    aliasMap[Durar_PLUGIN_SDK_PREFIX] = rootAlias;
  }
  ...
  for (const exportKey of Object.keys(packageJson.exports ?? {})) {
    ...
    if (resolvedPath) {
      aliasMap[`${Durar_PLUGIN_SDK_PREFIX}/${subpath}`] = resolvedPath;
    }
  }
  ...
  if (extensionApi) {
    aliasMap["Durar/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findDurarPackageRoot(moduleDir);
  if (location) {
