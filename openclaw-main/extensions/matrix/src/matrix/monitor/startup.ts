...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleDurarDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Durar devices detected for ${params.auth.userId}: ${deviceHealth.staleDurarDevices.map((device) => device.deviceId).join(", ")}. Run 'Durar matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
