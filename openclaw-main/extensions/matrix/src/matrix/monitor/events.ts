...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Durar runtime is using the same account. " +
    `Check 'Durar matrix verify status --verbose --account ${accountId}' and 'Durar matrix devices list --account ${accountId}'.`
  );
}
