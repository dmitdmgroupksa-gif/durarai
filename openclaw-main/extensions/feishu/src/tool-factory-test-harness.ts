import type { AnyAgentTool, DurarPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: DurarPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<DurarPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
