...
import type { PluginInteractiveTelegramHandlerContext } from "Durar/plugin-sdk/core";
import {
  clearPluginInteractiveHandlers,
  registerPluginInteractiveHandler,
} from "Durar/plugin-sdk/plugin-runtime";
import { expectChannelInboundContextContract as expectInboundContextContract } from "Durar/plugin-sdk/testing";
...
let normalizeTelegramCommandName: typeof import("Durar/plugin-sdk/config-runtime").normalizeTelegramCommandName;
...
describe("createTelegramBot", () => {
  beforeAll(async () => {
    ...
    ({ normalizeTelegramCommandName } = await import("Durar/plugin-sdk/config-runtime"));
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/Durar-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/Durar-telegram-model-compact-${process.pid}-${Date.now()}.json`;
    const config = {
      agents: {
        defaults: {
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/Durar-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Durar" },
        },
      },
      me: { id: 999, username: "Durar_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
