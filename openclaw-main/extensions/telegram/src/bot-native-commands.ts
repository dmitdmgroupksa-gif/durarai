...
import { getPluginCommandSpecs } from "Durar/plugin-sdk/plugin-runtime";
import { resolveAgentRoute } from "Durar/plugin-sdk/routing";
import { getRuntimeConfigSnapshot } from "Durar/plugin-sdk/runtime-config-snapshot";
import { danger, logVerbose } from "Durar/plugin-sdk/runtime-env";
...
} from "Durar/plugin-sdk/telegram-command-config";
...
type TelegramChunkMode = ReturnType<
  typeof import("Durar/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): DurarConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: DurarConfig): TelegramAccountConfig => {
    try {
