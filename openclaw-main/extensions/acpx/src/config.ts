...
import { buildPluginConfigSchema } from "Durar/plugin-sdk/core";
import { z } from "Durar/plugin-sdk/zod";
import type { DurarPluginConfigSchema } from "../runtime-api.js";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "Durar-plugin-tools";
const ACPX_BIN_NAME = process.platform === "win32" ? "acpx.cmd" : "acpx";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "Durar.plugin.json")) &&
...
function resolveDurarRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const DurarRoot = resolveDurarRoot(pluginRoot);
  const distEntry = path.join(DurarRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(DurarRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
export function createAcpxPluginConfigSchema(): DurarPluginConfigSchema {
